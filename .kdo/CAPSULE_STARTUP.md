...
git_head: d4dbfc582   ← 2026-08-26 晚 laowantong 会话更新（原值 33ed6487f 过期，漂移为 #546 备注等正常提交）   ← 启动时核对：git -C <wiki> rev-parse --short HEAD（2026-08-25 黄药师 #510 维护更新：风清扬路由行+身份卡补齐，提交后 HEAD 随本提交前进）
queue_tail: 546   ← 2026-08-26 晚 laowantong 会话更新（原值 543 过期，尾部已到 #546）       ← 启动时核对：grep 队列尾任务号（2026-08-25 黄药师 #510 维护更新）
```
...
> 🆕 **角色待办收件箱（#501，各角色启动必读）**：`90_control/todos/<拼音角色名>.md`——探针通知的 CLI 收件箱（飞书在外实例走；本地 CLI 实例启动读此文件即见待办：打回/终审/建议书/F-036 提醒）。
