...
status: reviewed
updated_at: '2026-08-23T18:17:31.873332+00:00'
version: v0.1
...
## 任务 2 硬性项补做（2026-08-24 黄药师，追加）

> 任务书执行期间被收紧（老朱 08-24 指令「每天整份快照必须取消」，风清扬 01:58 建议硬性）——首轮提审未含，现已补做，追加记录。

**补做内容**：日增量结构（取消按日整份）——
1. **取消按日整份目录**：capture 不再写 `L1_ROOT/today/tool`（跨天整份重拷=日增 770MB 根因），改 `L1_ROOT/<tool>/<rel>` 直接存储
2. **日增量**：mtime 增量判断（未变文件不复制不重复计体积）——二次跑实测 **13 新增/11098 跳过**
3. **周全量兜底**：旧天 zip 归档（2026-08-23/2026-08-24 旧按日目录已归档至 `L1-full-archive/`），新结构为增量主库；每周全量基线待立项（跨天增量已满足，基线另期）
4. **保留 L1 语义**：`trace-index.md` append 式索引（每次采集追加清单+mtime+大小）——「某时刻有哪些文件」可回溯，日增量不滑成"只留当前态丢历史"

**验证补充**：①首跑迁移 11111（旧结构一次性复制到新结构）；②二次跑 13/11098=增量生效；③旧按日目录已归档；④体积日志去重口径 844MB（红线 5000 触达大幅延后）；⑤`--verify` 主库自检 PASS

**未做项更新**：「周全量兜底（每周基线）」待立项——日增量+trace 索引已满足可回溯语义，周基线为灾备增强

*黄药师 · 2026-08-24 · 任务 2 硬性项补做*
...
**版本对齐三问**（代码类，全绿）：① 入仓：8eb98034a（02:08 主治理）+ 401f98786（02:11 任务 2 硬性补做）在 HEAD ② 生效：C 镜像已删/新结构实测 ③ 对齐：审查对象=HEAD

**O0 逐条溯源**：
1. **去 C 镜像（任务 1）** ✅：**C 镜像目录已删**（ls 实测不存在——1698MB 释放）；verify 改 D 单盘主库自检口径；体积统计只算 D 主库
2. **hash 去重** ✅：`_dir_size_mb` 按内容 hash 去重（体积口径真实化）
3. **旧天归档** ✅：`_archive_old_days` 幂等——`D:/KDO-memory/L1-full-archive/` 实存 2026-08-23.zip + 2026-08-24.zip
4. **任务 2 硬性补做（老朱指令）** ✅：**日增量结构**（取消按日整份——L1-full 顶层实测 = 6 工具目录 + trace-index.md，**无按日目录残留**）+ mtime 增量（报告实测 13 新增/11098 跳过）+ **trace-index.md append 式索引**（每次采集追加清单+mtime+大小——"某时刻有哪些文件"可回溯，日增量不滑成丢历史）
5. **体积实证** ✅：867MB（红线 5000——日增 770MB 的失控曲线被拉平，触达大幅延后）
6. **测试独立复现** ✅：8 passed（含 TestVolumeGovernance 2：hash 去重/旧天归档）；全量 73
7. **边界** ✅：只改体积/镜像逻辑（采集面 #489/调度 #471 不动）；容灾提示（L1 单盘 D——审计者义务已声明，同盘防误删镜像另立项）；周基线待立项（未做项诚实）

**发现问题**：🔵 无实质缺陷——观察项：周全量基线未做（日增量+trace 索引已满足可回溯语义，周基线为灾备增强——待立项）；L1 单盘 D 容灾（事件库仍 C+D 双盘——另行立项评估）

**魔鬼代言人**：3 个月后最可能出问题——日增量+mtime 判断在文件时间戳异常（未来时间戳/系统时钟漂移）时漏采（增量判断边界）；或 trace-index 膨胀（append 式长期增长——可归档轮转）

**存在性核查**（本意见书负向断言证据）：
- 「C 镜像已删」→ 核查：ls C:/Users/Administrator/.kdo-memory/L1-full-backup 不存在
- 「新结构」→ 核查：ls L1-full 顶层（6 工具目录+trace-index，无 2026-* 按日）
- 「归档」→ 核查：ls L1-full-archive（2 zip）
- 「8 passed」→ 核查：pytest 独立复现

**残余风险**：周基线待立项；单盘容灾另立项；mtime 增量边界观察。
