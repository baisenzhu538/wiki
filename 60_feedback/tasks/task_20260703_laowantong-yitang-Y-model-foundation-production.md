...
1. `00_inbox/底层逻辑之一-Y模型/底层逻辑之一Y模型-口述.txt`
2. `00_inbox/底层逻辑之一-Y模型/底层逻辑之一Y模型-笔记.txt`
...
6. OPC 智能体适配：可映射为 AI 输出审计助手 / 反面证据搜索助手。
7. Checklist ≥8 项。
8. Anti-patterns ≥4 个。
9. Critique + Related。
...
6. OPC 智能体适配：可映射为假设挑战助手 / 隐含假设挖掘助手。
7. Checklist ≥8 项。
8. Anti-patterns ≥4 个。
9. Critique + Related。
...
**主域**：epistemic-foundations / yitang  
**confidence**：0.85  
**trust_level**：high
...
**必须包含的 section**：
1. Background：Y模型火种·科学做事思考合集，段王爷整理。
...
**类型**：case  
**主域**：epistemic-foundations / marketing / yitang  
**confidence**：0.85  
**trust_level**：medium

**必须包含的 section**：
1. Background：2023 年一堂准备做抖音 AD 投放。
2. Problem：早期外包和内部团队都失败，ROI 0.3-0.6，每周亏损严重。
...
| 第五批 | `case-yitang-Y-model-advertising-turnaround` + `case-yitang-Y-model-seven-applications` | 两个案例 |
