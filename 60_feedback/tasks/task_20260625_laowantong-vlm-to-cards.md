...
### 3.0 VLM 漏挖清单（王语嫣基于洪七公重提取结果标注）

王语嫣已对全库 146 张待生产卡片的 VLM 描述做深度扫描，发现 **127 张存在明显漏挖**，主要缺口：

| 漏挖类型 | 数量 | 说明 |
|:---|:---:|:---|
| 缺少失败模式/边界/反例 | 104 | 一轮 VLM prompt 主要做视觉索引，极少提取失败模式 |
| 缺少操作步骤/SOP/检查单 | 22 | 图中隐含的“怎么做”未显式化 |
| 缺少暗知识/教训/心法标记 | 79 | 讲师随口说的心法、口诀、避坑经验未提取 |
| enrich 目标卡片未覆盖新主题 | 21 | VLM 标题中的新主题可能 enrich 时遗漏 |

**完整清单见**：`60_feedback/diagnosis/diag_20260625_wangyuyan_vlm-missed-knowledge.md`

老顽童生产成品卡前，必须先看这份清单；清单中列出的卡片，必须在原图/OCR 中补挖对应要素，不能仅依赖 VLM 描述。
