...
## 条件项处置（2026-08-09 王语嫣执行）

- ✅ **条件③（统计日期）**：domain-mapping.md 已补（2026-08-09 快照 + "—" 说明）
- ✅ **条件②（双轨设计意图）**：已裁定并写入 domain-mapping.md——MOC=知识导航（怎么查方法论）/ mapping=域清单（有哪些域），双轨不冲突；**教练 system-prompt 补 domain-mapping 引用**（转达教练，🟢 级）
- 🟡 **条件①（飞书端抽查指向）**：转达飞书端——任一 agent 问"KDO 有哪些域"→ 应命中 domain-mapping.md（唯一真相源）
