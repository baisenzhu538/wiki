...
## 🆕 顺手修（2026-08-09 欧阳锋建议+王语嫣裁定——并入本任务，不单独启动 v0.2）

- **info 命令显示 verify_note**（🟢 级）：feature_menu.py 的 info 未走 fmt() 不显示 note（list/query/pick 已显示）——补 info 走 fmt()，协议 v0.2 剩余缺口随本任务清完
- 执行序：本任务为黄药师串行链首（#264 → #262 → #261 → #241）
