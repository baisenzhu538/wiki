...
### 交叉印证补记（双实例第二判，2026-08-31）

> 本任务终审期间存在双欧阳锋实例并发（时钟 03:07 headless 拉起实例甲先行翻牌 b08b97682；老朱直令拉起实例乙即本节作者）。按 charter §2.6.2「双实例独立印证」，本节为独立复核——实例乙完成 8 组独立抽验后再读甲判，结论一致。

**独立抽验（8 组，Python 逐条重算，非转抄）**

1. A组 mnemo hit@1/hit@5/MRR/中位延迟 = 0.8 / 1.0 / 0.875 / 102.3ms——诊断 §1、执行报告、ab_results summary 三方逐项一致 ✅
2. A组 kdo = 0.7 / 0.8 / 0.733 / 1253.4ms ✅
3. B组 mnemo = 0.3 / 0.4 / 0.325 / 84.4ms——「B 组 hit@5 40%」关键声称成立 ✅
4. B组 kdo = 0.3 / 0.4 / 0.35 / 1208.8ms——双引擎 B 组同分，「场景③兜不住」依据成立 ✅
5. token 均值：A 18,406.1 / 17,409.9（-5.41%，诊断口径 -5.4% 为四舍五入）/ 1,055.2；B 16,288 / 15,423.9 / 634 ✅
6. F1/F2 案例引用：A4 mnemo rank=1 / kdo rank=0（top5 无目标卡）；A8 mnemo rank=4 / kdo 5 条全非目标 ✅
7. 175 倍自洽：25,637 / 146 = 175.6 ✅
8. 事故复跑口径：ingest_mnemosyne.py 现行版本 rmtree 真清库（mtime 02:52:24 早于 ab_results 生成 02:52:33）；memory.db 实查 50 条记录 / 49 id 全唯一 / 0 重复 / project 唯一 =kdo-dogfood-583——「干净库复跑」声明属实 ✅

**新发现（甲判未覆盖，不翻结论）**

- 🟡 O3 B9 量具公平性缺陷：目标卡 `case-wechat-article_4dd7be7cd82f7e80` 为全语料 50 卡中唯一无 `id:` frontmatter 字段者（逐卡实测），run_ab_test.py 按内容 frontmatter id 匹配 → 该查询对 mnemo 结构性不可命中。但 B9 top_ids 无空串条目 → 目标卡确实未进 top5，miss 真实，hit@5=0.4 不受影响。落点：场景①施工时注入器不得依赖内容 frontmatter id，须回退 ingest 已写入的 `meta.card_id`。
- 台账修正：本任务 REVIEW 段划销原写「已终审 PASS A-（2026-08-30 欧阳锋）」为日期倒挂（本任务 08-31 03:00 才提审），已随本补记修正为 08-31。

**终审结论（乙判）**：同意 PASS A-，与甲判独立一致。场景①施工（90_control/mnemo_cache/ + 标题路由注入）另行走立项流程授权，本单不含施工。另：03:07 conveyor_probe 登记的两行 gate-blocked（F-034 误报 + near-miss 三元组）属编排侧划销职责，移交王语嫣处置，不在本终审范围。
