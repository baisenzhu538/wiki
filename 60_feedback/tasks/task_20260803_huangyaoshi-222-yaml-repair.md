...
## ✅ 恢复编排审查（2026-08-03 06:00 欧阳锋）—— **编排合理，1 项防复发补强**
...
|:--|:--|
| ① 全库 YAML ≥98% | ✅ 99.4% |
| ② 串行执行（#222→#223）| ✅ 覆盖根因 2（并行写入冲突）|
| ③ 目录划分零重叠 | ✅ **O3 实测**：#222 8 高价值目录 ∩ #223 6 普通目录 = ∅，且全部目录存在 |
| ④ aliases 合并规则（git 恢复+去重）| ✅ 覆盖根因 1（追加块覆盖）|
| ⑤ dry-run + git diff | ✅ |

### ⚠️ 防复发补强（1 项，建议并入 #217 或另开）

**lint 增加 frontmatter 重复键检测**——本次事故的终极防线缺失：
- 现状：`kdo_lint.py`/`pre_submit.py` **无重复键检测**（`^aliases:` 出现 ≥2 次不被拦截）——如果 pre-submit 时 246 张双 aliases 被拦，事故在提交前就会暴露
- 建议：lint 检测 `^aliases:` / `^tags:` / `^diagnostic_signals:` / `^related:` 重复出现 → **ERROR** `"frontmatter 重复键 <key>，请合并"`（与 #217 F3 重复 ID 同模式，一行规则）
- 归属：黄药师（可并入 #217 结构门禁，或 #223 完成后）

### 结论

**编排方案合理、可执行**——5 条件覆盖事故双根因，目录零重叠实测通过。恢复可以开始（#222 先 → #223 后）。防复发 lint（重复键检测）建议尽快补——它会让 E010 这类事故在提交阶段被拦，而不是靠事后全库扫描。

---

## 🏁 最终验收（2026-08-03 05:30 欧阳锋）—— **verdict: PASS（结构修复完成）**

### O3 终验数据

| 指标 | 终态 |
|:--|:--|
| 全库 YAML 通过 | **2617/2634（99.35%）** |
| 剩余失败 | **17 张（0.65%）**——黄药师报 16，O3 扫 17（plan_20260531_data-curator 或统计差 1，不实质影响）|
| 双 aliases | 125（O3 口径）——YAML 可解析，结构冗余，#223 合并规则处理 |

### 17 张顽固卡处置裁决（同意黄药师方案，细化执行）

抽查确认多层腐败：**编码乱码（GBK 损坏）+ 多值融合（author/reviewed_by 融合）+ 双 aliases + 空 related + src_unknown 粘连**——机械修复无法恢复乱码原值。

| 项 | 处置 |
|:--|:--|
| 归属 | **纳入 #223 恢复范围**（黄药师建议采纳）|
| 乱码/值融合字段（author/reviewed_by/source_refs）| 从源素材（VLM/口述）或 `git show 16b64db39` 恢复原值 |
| 双 aliases / 空 related | 机械合并（可修部分）|
| 验收门槛 | 重采/修复后 `yaml.safe_load` 通过 + 内容不劣化 + 原搜索词在位 |
| 优先级 | **不阻塞 #222/#223 恢复**（0.65% 影响，作为 #223 收尾项）|

### 恢复放行

**✅ 结构修复彻底完成——同意放行 #222/#223 恢复**，条件（已在"恢复硬性条件"节）：
1. 串行执行（#222 → #223）
2. 目录划分（零重叠）
3. aliases 合并规则（git 恢复 + 去重）
4. dry-run + git diff
5. 17 张顽固卡纳入 #223 收尾（验收：yaml 通过 + 内容不劣化）
