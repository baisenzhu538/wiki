...
| # | 卡ID | 类型 | 说明 | 来源 |
|:--|:---|:---|:---|:---|
| 10 | `bridge-高阶建模↔Y模型解放思想` | bridge | 提炼建模=Y模型解放思想最后一叉，高阶建模域登顶前最后一座山；双向链接 `yt-decision-y-model` 和 `modeling-three-stages` | 口述L3582-L3618，黄药师定位 |
...
| # | 目标卡 | 补充内容 | 来源 |
|:--|:---|:---|:---|
| 11 | `framework-TCPR皇冠模型` | 新增§建模过程：P/C/T 1.0→发现缺R→4.0四角色映射；related新增 `case-TCPR四角色建模演进` | 口述§3.3 |
| 12 | 双三角系列核心卡 | 新增§建模迭代史：v1→v6六版迭代关键决策+每版为什么改 | L1796-L1868 |
| 13 | 终局光谱图核心卡 | 新增§建模抉择：金字塔→阶梯→光谱三次推翻过程 | L2102-L2188 |
| 14 | 共识冰山核心卡 | 新增§建模过程：共识四层→爬山+冰山双意象抉择 | L2040-L2096 |
| 15 | 动力阻力三曲线核心卡 | 新增§建模演进：PNL缩写→轨道模型→三曲线 | L1972-L2036 |
| 16 | 五步法核心卡 | 新增§组件拆解：五步法=4个组件的拼接（标注具体哪4张牌） | L2756-L2780 |
| 17 | `modeling-three-stages` | related新增 `framework-建模四步法`、`tool-18组件卡牌库`、`concept-四层觉察` | 本课 |
| 18 | `modeling-level-map` | related新增 `concept-四层觉察`（段位图=能力评级，四层觉察=认知进化路径） | 口述L2560-L2606 |
