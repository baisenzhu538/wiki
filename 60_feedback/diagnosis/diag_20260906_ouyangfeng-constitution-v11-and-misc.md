...
## 四、顺带：宪法第二条执行口径不对称（🟠Medium）

**现象**：#654 执行报告无 `**存在性核查**` 节（机器预审 🔴 已提示），complete 提审照常放行；而审查侧意见书同情况被 F-035 硬拦（欧阳锋 09-06 当天被拦一次实证）。宪法第二条对生产侧=机器预审软提示，对审查侧=硬门禁，同条宪法两种执行力度。

**建议方向**：把「执行报告含负向判词必须附存在性核查节」升级为 complete 门禁的硬校验（或至少 WARNING 计数进 F-034 五字段检查面）；生产侧 09-06 起 #654 已属过渡期个案，不追溯。

---
*三行口径（#460）：现象/在哪发现/建议方向均已含。本建议书与 #652 复审终审记录、#654 终审记录一一对应（出口自检钩子）。*
