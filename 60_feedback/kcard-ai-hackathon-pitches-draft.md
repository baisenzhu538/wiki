---

id: "kcard-ai-hackathon-pitches-draft"
type: "concept"
status: "draft"
source_refs:
  - "theme-ai-hackathon-pitches-summary"
  - "rec-4046714"
  - "rec-4044940"
  - "rec-4044361"
  - "rec-4042877"
  - "rec-4033918"
  - "rec-4033871"
  - "rec-4033525"
  - "rec-4033304"
  - "rec-4033147"
  - "rec-4025217"
  - "rec-4023226"
component_of: []
related:
  - "yt-five-step-method"
  - "yt-entrepreneur-key-hypotheses"
  - "yt-tool-product-core-canvas"
  - "yt-pitch-colloquialization"
  - "concept-ai-native-organization-five-steps"
  - "ai-collaboration-mindset-shift"
  - "ai-landing-scene-selection"
  - "find-old-do-small"
query_triggers:
  - "AI大航海路演"
  - "hackathon pitch"
  - "项目汇报"
  - "MVP验证"
  - "AI项目路演"
  - "一堂大航海"
reviewed_by: ""
tags:
  - #scene/ai-landing
  - #scene/entrepreneurship
  - #scene/hackathon
  - #scene/yitang
  - #source/itingnao-kit
  - #status/draft
pipeline:
  - confidence-draft
  - confidence-source-cited

domain:
  - "ai-landing"
  - "entrepreneurship"
  - "yitang"

---

# AI 大航海项目路演：主题综合知识卡（草稿）

> 本卡基于 `90_control/itingnao-kit/work/theme-ai-hackathon-pitches-summary.md` 中的 11 条录音 meetingSummary 摘要提炼而成。**未逐段核对原文**，关键结论已按置信度分层，不确定处标注「待原文复核」。

## 主题定义

**AI 大航海项目路演**（ai-hackathon-pitches）是一组围绕「一堂 AI 大航海」结业/阶段路演录制的主题集合。它记录个人或团队如何在 AI 时代，以一堂方法论（五步法、MVP、关键假设、产品内核等）为底层框架，把生成式 AI 转化为可演示、可验证、可商业化的项目原型或服务。

该主题不是单一概念，而是**跨域项目样本群**：覆盖知识库构建、组织 AI 落地、工具平台、知识产权服务、金融分析、内容营销、AI 剧本杀、非遗珠宝、AI 硬件入口等场景，可用于观察 AI 落地的共性路径与差异边界。

## 核心洞察

1. **方法论是项目底层的“默认操作系统”**
   - 几乎所有项目都显式引用一堂方法论：五步法、MVP、关键假设、产品内核、双三角模型、外模型、找老的干小的等。
   - 证据：科创号 1301 用「一堂创业地图/管理地图/案例库」和「双三角模型」构建专利智能体；无限进步战队用「外模型（steps 多维五步）」打磨 AI 内容营销工具；房浩洋剧本杀项目用 MVP 测试 + 11 版迭代验证可行性。
   - 边界：录音中自述的方法论应用程度需原文复核，不能排除“用后合理化”倾向。

2. **AI 落地呈现“垂直场景 + 可调用能力”两层结构**
   - 垂直场景层：商业假设验证、组织 SOP 重构、知识产权服务、A 股价值投资、内容获客、非遗珠宝销售、剧本杀体验等。
   - 可调用能力层：知识库、Agent、MCP、提示词工程、多模态解析、语音/视觉硬件入口、AI 辅助设计等。
   - 证据：奶糖平台定位“AI 时代个性化工具库”，基于 MCP 协议跨客户端调用；AI 旋风组用扣子/阿里云千问把一堂知识库接入语音硬件；瑶光阁用 AI 辅助大漆珠宝设计、视频制作与小红书数据分析。

3. **验证方式从“讲概念”转向“亮数据 + 亮 demo”**
   - 各项目普遍给出可量化早期信号：剧本杀 73% 满意度、40% 付费意愿、单局 API 成本 0.3 元；科创号 1301 12 万高校订单、800 万中小企业客户渠道；非遗珠宝完成 3 单成交；2298 战队 5 个项目完成率 90%、专利撰写周期从 24h 缩至 8h。
   - 边界：以上数字均来自口述摘要，样本量、统计口径、是否可复现**待原文复核**。

4. **组织落地与个人成长是同一枚硬币的两面**
   - 组织侧：2298 战队提出“AI 落地是一把手工程”“业务驱动、SOP 先行、流体组织”；AI 旋风组用树莓派 + 扣子搭建企业级知识库 Agent。
   - 个人侧：个人汇报者从 AI 门外汉到输出 14 份业绩文档、效率提升 32%；AI 博客项目发言人从“AI 旁观者”转变为可私有化部署大模型的实践者。
   - 启示：AI 项目路演的价值不仅在于产品，更在于展示“人 + 方法 + 工具”的协同进化。

5. **商业化路径初显，但规模化假设普遍未经验证**
   - 奶糖平台规划广告、权重调用、开发者返佣；科创号 1301 通过渠道合作触达 800 万中小企业；瑶光阁已建立小红书账号矩阵与转化率看板 demo。
   - 风险：多数项目处于 MVP 或内测阶段，收入模型、获客成本、合规边界、竞争壁垒仍需独立验证。

## 代表案例 / 录音清单

| 录音 ID | 主题 | 在本主题中的角色 | 关键产出/概念 |
|--------|------|----------------|--------------|
| 4046714 | 一堂武智院-项目介绍 | 知识库/教育产品化 | ECD 知识库、提示词智能体系、小镇实验、贡献教程 |
| 4044940 | 2298战队-AI组织落地探索 | 组织变革案例 | 四阶段落地、SOP 先行、一把手工程、流体组织 |
| 4044361 | 唐三超-奶糖项目汇报 | AI 工具平台 | MCP 协议、个性化工具库、零代码工作流、返佣机制 |
| 4042877 | 科创号1301-AI应用落地汇报 | ToB 服务落地 | 专利智能体、12 万订单、800 万渠道客户、双三角模型 |
| 4033918 | 稳住战队-价值投资大师项目 | 金融分析工具 | A 股价值投资报告、百度秒搭、投资大师知识库 |
| 4033871 | 无限进步战队-AI内容营销路演 | 内容营销工具 | 人工审美补 AI 减法、3000 篇笔记抓取、IPO 流程 |
| 4033525 | 个人-AI航海成果汇报 | 个人成长路径 | 14 份业绩文档、效率提升 32%、上下文工程 |
| 4033304 | AI博客项目-经验分享 | 多模态内容生成 | 音视频转博客、外模型、私有化部署 |
| 4033147 | 房浩洋-AI剧本杀项目路演 | C 端娱乐产品 | AI 单人剧本杀、73% 满意度、40% 付费意愿 |
| 4025217 | 瑶光阁-非遗珠宝项目汇报 | 文创/电商产品 | 非遗大漆 + NFC、AI 设计、小红书矩阵、3 单成交 |
| 4023226 | AI旋风组-项目介绍 | AI 硬件/企业 Agent | 扣子 + 阿里云千问、语音克隆、树莓派 5、一堂知识库 Agent |

## 六层交叉验证

> 基于录音摘要与外部公开信息（一堂方法论公开资料、AI 创业项目公开报道、MVP/关键假设验证理论）交叉核对。**本主题未生成独立 six-layer-validation 报告。**

### L1 可证伪性：中等
- 项目存在、使用平台、方法论引用等可证伪性较强。
- 具体数据（12 万订单、800 万渠道客户、73% 满意度、40% 付费意愿、效率提升 32%）可证伪但缺少独立证据。
- 商业化路径、规模化假设可证伪性较弱。

### L2 行为一致性：中等
- 路演语境下，项目方自然倾向于展示积极数据和进展。
- 未记录失败案例、负面反馈、项目停滞等反向信息。

### L3 多源验证：中低
- 一堂方法论（五步法、MVP、关键假设）有外部理论支持。
- 各项目数据均为自述，样本量、统计口径、是否 audited 待复核。
- 平台和技术栈（扣子、阿里云千问、百度秒搭、MCP）可公开验证，但项目效果不可。

### L4 情绪标记：中等
- 项目展示带有明显的积极、兴奋、希望获得认可的情绪。
- 对风险和合规边界的讨论较少。

### L5 时间稳定性：中低
- 早期项目、MVP、平台生态变化快，当前结论可能很快失效。
- 假设验证、迭代打磨、路演表达等方法论相对稳定。

### L6 利益相关度：高
- 路演项目通常追求融资、客户、课程成绩、团队认可。
- 存在强烈的选择性呈现和夸大动机。

### 综合可信度评级

| 层级 | 评分 | 权重 | 加权得分 |
|------|------|------|----------|
| L1 | 0.60 | 0.18 | 0.108 |
| L2 | 0.52 | 0.18 | 0.094 |
| L3 | 0.50 | 0.20 | 0.100 |
| L4 | 0.55 | 0.15 | 0.083 |
| L5 | 0.48 | 0.15 | 0.072 |
| L6 | 0.75 | 0.14 | 0.105 |
| **综合** | — | — | **0.54** |

**评级**: 🟡 中可信度（0.50-0.75），偏向中低。

**可纳入知识卡的内容**：方法论是项目底层操作系统、AI 落地“垂直场景 + 可调用能力”两层结构、验证方式从“讲概念”转向“亮数据 + 亮 demo”、组织落地与个人成长相互促进。
**不建议直接纳入的内容**：各项目自述的订单金额、客户数、满意度、效率提升、付费意愿等数字。

---

## 与现有 30_wiki 的差异点

- **现有卡片多为单点方法论或工具**：如 [[yt-five-step-method]]、[[yt-entrepreneur-key-hypotheses]]、[[yt-tool-product-core-canvas]]、[[yt-pitch-colloquialization]]、[[concept-ai-native-organization-five-steps]] 等，分别覆盖方法、假设、产品、表达、组织，但缺少把“方法 → 项目 → 验证数据 → 商业化”串起来的**主题视图**。
- **本主题提供跨域项目样本群**：11 条录音覆盖知识库、组织、工具平台、专业服务、金融、营销、娱乐、文创、硬件入口，可作为 AI 落地成熟度的横向参照。
- **包含可量化的早期验证数据**：如订单金额、满意度、付费意愿、成本、完成率等，现有概念卡较少直接引用此类一手口述数据。
- **强调“讲香/路演”作为交付形态**：不仅是产品，还包括如何向听众展示假设、数据与迭代过程，这与 [[yt-pitch-colloquialization]] 形成应用场景互补。

## 置信度分层

### 事实层
- 主题下共有 11 条录音，均为一堂 AI 大航海相关项目路演/汇报。
- 各项目主体、产品名称、使用平台（百度秒搭、扣子、阿里云千问、MCP 等）在摘要中明确出现。
- 关键数字在摘要中明确记录：12 万订单、800 万客户渠道、73% 满意度、40% 付费意愿、0.3 元单局成本、3 单成交、5 个项目 90% 完成率、14 份业绩文档、效率提升 32% 等。
- 主讲人/发言人身份多数标注为“发言人 1”“金科”“王飞”“房浩洋”“唐三超”等，部分人名/角色待原文复核。

### 条件层
- 所有量化数据均基于 meetingSummary 摘要，统计口径、样本量、测量时间、是否 audited **待原文复核**。
- 项目成功描述带有路演展示语境，可能存在选择性呈现，不代表同等条件下可复现。
- 方法论的适用性取决于团队能力、行业监管、数据可获取性等外部条件。

### 观察层
- 多数项目呈现“小切口 MVP → 数据验证 → 商业化路径”三段式结构。
- 知识库/Agent/Skill 成为跨项目复用的通用能力层。
- 组织类项目更强调 SOP、一把手工程、业务驱动；个人/小团队项目更强调快速学习、工具组合、审美/判断补位。

### 风险层
- **路演幸存者偏差**：能进入路演录音的项目本身经过筛选，失败或停滞项目未被记录。
- **数据未经独立验证**：满意度、付费意愿、订单金额、效率提升等数字均为自述，需原始材料佐证。
- **工具与平台依赖**：百度秒搭、扣子、MCP、阿里云千问等生态快速变化，项目可复制性受限。
- **合规与知识产权风险**：专利智能体、AI 生成内容、投资分析报告、剧本杀内容等涉及法律与合规边界，需专业复核。

## 建议后续行动

1. **原文复核**：对 4033147（剧本杀）、4042877（科创号 1301）、4025217（瑶光阁）、4044940（2298 战队）中的关键数据拉取原始录音/转写文本确认。
2. **案例拆分**：将 2298 战队组织落地、奶糖 MCP 平台、科创号专利智能体、瑶光阁非遗珠宝、房浩洋剧本杀等作为独立案例卡或概念卡候选，再与本主题卡建立双向链接。
3. **冲突检查**：核对 30_wiki 是否已有 MCP、百度秒搭、扣子、一堂知识库 Agent 等概念卡，避免重复建卡。
4. **证据补充**：对“73% 满意度”“40% 付费意愿”“12 万订单”“800 万客户”等数据补充原始出处或标注为“未验证口述数据”。
5. **管线注册**：如本卡后续升级进入 30_wiki，需按 KDO 三步编译法补充 Critique 与 Synthesis，并注册 source_refs 到 `10_raw/sources/`。

## 药柜 / 医疗内容标注

**本主题未发现明显属于药柜/医疗的内容。**

- 唯一涉及“医疗”字样的地方是 4033918（稳住战队）中提到团队成员 A “2026 年收益 45%（医疗领域）”，这仅指投资标的行业分类，不构成医疗/药柜服务或产品内容。
- 其余录音围绕知识库、组织、工具平台、知识产权、金融分析、内容营销、剧本杀、非遗珠宝、硬件 Agent 等主题，无需移入药柜处理队列。

**建议**：若后续原文复核发现某条录音实际涉及医疗服务、药品、医疗器械或健康咨询，再单独移入 `90_control/itingnao-kit/medical-queue/` 处理。

---

*生成时间：2026-06-14 | 生成来源：theme-ai-hackathon-pitches-summary.md | 状态：草稿，待复核*
