...
# -*- coding: utf-8 -*-
"""
用 MiniMax-M3（通过 Anthropic SDK）对销售专题图片做 OCR + 结构化分析。
"""
import os
import json
import base64
...
INPUT_DIR = Path(r"C:\Users\Administrator\Desktop\wiki\00_inbox\销售专题")
...
def analyze_image(image_path: Path) -> dict:
    ...
    system_prompt = """你是一位资深的销售管理知识萃取专家，擅长把课程图解、清单、流程图、雷达图等转化为结构化、可落地的销售管理知识。
请严格按以下格式输出：
...
【图片类型】
判断属于：框架图/流程图/雷达图/矩阵图/清单表/案例图/对比图/其他

【核心内容】
用3-5条 bullet points 总结图片核心信息。

【结构化解析】
如果图中有模型/矩阵/阶段/清单，请用表格或分级列表呈现。

【与科学销售五步法的关联】
指出这张图属于五步法中的哪一步或哪几步，以及它的管理价值。
        ...
        messages=[
            {
                ...
                "content": [
                    ...
                    {"type": "text", "text": "请对这张销售管理课程图片进行 OCR + 结构化深度解析。"},
                ],
            }
        ],
    )
