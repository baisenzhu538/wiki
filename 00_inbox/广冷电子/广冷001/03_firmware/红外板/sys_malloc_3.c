...
void sys_mem_Clear(void *ptr)
{
  ...
  if(offset<sysmemsize)//偏移在内存池内. 
	{  
			...
			int nmemb=sys_mallco_dev.memmap[index];	//内存块数量
		  unsigned char *p=ptr;
		  for(i=0;i<nmemb*sysmemblksize;i++)//清除数据
		  {
				p[i]=0x00;
			}
...
{  
  ...
  INTX_ENABLE();	
    if(offset==0XFFFFFFFF)
		...
		{
			return (void*)((unsigned int)sys_mallco_dev.membase+offset);
		}			
}

void *SysMem_realloc(void *ptr,unsigned int size)  
{  
    unsigned int offset;  
    offset=sys_mem_malloc(size);  
