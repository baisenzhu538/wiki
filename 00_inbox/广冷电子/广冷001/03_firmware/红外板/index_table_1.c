...
*		版本号  日期        作者     说明
*		V1.0    2017-06-24  欧阳     
*   V1.0.1  2018-11-21  欧阳     修改sIndexTable_WriteData()和mIndexTable_WriteData()
    V1.0.2  2019-01-16  欧阳     修改定时触发timeover标志位的初始化，解决复位随机值造成无法正常触发
*********************************************************************************************************
*/	
...
修改记录 :
		...
		V1.0    2017-06-24  欧阳     新增
******************************************/
IndexTableUintTypeDef *IndexTable_TraverseUint2(IndexTableTypeDef *pTable,uint16_t devicetype,uint8_t devicenum,uint8_t sourceid) //获取对应任务块的地址
{
	IndexTableUintTypeDef *pTableUint;
	if(pTable->head==NULL)             //链表中无数据
		return NULL;
	...
	{
		if(
			 pTableUint->Source.SourceData.devicetype==devicetype
		   &&pTableUint->Source.SourceData.devicenum==devicenum
			 &&pTableUint->Source.SourceData.sourceid==sourceid
		  )
			return pTableUint;
		pTableUint=pTableUint->next;
	}
...
uint8_t IndexTable_AddSource2(IndexTableTypeDef *pTable,SourceInfoTypeDef *pSource)
{
	IndexTableUintTypeDef *pTableUint;
	
	if(pTable->tablesize==pTable->maxsize)
		return 0x00;                                                        //空间满
	if(IndexTable_TraverseUint2(pTable,pSource->SourceData.devicetype,pSource->SourceData.devicenum,pSource->SourceData.sourceid)!=NULL)
...
static uint8_t id_tb_writedata(IndexTableTypeDef *pTable,uint8_t node_id,uint8_t index_num,void *pdata,uint16_t datalen)
{
	...
	{
		if(index_num<INDEXTABLE_SERIALPORTADD)
		{
		...
		{
			uint8_t (*pFun)(uint8_t*,uint16_t);
		  pFun=pTableUint->Source.SourceData.data;
		  (*pFun)(pdata,datalen);//调用用户自定义数据处理函数
		}
	}
	return 0xFF;//写入成功
}
...
修改记录 :
		版本号  日期        作者     说明
		V1.0    2017-06-24  欧阳     新增
    V1.01   2018-11-21  欧阳     修改if(index_num<INDEXTABLE_SERIALPORTADD)为if(index_num>(INDEXTABLE_SERIALPORTADD-1))
*************************************************************/
uint8_t mIndexTable_WriteData(uint8_t node_id,uint8_t index_num,void *srdata,uint16_t datalen)
{
	...
	if(index_num>(INDEXTABLE_SERIALPORTADD-1))//资源节点地址范围属于通信接口
	 cb_state|=id_tb_writedata(&mIndexTable,0xFF,index_num,srdata,datalen);
	return cb_state;
}
...
修改记录 :
		...
		V1.0    2017-06-24  欧阳     新增
*************************************************************/
uint8_t mIndexTable_AddSource(uint16_t devicetype,uint8_t devicenum,uint8_t sourceid,uint8_t type,void *srcdr,uint16_t datalen)
{
	SourceInfoTypeDef src;
	uint8_t cb_state;
	src.SourceManage.type=type;
	src.SourceData.sourceid=sourceid;
	src.SourceData.devicetype=devicetype;
	src.SourceData.devicenum =devicenum;
	src.SourceData.nodeid=0x00;
	...
	cb_state=IndexTable_AddSource2(&mIndexTable,&src);
	return cb_state;
}
/*************************************************************
...
修改记录 :
		...
		V1.0    2017-06-24  欧阳     新增
*************************************************************/
void mIndexTable_SetConnectFlag(uint16_t devicetype,uint8_t  devicenum,uint8_t nodeid,uint8_t connectflag)
{
	IndexTableUintTypeDef *pTableUint;
	...
	while(pTableUint)
	{
		if(pTableUint->Source.SourceData.devicetype==devicetype&&pTableUint->Source.SourceData.devicenum==devicenum)
		{
			pTableUint->Source.SourceData.nodeid=nodeid;
			...
			if(pTableUint->Source.SourceManage.connectflag==1)
			{
				...
				else if(pTableUint->Source.SourceManage.type==RO)
				{
...
修改记录 :
		版本号  日期        作者     说明
		V1.0    2017-06-24  欧阳     新增
    V1.1    2019-01-16  欧阳     增加timeoverflag的初始化
*************************************************************/
uint8_t mIndexTable_SetCycSync(uint16_t devicetype,uint8_t devicenum,uint8_t sourceid,uint16_t time)
{
	IndexTableUintTypeDef *pTableUint;
	if(sourceid>(INDEXTABLE_SERIALPORTADD-1))
		return 0x00;
  pTableUint=IndexTable_TraverseUint2(&mIndexTable,devicetype,devicenum,sourceid);
	if(pTableUint==NULL)
		return 0x00;
	pTableUint->Source.SourceManage.cycle_enable=1;
	pTableUint->Source.SourceManage.time        =time;
	pTableUint->Source.SourceManage.timeoverflag=0x00;
  return 0xFF;
}
...
修改记录 :
		...
		V1.0    2017-06-24  欧阳     新增
*************************************************************/
uint8_t mIndexTable_SetStateSync(uint16_t devicetype,uint8_t devicenum,uint8_t sourceid)
{
	IndexTableUintTypeDef *pTableUint;
	if(sourceid>(INDEXTABLE_SERIALPORTADD-1))
		return 0x00;
...
返回：
*************************************************************/
uint8_t mIndexTable_UpDataLate(uint8_t nodeid,uint8_t sourceid)
{
	IndexTableUintTypeDef *pTableUint;
	uint32_t DataBuf;
	uint32_t State;
...
void mIndexTable_ReceiveTask(TransportCanMsgTypeDef *pRxMsg)
{
	...
	if(pTableUint==NULL)
		return;
	if(pRxMsg->TransportMsgFilter.SourceID<INDEXTABLE_SERIALPORTADD)//判断操作的资源地址是否为用户自定义地址
	{
...
/*************************************************************
函数：mIndexTable_SetReceiveCallBack
功能：设置从节点数据接收回调函数
参数：pLinkManage 节点连接状态
返回：
*************************************************************/
void mIndexTable_SetReceiveCallBack(void (*fReceive_CallBack)(SourceDataTypeDef*))
{
	mIndexTableTask.Receive_CallBack=fReceive_CallBack;
}
...
void mIndexTable_SyncTaskRun(void)
{
	...
	{	
		if(mIndexTableTask.now->Source.SourceData.sourceid<INDEXTABLE_SERIALPORTADD)
	  {
			if(mIndexTableTask.now->Source.SourceManage.type!=RO)
			{
				if(mIndexTableTask.now->Source.SourceManage.star_tx)//用于支持主动启动发送
				{
					if(IndexTable_SendMsg(&mIndexTableTask.now->Source,FUNCID_CYCLETRIGGER))
					{
						mIndexTableTask.now->Source.SourceManage.star_tx=0;
		...
		if(pTableUint->Source.SourceManage.connectflag==1)
		{
			if(pTableUint->Source.SourceManage.timeoverflag==0x00&&pTableUint->Source.SourceManage.cycle_enable==1)
			{
				...
				else
				{
					...
					pTableUint->Source.SourceManage.timeoverflag=0x01;
				}
			}
		}
		pTableUint=pTableUint->next;
...
{
	uint8_t cb_state=0x00;
	cb_state=id_tb_writedata(&sIndexTable,node_id,index_num,srdata,datalen);
	if(index_num>(INDEXTABLE_SERIALPORTADD-1))//资源节点地址范围属于通信接口
		cb_state|=id_tb_writedata(&sIndexTable,0xFF,index_num,srdata,datalen);
	return cb_state;
}
/*************************************************************
...
修改记录 :
		版本号  日期        作者     说明
		V1.0    2017-06-24  欧阳     新增
    V1.0    2019-01-16  欧阳     新增timeoverflag初始化
*************************************************************/
...
void sIndexTable_ReceiveTask(TransportCanMsgTypeDef *pRxMsg)
{
	...
	if(pTableUint==NULL)
		return;
	if(pRxMsg->TransportMsgFilter.SourceID<INDEXTABLE_SERIALPORTADD)
	{
...
/*************************************************************
函数：sIndexTable_SetReceiveCallBack
功能：设置从节点数据接收回调函数
参数：pLinkManage 节点连接状态
返回：
*************************************************************/
void sIndexTable_SetReceiveCallBack(void (*fReceive_CallBack)(SourceDataTypeDef*))
{
	sIndexTableTask.Receive_CallBack=fReceive_CallBack;
}
/*************************************************************
函数：indextable_synctask
功能：资源表同步任务，内部调用
...
void sIndexTable_SyncTaskRun(void)
{
	...
	{	
		if(sIndexTableTask.now->Source.SourceData.sourceid<INDEXTABLE_SERIALPORTADD)
		{
