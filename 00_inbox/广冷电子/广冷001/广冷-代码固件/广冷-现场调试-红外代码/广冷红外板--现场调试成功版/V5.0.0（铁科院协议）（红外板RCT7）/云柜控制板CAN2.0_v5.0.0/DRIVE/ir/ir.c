...
///**
// * @brief 2个SN74LV595级联，控制16个发射灯珠的开启/关闭
// * @param no 发射灯珠编号（0-15，对应16个灯珠：0-7→第一个595，8-15→第二个595）
// * @param enable 使能标志（1：开启指定灯珠；0：关闭所有灯珠）
// * @note 级联逻辑：U1.QH' → U2.SER，SRCLK/RCLK同步，SER串行输入16位数据
// */
//void Ir_Select_Send(u8 no, u8 enable)
//{
//    u8 i;
//    u16 mask = 0; // 16位掩码（对应2个595的16个并行输出）

//    // 生成16位掩码：enable=1时仅指定位为1，否则全0
//    if (enable)
//    {
//        if (no < 16) // 防止编号越界
//            mask = 1 << no; 
...
//    // 1. 锁存时钟拉低（准备接收移位数据，避免移位过程中输出变化）
//    IR_595_RCLK = 0;
//    Ir_Delay(15); // 确保电平稳定（根据实际主频调整，最低可到1us）

//    // 2. 串行移位：16位数据（从高位bit15到低位bit0）
//    for (i = 0; i < 16; i++)
//    {
//        IR_595_SRCLK = 0; // 移位时钟拉低（准备输入1位数据）
//        Ir_Delay(15);

//        // 控制SER引脚电平（仅1个引脚！根据掩码的当前位决定）
//        // 掩码高位（bit15）对应第二个595的Q7，低位（bit0）对应第一个595的Q0
//        if (mask & ((u16)0x8000 >> i)) 
//        {
//            IR_595_QD1 = 1; // 输入高电平（对应灯珠开启）-- SER
//        }
//        else
//        {
//            IR_595_QD1 = 0; // 输入低电平（对应灯珠关闭）
//        }
//        Ir_Delay(15); // 确保数据稳定

//        IR_595_SRCLK = 1; // 移位时钟上升沿→将SER的1位数据移入移位寄存器
//        Ir_Delay(15); // 确保移位完成
//    }

//    // 3. 锁存时钟拉高→将16位移位数据输出到并行端口（Q0~Q7）
//    IR_595_RCLK = 1;
//    Ir_Delay(15);
//}
...
u8 Ir_Read_Recive(u8 no)
{
	...
	//if((IR_4051_IN1&&IR_4051_IN2)||(IR_4051_IN3&&IR_4051_IN4)) // 判断 红外接收管 是否 接收到红外光
	if(IR_4051_IN1&&IR_4051_IN2&&IR_4051_IN3&&IR_4051_IN4) // 判断 红外接收管 是否 接收到红外光
		return 1;// 灯珠 没有物体遮挡
	else
		return 0;// 灯珠 有物体遮挡
...
{
	if(IrTask.sta==0xFFFF) // 若所有灯珠都检测到信号（sta全1,每个扩展芯片扩展8个灯珠）-- 全部红外接收管 都接收到红外
	{
		// 高电平 -- 关闭 LED指示灯
		IR_LEVEL_OUT=1;
		IR_LED_OUT=1;  
