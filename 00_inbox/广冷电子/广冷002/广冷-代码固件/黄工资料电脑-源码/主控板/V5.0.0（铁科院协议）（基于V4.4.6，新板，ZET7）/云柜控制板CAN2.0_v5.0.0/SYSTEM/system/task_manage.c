...
*	文件名称 : task_manage.c
*	版    本 : V1.0
*	说    明 : 实现任务的单次执行，定时执行，任务添加，任务删除等功能
*	修改记录 :
*		版本号  日期       作者    说明
*		V1.0    2019-02-16 OUSI   
*********************************************************************************************************
*/
...
void TaskManage_DeleteTasks2(void (*pFuntion)(void))
{
	uint8_t i_index;
	for(i_index=0;i_index<SYSTEM_MAX_TASKS;i_index++)
	{
		if(SystemTask[i_index].pTask==pFuntion)
		{
     SystemTask[i_index].pTask=NULL;
	   SystemTask[i_index].enable=0;
	   SystemTask[i_index].delay=0;
	   SystemTask[i_index].period=0;
	   SystemTask[i_index].runmod=0;
	   SystemTask[i_index].runme=0;
		}
	}
}
