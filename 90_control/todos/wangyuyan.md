...
[2026-09-01 14:41] 📐 新错误模式候选（自记 near-miss）：「open('wb') 内联写共享文件」——求值顺序陷阱（打开截断在前、内容参数求值在后），参数异常=静默留空文件；同族 E046（追加意图做替换）但机制不同层。纪律候选：写共享文件一律先算 content 变量再 open().write(content)，或 temp+os.replace 原子替换；复盘时入错误模式库