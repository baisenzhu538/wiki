...
当素材包含知识地图（视觉框架）+ 口述稿（深度讲解）的混合包时，它们不是独立概念——是上下位关系的知识集群。逐张处理为独立卡片会导致碎片化、关联断裂、质量低下（单张 OCR 文字不足 300 字，无法满足 L2 要求）。

**原则：聚类复合编译优于逐张独立处理。** 口述稿提供方法论主干，知识地图提供框架可视化和案例佐证，二者合并编译为一张复合概念卡。

详细策略见：[[high-density-composite-compilation-strategy]]，执行规范见本手册 §十二。
...
当 ingest 产出大量（>20 张）来源相同主题的知识地图骨架时，enrich 阶段切换为复合编译模式：

| 步骤 | 动作 | 退出标准 |
|------|------|---------|
| **1. 素材分类** | 按主题聚类口述稿 + 知识地图 | 聚类方案已写入决策文件 |
| **2. 源文件归档** | 图片从 `00_inbox/` → `10_raw/sources/`，口述稿指向 source 文件 | 无 `00_inbox/` 引用残留 |
| **3. 读取建理解** | 先读口述稿全文，再逐张打开原图分析视觉结构 | 每张原图均已多模态分析 |
| **4. 合并编译** | 在口述稿 wiki 骨架上扩展，知识地图作为 Framework Gallery 附件融入 | 三轮编译完成（Condense ≥5 / Critique / Synthesis ≥2） |
| **5. Visual Analysis** | 每张引用的知识地图原图，分析其视觉结构 | 每张图均有层级/分组/路径/强调/留白分析 |
| **6. 验证** | `kdo lint --wiki-path` | 0 warning |

> 详细模板和聚类方案见 [[high-density-composite-compilation-strategy]]。
...
| 规则 | 严重度 | 阈值 |
|------|:------:|------|
| Condense 有实质性内容（非 "TBD" 或空段落） | 🟠 P1 | ≥3 条核心结论（普通卡）/ ≥5 条（复合卡） |
| Critique ≥ 1 条，且至少一条**指名具体假设或边界** | 🟠 P1 | 禁止"需要更多验证"式万能废话 |
| Synthesis 有 ≥ 2 个 wikilinks（允许含 index 页） | 🟡 P2 | — |
| 全文 > 500 字 | 🟡 P2 | — |
| **Visual Analysis**（仅复合卡）：每张引用原图均有分析 | 🟠 P1 | 层级/分组/路径/强调/留白 五个维度至少覆盖三个 |
...
| 编号 | 铁律 | 来源 | 严重度 |
|:----:|------|------|:------:|
| **KF-019** | **知识地图必须看原图**：提炼知识地图内容时，必须逐张打开原图分析视觉结构。不得仅依赖 OCR 文字提炼知识——OCR 丢失了空间层级、分组逻辑、阅读路径、视觉强调和留白含义 | 2026-05-10 复合编译策略 | 🔴 |
...
| **KF-021** | **高密度素材禁逐张独立处理**：知识地图+口述稿的混合素材包不得按"一文件一卡片"逐张处理。必须按主题聚类后合并编译为复合概念卡。知识地图作为 Framework Gallery 附件融入口述稿卡片 | 2026-05-10 复合编译策略 | 🔴 |
| **KF-022** | **单次会话复合卡上限**：Builder 单次会话不得处理超过 3 个复合卡片。每个复合卡需要读取大量上下文（口述稿 + 多张原图），超过 3 张会导致 context overload 死锁 | F-KDO-012/F-KDO-015 | 🟠 |
...
**Visual Analysis 遗漏防呆**：复合卡审查时，检查卡片引用的每张知识地图原图是否在 `## Visual Analysis` 节中有对应分析。遗漏任何一张 → 退回。
