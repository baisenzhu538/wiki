...
> **编制**：欧阳锋（Architect）| 初版 2026-05-09 | v2.0 2026-08-22（#417 整改：版本归一 + 入宪六方角色对齐）
> **版本规则**：改一次升一次，frontmatter/正文同号，后续由脚本校验（溯源 #416 附则/B4-1）
> **范围**：KDO 工作空间全部知识生产活动
...
KDO 对应原则：**审查者（欧阳锋）审查产出 → 指出问题 → 生产者（老顽童）修正内容卡 / 建设者（黄药师）修正基建 → 欧阳锋复审**。欧阳锋不直接编辑生产者/建设者的产出文件——审查和执行的分离防止"自己审自己"的盲区。

唯一的例外：欧阳锋可以起草 `90_control/` 下的控制文件变更（本手册、AGENTS.md、failure-modes.md 等），因为这些是审查者的职责域——但此类变更仍受 KF-001 双重签发约束（提案 + 用户批准）。
...
| type | 用途 | Claims 范围 | 行数上限 | tokens 目标 | tokens 强制拆分 |
|------|------|:--:|:--:|:--:|:--:|
| `composite-concept` | 多源聚合 | 15-30 | ≤500 | ≤3500 | >5000 |
| `framework` | 知识地图结构化描述 | 5-15 | ≤300 | ≤2500 | >3500 |
| `case` | 案例卡 | 5-10 | ≤200 | ≤2000 | >3000 |
| `tool` | 检查清单/画布/模板 | 3-10 | ≤150 | ≤2000 | >3000 |

**拆分规则**：超过 tokens 强制拆分列 → 必须拆分。超过 tokens 目标 → 不强制，但需在 Constraints 中说明"为什么不能拆"。
...
对标 EC 的四角色循环（设计者/协调者/开发者/测试者），KDO 现行采用与 #416 基本法（[[kdo-charter-v0.1-draft]] §2.1）逐字一致的**入宪六方**：
...
**权责分界**（charter §2.2）：王语嫣裁"值不值得做"，欧阳锋判"做得好不好"。建议只是建议——全员有权向王语嫣写建议书，但王语嫣必须独立判断、与老朱对齐后才编排，不得因建议方权威而放行。

**执行前三问**（charter §2.4）：任何角色接到流转来的工作——有疑问有权提三个问题、上游必须解答；无疑问马上开工；终审打回的返工，生产者/建设者必须马上消化处理。

**挂起角色**（charter §2.1〔B2-2〕）：段王爷（内容经销商）、洪七公、历史业务 agent——不进现行框架，未来启用时再入。
...
|------|------|---------|
| 老朱 | —（未建） | —（未建） |
| 王语嫣 | `30_wiki/agent-specs/agent-spec-wangyuyan-orchestrator.md`（draft） | `.agent/wangyuyan-context.md` |
| 欧阳锋 | `30_wiki/agent-specs/agent-spec-ouyangfeng-reviewer.md` | `.agent/ouyangfeng-context.md` |
| 老顽童 | `30_wiki/agent-specs/agent-spec-laowantong-producer.md`（draft） | `.agent/laowantong-context.md` |
| 黄药师 | `30_wiki/agent-specs/agent-spec-huangyaoshi-builder.md`（draft） | `.agent/huangyaoshi-context.md` |
| 风清扬 | `30_wiki/agent-specs/agent-spec-fengqingyang-observer.md`（在途 #428） | —（未建） |

**协作协议**：完整定义在 [[debate-protocol.md]]。核心原则——角色间的辩论不消耗用户注意力。达成共识的事项直接执行。辩论升级到用户时压缩为 ≤3 选项的选择题。

**唯一信息枢纽**：对标 EC 的协调者中枢，KDO 的协调者是**文件系统本身**——各方在同一文件中 append 辩论，用户通过 Obsidian 查看。不需要人工转发。
...
当素材需要跨域整合 + 独立判断（非纯知识提取）时，在三步编译法之后增加第四步 **Judge（独立判断）**：
...
| 产线 | 编译方法 | 产出类型 | 适用素材 |
|------|---------|---------|---------|
| **标准线** | 三步编译法 | concept/tool/framework 卡片 | 已有明确知识结构的分享 |
| **深度合成线** | 四步编译法 | deep-synthesis article | 需要跨域整合+独立判断的素材 |

**深度门禁（D1-D4）**：见 `90_control/quality-gates/content.md` §P1.5。

> 深度合成线是标准线的超集——标准线能产出的，深度线都能产出；反之不成立。不要求所有文章走深度合成线，仅用于需要独立判断的跨域整合（预估频率 1-2 篇/月）。

### 3.4 现行任务队列与提交门禁口径（v2.0 新增，#417）

**任务队列状态机**（`90_control/scripts/queue_transition.py`）：

```
queued → claimed-{instance} → pending_review → reviewed（不通过则退回 queued）
```

**自动登记段**：REVIEW-PENDING（#389/#413）、INBOX-PENDING（watch_inbox）、PROPOSAL-PENDING（#421）——到件自动登记，登记与通知同源。

**提交链**：王语嫣编排门禁 → `kdo pre-submit -f` 阻断门禁 → 欧阳锋终审 → 老朱终稿拍板。
...
| **深度门禁**（v1.8 新增） | 🟠 P1 | **概念卡必须先完成三步编译（浓缩→质疑→对标），才能产文章。** 文章不得基于含 TODO 占位符的概念卡直接写出。三步编译中的"质疑"环节是挖深度的关键——评估前提假设、边界、盲区。跳过质疑 = 文章停留在框架描述层 |

L2 全部为**警告级别**，不做阻断。

> **注（v2.0）**：L2 为 lint 警告级；`kdo pre-submit` 是独立的阻断式提交门禁，未被本节豁免——L2 警告不阻断 lint，但不等于能通过 pre-submit 提交链（见 §3.4、KF-010）。
...
对标 EC 的 20 条核心铁律（CF-001~020），定义 KDO 工业化铁律（KF-001~026）。
...
| KF-010 | **Ship 前全量检查**：`kdo lint --strict` + `kdo validate` + `kdo pre-submit` 全通过后方可 ship | CF-011 | 🔴 |
...
| 编号 | 铁律 | 对标 EC | 严重度 |
|:----:|------|:------:|:------:|
| KF-011 | **审而不改 + 写审分离**：审查者审查产出时只指出问题，不直接修改生产者/建设者的产出文件；写审分离——frontmatter `author` ≠ `reviewed_by`（lint 强制）。审查和执行的分离防止盲区 | CF-012 | 🔴 |
...
| **KF-026** | **卡片体量上限**（v1.7 新增；v2.0 重号修复：原编号 KF-024 与「卡片层行为转化三要件」重号，#417 改为 KF-026）：单卡 estimated_tokens 超过强制拆分线必须拆分。超过目标线不强制，但须在 Constraints 中说明不可拆分原因。拆分线按类型分级：tool/case ≤3000, framework ≤3500, composite-concept ≤5000。只拆不分拆的卡是上下文黑洞——agent 读一张就半饱 | 2026-05-17 设计域讨论 | 🔴 |
| **KF-025** | **域完成四问自检**（v2.0 更新）：Producer 每个域完成前必须自答：① 案例够了吗——每个框架至少配 1 张真实案例卡？② 暗知识在哪里——讲师随口说的操作心法、失败模式、判断口诀有没有被独立提取为 dk 卡？③ 案例有共同模式吗——域内跨案例的共性根因是否已被识别？④ 跨域共同模式——每完成两个以上域，必须做一次跨域对比，提取跨域共同失败模式或可迁移方法论（如"ToB+Lean 的共同验证陷阱"）。四问答不上来→域未完成，不得标记为收工 | 2026-06-16 建模域+业务公式域+精益域审计 → 2026-06-18 跨域模式识别补充 | 🔴 |
...
kdo feedback triage（合并去重积压 feedback）
  ...
  → revision（生产者/建设者按域修正）
  → verify（Architect（欧阳锋）复审关闭）
```
...
### 12.2.1 清单体结构标准（2026-09-04 新增，#639）

卡产出（知识卡/笔记/编译物）的结构表达按清单体五条执行：

1. **分层 ≤3 层**——层级再深就拆卡，不在卡内堆第四层
2. **编号序列**——并列要点用有序/无序列表，不写连续大段散文
3. **重点前置**——每节第一句是结论，细节跟在后面
4. **提出-展开-回收**——节内闭环：先给主张，再展开论据，末尾回收一句
5. **一分钟扫完**——只看标题+列表首行能拿走全卡骨架（全>简但不漏）

**方法论锚（只链不抄，细节以卡片为准）**：[[yt-note-checklist-concept]]（清单体核心概念与原理）、[[yt-note-fact-pattern-insight]]（事实→规律→见解三段论结构）、[[yt-note-five-levels-training]]（清单体训练段位）、[[yt-note-live-field-skill]]（现场实战技能）、[[concept-提升笔记阅读舒适度]]（视觉编码降认知摩擦）。

**门禁落点**：`kdo pre-submit` `_check_qingdanti_structure`（WARNING 提示制不拦截，2026-09-04 上线）——连续散文化段落 ≥8 行或单段 ≥400 字即提醒重组；只向前生效，存量不回扫。
...
| [[failure-modes.md]] | 失败模式库（F-KDO-001~016） | 本手册 §八 的详细展开 |
...
> **历史快照**：本路线图停在 2026-05（Sprint 1-7），现行任务队列以 `production-queue.md` 为准。
...
| 四角色循环 | 入宪六方 + Agent-in-the-loop | 对齐 #416 基本法 §2.1（老朱/王语嫣/欧阳锋/老顽童/黄药师/风清扬），另保留多模态 Agent 辅助模式 |
...
| CF-001~020 铁律 | KF-001~026 铁律 | 18 条对标 + 8 条 KDO 新增（KF-019~026） |
| 只测不修 | 审而不改 | 完全同构 |
...
## §1.13 KF-022 / KF-026 执行感受（v1.7 新增）

Batch C（29 张卡 v1.5 升级）中严格按 KF-022（≤5 张/会话）和 KF-026（estimated_tokens ≤3500）执行的实际观察：
...
**建议**：KF-026 保持 ≤3500 作为基准。如果卡片内容密度低但行数多（大量表格/列表），允许 ≤4000 并标注原因。如果 estimated_tokens 接近 3500 且所有内容必须保留，优先压缩 Claims 节的冗余措辞，不加新内容。
...
| 2026-05-18 | 1.7 | **Batch C 执行经验迭代**——29 张 card v1.5 回溯升级完成后沉淀：新增 §1.10 四种卡片结构与 v1.5 升级路径（标准/pan-product/research/catalog）；新增 §1.11 跨域引用桥接策略（通用桥接卡 + 质量信号）；新增 §1.12 新工具用法（`kdo cards --missing`、`kdo lint --accept-baseline`、`kdo lint --structure-report`、`kdo graph rebuild`）；新增 §1.13 KF-022/KF-026 执行感受与实际建议 | 黄药师 |
...
| 2026-06-01 | 1.9 | **深度门禁**——概念卡必须先完成三步编译（浓缩→质疑→对标），才能产文章。防止"骨架卡→直接写文章"导致输出停留在框架描述层。AGENTS.md 同步更新 | 欧阳锋 |
| 2026-08-22 | 2.0 | **#417 整改**——版本归一（frontmatter/正文同号 v2.0 + 版本规则入文）、§二角色体系替换为入宪六方（逐字对齐 #416 基本法 §2.1）、§3.4 队列/提交门禁口径（queue_transition 状态机 + pre-submit 提交链）、KF-010 纳入 pre-submit、KF-011 扩为审而不改+写审分离、KF-026 重号修复（原 KF-024 体量上限）、现实对账记录（附录 E） | 王语嫣 |

## 附录 E：2026-08-22 现实对账记录（#417）

以下外部漂移已确认存在，但不在本单直接修改（另立项整改）：

- ① `90_control/AGENTS.md` 角色流程仍为旧制（欧阳锋唯一协调节点 / 王语嫣只看不改 / 洪七公、段王爷现役），与 #416 基本法入宪六方冲突——另立项整改。
- ② `90_control/AGENTS.md:386` 与 `90_control/rules-core.md:34` 铁律数仍写 22，过时，应改 26。
- ③ `.agent/laowantong-context.md` / `.agent/context.md` 仍写 KF-025 三问，落后于本手册四问口径。
- ④ 王语嫣/老顽童/黄药师 spec 卡仍为 draft；风清扬 spec 在途（#428）。
- ⑤ #399 `full-library-rescan.py` 无「手册引用口径」检查项，本次以 grep 扫描清单替代附证。
