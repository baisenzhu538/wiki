...
| **KF-025** | **域完成三问自检**（v1.9 新增）：Producer 每个域完成前必须自答：① 案例够了吗——每个框架至少配 1 张真实案例卡？② 暗知识在哪里——讲师随口说的操作心法、失败模式、判断口诀有没有被独立提取为 dk 卡？③ 这些案例有共同模式吗——跨案例的共性根因是否已被识别并写成一张 synthesis 卡？三问答不上来→域未完成，不得标记为收工 | 2026-06-16 建模域+业务公式域+精益域审计 | 🔴 |
