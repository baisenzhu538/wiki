...
## 十二、高密度素材编译规范（v1.2 修订）

本章从 [[high-density-composite-compilation-strategy]] v2.0 提取可执行的规则和标准。

> **v1.0 的复合编译方案（合并为大卡）已被废弃**——该方案按人类线性阅读优化，不适用于 agent 的 RAG 检索模式。

### 12.1 核心架构：细粒度 + 导航层

```
知识层（细粒度卡片）          导航层（Hub Page）

 [[用户视角]] ←─────────── [[需求工具箱 Hub]]
 [[用户分层]] ←───────────      │
 [[场景推演]] ←───────────      │ 概览 + wiki-link 列表
 [[动机阻力]] ←───────────      │ < 3000 字

 [[内核边界]] ←─────────── [[落地工具箱 Hub]]
 [[逻辑MECE]] ←───────────      │
 [[管理三段论]] ←──────────      │ 概览 + wiki-link 列表
```

**原则**：
1. 每张知识地图 → 一张独立概念卡（agent RAG 精确命中）
2. 每份口述稿 → 一张独立概念卡
3. Hub Page 只做导航，不做内容搬运

### 12.2 质量要求（每张独立卡片）

| 要求 | 标准 |
|------|------|
| **三步编译** | Condense ≥3 条 / Critique ≥1 条（指名假设或边界）/ Synthesis ≥2 个 wikilink |
| **Visual Analysis** | 每张知识地图原图五维分析，至少覆盖三个维度 |
| **source_refs** | 全部指向 `10_raw/sources/`（无 `00_inbox/` 残留） |
| **全文字数** | >500 字 |
| **kdo lint** | 0 warning |

### 12.3 Hub Page 构建规范

Hub Page 的定位是**导航节点**，不是内容仓库。它帮助 agent 通过 wiki-link 图快速定位到正确的子卡。

| 要求 | 标准 |
|------|------|
| **大小** | <3000 字 |
| **内容** | 概览（3-5 句）+ 核心概念 wiki-link 列表（每项一句话定位）+ Critique + Synthesis |
| **禁止** | 不得复制子卡完整内容到 Hub Page |
...
1. **细粒度优先**：每张知识地图一张卡，不合并
2. **源文件归档**：图片从 `00_inbox/` → `10_raw/sources/`，source_refs 更新
3. **Visual Analysis**：逐张打开原图分析，不依赖 OCR 文字
4. **三步编译**：每卡独立完成，不批量套模板
5. **Hub Page 轻量**：只写概览 + 导航，不复制子卡内容
6. **单会话上限**：质量修复 ≤5 张 / Hub Page ≤2 张（KF-022）
...
- ❌ 不要跳过 Critique 或 Visual Analysis（L2 强制要求）
- ❌ Hub Page 不要搬运子卡内容（KF-021）
- ❌ 单次会话不要超过批量上限（KF-022）
...
| [[high-density-composite-compilation-strategy]] | 高密度素材编译策略 v2.0 | 本手册 §十二 的操作标准来源 |
...
| **Sprint 5**（v1.2 修订） | 127 张卡质量修复（P0：source_refs + Critique） + Hub Page 构建 | 127 张卡全部通过质量门禁 + 至少 3 张 Hub Page 完成 | ⏳ 进行中 |
| **Sprint 6**（v1.2 新增） | Visual Analysis 补全（81 张）+ 剩余 Hub Page | 全部卡片 Visual Analysis 到位 + 14 张 Hub Page 完成 | ⬜ |
...
| 六大防呆 | 六项防呆 | 去掉"环境锁定"，增加"源文件路径防呆"和"Hub Page 大小防呆" |
...
| 只测不修 | 审而不改 | 完全同构 |
| — | **Agent 优先设计原则** | KDO 独有：细粒度卡片优化 RAG 检索，Hub Page 提供导航 |
...
| 2026-05-11 | 1.1 | 重大更新——手册更名、新增复合编译哲学、Visual Analysis 方法论、KF-019~022 铁律、F-KDO-013~015 失败模式、技术笔记、§十二复合编译规范 | 欧阳锋 |
| 2026-05-11 | 1.2 | **架构修正**——废弃 v1.0 复合编译方案。重新判断：知识库主要用户是 AI agent，细粒度卡片是最优结构。①§1.4 改为「Agent 优先」原则②§3.2 改为细粒度质量达标 + Hub Page 变体③修订 KF-021（Hub Page 不得搬运内容）④修订 §十二为细粒度+导航层架构⑤模板复合版 → Hub Page 版⑥更新实施路线图（Sprint 5-6 修正） | 欧阳锋 |
