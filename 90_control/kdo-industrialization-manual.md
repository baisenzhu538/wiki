...
| **KF-024** | **卡片体量上限**（v1.7 新增）：单卡 estimated_tokens 超过强制拆分线必须拆分。超过目标线不强制，但须在 Constraints 中说明不可拆分原因。拆分线按类型分级：tool/case ≤3000, framework ≤3500, composite-concept ≤5000。只拆不分拆的卡是上下文黑洞——agent 读一张就半饱 | 2026-05-17 设计域讨论 | 🔴 |
