...
| MiniMax 直连（无 CLI 壳，VLM/OCR 管线） | **MiniMax**（api.minimaxi.com） | `MiniMax-M3` | **订阅 key 挂 `~/.kdo/config.yaml` minimax 段**（老朱 09-06 提供，厂级共享凭据不进 git），指纹 `e0f4018e…oFdc`【欧阳锋 12:43 终审核算】 | HTTP POST `/v1/text/chatcompletion_v2`（199 tokens 实测 200；欧阳锋终审问端点——文本/VLM 同走此端点，非 anthropic 形态，王语嫣 12:40 实测口径） | 🟢 活（12:33 挂载+12:40 实测；按量旧 key 已余额尽 status 1008，见洪七公建议书） |
