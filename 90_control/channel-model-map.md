...
| MiniMax 直连（无 CLI 壳，VLM/OCR 管线） | **MiniMax**（api.minimaxi.com） | `MiniMax-M3` | **订阅 key 挂 `~/.kdo/config.yaml` minimax 段**（老朱 09-06 提供，厂级共享凭据不进 git） | chatcompletion_v2 最小调用（199 tokens 实测 200） | 🟢 活（12:40 王语嫣挂载+实测；按量旧 key 已余额尽 status 1008，见洪七公建议书） |
