...
用例：tier1 完整路径 / tier2 反引号裸 id / tier3 裸 id+声明目录（#666 写法）/
     tier3b type×N（#668 写法）/ 非 draft 幂等护栏 / 未识别降级提醒 /
     翻转异常不阻断终审 / fail 分支不翻转 / mark_card only_flip_from 门控。
...
def _sandbox_index(td):
    root = Path(td) / "30_wiki"
...
class TestResolveDeliveredCards(unittest.TestCase):
    def setUp(self):
        self._old_index_fn = qt._card_stem_index

    def tearDown(self):
        qt._card_stem_index = self._old_index_fn

    def _bind(self, td):
        qt._card_stem_index = lambda: _sandbox_index(td)
    ...
    def test_tier1_full_path(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            cards, unres = qt._resolve_delivered_cards(
                _report(f"新卡 `30_wiki/frameworks/framework-demo-a.md`。"), "t.md")
    ...
    def test_ambiguous_title_defers_to_human(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            self._bind(td)
            _mk_card(td, "framework-一堂-关键假设")
            _mk_card(td, "framework-一堂-关键假设-三板斧")
            cards, unres = qt._resolve_delivered_cards(
                _report("framework×1（关键假设）。"), "t.md")
...
class TestFlipDeliveredCards(unittest.TestCase):
    def setUp(self):
        self._old_index_fn = qt._card_stem_index
        self._old_commit = qt._git_commit_card_flips
        qt._git_commit_card_flips = lambda *a, **k: None  # 沙盒卡在仓外，commit 走不到；显式短路
    ...
    def test_flip_draft_card_on_disk(self):
        with tempfile.TemporaryDirectory() as td:
            qt._card_stem_index = lambda: _sandbox_index(td)
            p = _mk_card(td, "framework-demo-a")
            msg = qt._flip_delivered_cards(_task_file(td, _report(
                "新卡 `30_wiki/frameworks/framework-demo-a.md`。")), "欧阳锋")
    ...
    def test_no_wiki_deliverable_returns_empty(self):
        with tempfile.TemporaryDirectory() as td:
            qt._card_stem_index = lambda: _sandbox_index(td)
            msg = qt._flip_delivered_cards(_task_file(td, _report(
                "脚本 `90_control/scripts/x.py` 与 log `logs/x.log`。")), "欧阳锋")
            self.assertEqual(msg, "")


def _task_file(td, report):
    tf = Path(td) / "task_8888_flipcards.md"
    tf.write_text(
        f"---\nid: task_8888_flipcards\nstatus: pending_review\nassignee: laowantong\n---\n\n"
        f"# 任务\n\n## 执行报告\n\n{report}\n{OPINION}",
        encoding="utf-8",
    )
    return tf
...
class TestActionReviewEndToEnd(unittest.TestCase):
    ...
    def setUp(self):
        self._saved = (
            ...
            qt._check_issue_disposition, qt.QUEUE_PATH, qt._card_stem_index,
        )

    def tearDown(self):
        (qt.parse_queue, qt.find_task, qt._find_task_file_dual,
         qt.apply_updates, qt.QueueLock, qt._check_review_authority,
         qt._check_issue_disposition, qt.QUEUE_PATH, qt._card_stem_index) = self._saved
    ...
    def test_pass_already_reviewed_card_no_double_write(self):
        with tempfile.TemporaryDirectory() as td:
            tf, p = self._wire(td, card_status="reviewed")
            ok, msg = qt.action_review("task_8888_flipcards", "pass", "欧阳锋", "A-")
            self.assertTrue(ok, msg)
            self.assertIn("未动", msg)
            self.assertIn("reviewed_by: 欧阳锋", p.read_text(encoding="utf-8"))
    ...
    def test_unresolvable_report_still_fires_612_reminder(self):
        """#612 兜底网：自动翻转识别不出 → 老提醒仍输出。"""
        with tempfile.TemporaryDirectory() as td:
            qf = Path(td) / "queue.md"
            qf.write_text("# q\n", encoding="utf-8")
            qt.QUEUE_PATH = qf
            ...
            tf.write_text(
                "---\nid: task_8888_flipcards\nstatus: pending_review\nassignee: laowantong\n---\n\n"
                "# 任务\n\n## 执行报告\n\n**交付物**：交付卡 `30_wiki/frameworks/手工路径写法.md`"
                "（盘上无此文件）。\n**完成内容**：略\n" + OPINION,
                encoding="utf-8",
            )
            qt._card_stem_index = lambda: {}
            rows = [{"seq": "8888", "task_id": "task_8888_flipcards", "name": "n",
                     "status": "pending_review", "assignee": "laowantong", "raw": "| 8888 |"}]
            qt.parse_queue = lambda: rows
            qt.find_task = lambda tid, rows_=None: next(
            ...
            ok, msg = qt.action_review("task_8888_flipcards", "pass", "欧阳锋", "A-")
            self.assertTrue(ok, msg)
            self.assertIn("待 review_mark 转正", msg)
