...
容灾口径（2026-08-24 用户校正）：**git 即容灾**——wiki 全资产（脚本/文档/队列/任务单/总表）在 git 历史+多副本中，丢失可恢复；L1-full 为**可再生采集副本**（源文件仍在原位置，重采集即可重建，归档 zip 兜底旧天）。**容灾无缺口**。
...
3. 依赖图随资产增补维护（L4）
4. ~~L3 演练每季度人工复演~~ → **自动化（2026-08-24 用户指令"探针现在是阶段性的，以后必须自动化"）**：可自动项已挂 health-check（infra-status 资产+未登记 / 事件库恢复副本验证），每日自动；破坏性演练保留人工低频

## 自动化状态（2026-08-24 用户指令落地）

| 检验 | 自动机制 | 频率 |
|:--|:--|:--|
| 资产存在+未登记 | health-check → infra-status | 每日 02:07 |
| 事件库恢复副本验证 | health-check → recovery-check | 每日 02:07 |
| L1 verify | l1_capture 采集后自动 | 每 30 分钟 |
| 幂等（增量） | l1_capture 增量机制本身 | 每 30 分钟 |
| 探针六信号 | conveyor-probe 计划任务 | 每 10 分钟 |
| 破坏性恢复演练 | 人工低频（低风险窗口） | 季度 |
