...
function subThemeClassify(r) {
  ...
  if (allText.includes('ipo模型') || allText.includes('ipo学习') || allText.includes('ipo科学')) return { theme: 'yitang', subTheme: 'IPO学习模型' };
  if (allText.includes('业务公式')) return { theme: 'yitang', subTheme: '业务公式拆解' };
  if (allText.includes('波特五力') || allText.includes('商业分析方法论') || allText.includes('差距分析') || allText.includes('鱼骨图')) return { theme: 'yitang', subTheme: '商业分析工具' };
  ...
  if (allText.includes('需求评估')) return { theme: 'yitang', subTheme: '需求评估方法论' };
  if (allText.includes('九层宝塔') || allText.includes('战略培训') || allText.includes('战略分析')) return { theme: 'yitang', subTheme: '战略规划方法论' };
  if (allText.includes('精益')) return { theme: 'methodology', subTheme: '精益创业与假设验证' };
  if (name.includes('原则') || summary.includes('《原则》')) return { theme: 'personal', subTheme: '《原则》与个人决策系统' };
  if (name.includes('凯文凯利') || name.includes('必然')) return { theme: 'personal', subTheme: '科技趋势与个人判断' };
  ...
  if (name.includes('偶然') || name.includes('必然')) return { theme: 'personal', subTheme: '复杂系统与偶然必然' };
  if (name.includes('中国行') || name.includes('使命')) return { theme: 'personal', subTheme: '个人使命与教学创新' };
  if (name.includes('ai场景落地') || name.includes('ai落地') || name.includes('ai方法论') || allText.includes('找老的干小的')) return { theme: 'ai', subTheme: 'AI落地方法论与场景选择' };
  if (name.includes('ai数据') || allText.includes('数据飞轮')) return { theme: 'ai', subTheme: 'AI数据资产与数据飞轮' };
  if (name.includes('龙虾') || name.includes('openclaw') || name.includes('open cloud') || name.includes('智能入口') || name.includes('ai工具')) return { theme: 'ai', subTheme: 'AI工具栈与协作平台' };
  if (allText.includes('ai组织') || allText.includes('组织行为')) return { theme: 'ai', subTheme: 'AI时代的组织分工' };
  if (name.includes('路演') || name.includes('项目汇报') || name.includes('战队') || name.includes('大航海')) return { theme: 'ai', subTheme: 'AI大航海项目路演案例' };
  if (name.includes('酒店') || name.includes('贝壳') || name.includes('选品') || name.includes('标签审核') || name.includes('外呼')) return { theme: 'ai', subTheme: '产业AI运营案例' };
  if (name.includes('剧本创作') || name.includes('短剧') || name.includes('四格漫画') || name.includes('ai内容营销') || name.includes('剧本杀')) return { theme: 'ai', subTheme: 'AI内容创作案例' };
  if (name.includes('geo') || name.includes('aio') || name.includes('机优') || name.includes('营销')) return { theme: 'ai', subTheme: 'AI营销与搜索优化' };
