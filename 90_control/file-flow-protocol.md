...
**aliases 规范（#494，2026-08-24）**：aliases = 检索别名，只放角色名/中文名/主题词；**禁结构词**（`audience:`/`scene:`/`skill-level:`/`domain:`/`type:` 等前缀词——结构词归 tags 维度）、**禁路径词/文件名**（如 `decisions.md`/`20_memory`）——字段不承载不该承载的内容（与 #428/#431 路径词、#484 来源词同族）。tags-audit 第6指标（aliases 污染率）把门，检查器见 #494。
...
### 6.5 外部监督维度（#486，2026-08-24 老朱拍板）

- **任何门禁机制必须指明外部监督者；无外部监督者的机制 = 个人自绑定，不得声称组织级门禁。**
- 门禁设计/审查时必问：「**该机制有没有外部监督者？**」——例外由谁复核、解冻由谁批准、验收由谁执行、声明由谁复核，逐项写明监督者。
- 核心差异（#469 同构背书补差异）：组织治理效力来自**外部监督/制衡**，个人自绑定无制衡。同构的是「障碍设计思想」，不同构的是「谁执行与谁监督」——机制在规范但无外部监督者 = 自己执行自己检查 = 个人自绑定，效力打折；force 台账留痕但无复核回路 = 例外被记录 ≠ 例外被审查。
