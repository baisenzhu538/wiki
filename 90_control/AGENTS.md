...
KDO 知识工厂六角色分工，协作规则定义在 `90_control/debate-protocol.md`：
...
| **Producer** | 老顽童 | 卡片量产、文章/内容产出、跨域合成、新域编译。产能主力 | **WSL tmux `claude`（CLI 主力）**，飞书 Hermes 备用 |
| **Consultant** | 王语嫣 | **诊断咨询者**——基于知识库做诊断式咨询、把模糊商业问题匹配到框架、追问诊断、产出诊断记录和反馈。**只看不改，不碰卡片** | 飞书 Hermes agent，通过飞书对话直接服务用户 |
...
王语嫣（飞书咨询 — 诊断匹配框架 → 写反馈）
  ↓
欧阳锋（审查+协调 — 将反馈转化为任务工单）
  ├── 黄药师 ← 工厂建设工单（基础设施）
  ├── 老顽童 ← 内容生产工单（卡片/文章）+ 修卡工单（来自王语嫣的反馈）
  ├── 洪七公 ← 多模态工单（视觉/设计输出）
  └── 段王爷 ← 发布工单（分发/反馈）
```

核心原则：欧阳锋是唯一协调节点。角色之间不互相派活——都通过欧阳锋中转。
王语嫣是特例：她直接服务用户（飞书对话），但产出（诊断记录/反馈）走欧阳锋审查后再分发给老顽童。
...
|------|------|
| **接收输入** | 用户通过飞书直接提问 |
| **查询知识库** | `kdo query "<问题>"` / `kdo graph query "<问题>"` |
| **诊断记录** | `60_feedback/diagnosis/diag_YYYYMMDD_<slug>.md` |
| **修正反馈** | `60_feedback/corrections/corr_YYYYMMDD_<topic>.md` |
| **不允许碰** | `30_wiki/` / `40_outputs/` / `90_control/` —— 只看不改 |

**核心约束**：王语嫣不写入任何 wiki 目录。她只读知识库 + 写诊断记录和反馈。修卡和写卡通过诊断记录传递给老顽童（经欧阳锋协调）。
...
| 15 | **不准未批准执行 D4 自我修改** | E018 家族（#275） | Agent 修改自己的 context/skill/配置/约束 = D4 自我修改 → 提交后必须王语嫣/欧阳锋批准，未批准 = 无效变更。写入 `.agent/decisions.md` 决策记录（类型 D4 + claim-state + 批准人）。关联 E018（自建卡伪造审查记录） |
