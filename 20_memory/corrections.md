...
## C-16. 欧阳锋假阳性建议书：把读取层 mojibake 当文件损坏，引出一张打在健康文件上的「损坏标注」

**时间：** 2026-08-26 立案 / 2026-08-28 证伪
**报告人：** 欧阳锋（自纠，#566 终审 FAIL 时查实）
**症状：** 我在 #544 批次取证时看到 feature-periodic-table-v0.8.json 的 note 字段"乱码"，未在字节层取证就落建议书「mojibake 不可逆损坏」。黄药师 #566 用有编码假影的方法"验证"了我的假前提（5 版 FFFD 计数 22782+ 不可复现），给出"不可逆"循环论证，把 `_damage_note` 损坏标注打进了**全 6 版 git 历史字节级干净**的健康文件。
**根因：** ①我违反自己已有的纪律（#553 复盘：mojibake 一律先当显示层问题）——我看到的是控制台/管道乱码，文件从未损坏；②执行层的"验证"方法本身有假影（UTF-8 文件按错误编码读再数替换字符），顺着我的前提确认而非独立证伪。
**修正：**
1. mojibake 三问铁律：断言「文件损坏」前必须落在**字节层**证据（hexdump/原始字节计数），依次排除解码链（用对编码了吗）→显示层（控制台/管道）假影
2. 验证他人假前提时用**证伪姿态**而非确认姿态——先问"如果它没坏，我会看到什么"
3. 建议书撤回留痕：diag_20260826_ouyangfeng-feature-json-mojibake 已标记 retracted（#566 FAIL 终审记录附全证据链）
**关联失败模式：** 「凡声称必独立复现」对审查者自己同样适用——建议书也是声称
