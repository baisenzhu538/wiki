...
| 记忆胶囊 L0 主库 | `C:\Users\Administrator\.kdo-memory\L0\activity_log.db` | #432/#427 拍板 A 本机主库（git 外，WAL）；镜像 `D:\KDO-memory\L0-backup\`（robocopy /MIR）；verify=memory_capsule.py；**写入端**=daily-context-save 挂钩（#434，save 成功自动写 review_saved 事件）；**权责**=黄药师建设/维护、风清扬审计（老朱 08-23 拍板） |
