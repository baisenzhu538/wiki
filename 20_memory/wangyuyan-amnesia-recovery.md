...
**老朱说"继续"→ 读本节 + `90_control/parking-lot.md` + `queue_transition.py status` 核实时队列；**上线第一动作=自检五件套**（启动恢复清单顶部：门铃 cron/探针三件套/relay 4444/拉起器通路/队列回源，09-03 老朱令自动执行）；新会话第一件事重建门铃 cron（会话级，死了就没了）。报状态必回源（E051/W9）；常驻服务只挂计划任务不挂会话；远程操作先轻量 GET 再写；时间戳一律机器取（禁手写，夜班自纠过一次）。**
