...
| **26** | **🆕 咨询框架桥接卡（王语嫣需求）** | **🔜** | **P0试点：MECE + Issue Tree（格式验证）→ P0量产：Hypothesis-Driven系列 + 5 Whys → P1：Pyramid Principle + 7-S + Trusted Advisor。源材料见王语嫣诊断记录 `60_feedback/diagnosis/diag_20250611_consulting-skills-research.md`。每张必须写Bridge节+diagnostic_signals+≥2攻击者。80/20建议不做** |
...
| # | 任务 | 状态 | 备注 |
|---|------|:--:|------|
| **1** | **🆕 持续诊断：接用户飞书提问** | **🔜** | **核心职责。每次咨询写诊断记录到 60_feedback/diagnosis/** |
| **2** | **🆕 概念 Gap 标注** | **🔜** | **诊断中识别知识库缺口，在 Gap 节标注。来源锚点+桥接关系必须写** |
