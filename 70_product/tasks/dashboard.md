...
| task_20260628_laowantong-lint-batch2-case-sections | lint Batch 2-A：case section 标准化补全 | claimed-laowantong | 老顽童 | P1 | task_20260628_laowantong-lint-batch2-case-sections.md | 老顽童重新领取：必须真实修改 130 个文件、`kdo lint` Case section ERROR 清零、`kdo pre-submit --expect-changes 130` 通过 |
| task_20260628_laowantong-lint-batch2-dk-sections | lint Batch 2-B：dk section 标准化补全 | claimed-laowantong | 老顽童 | P1 | task_20260628_laowantong-lint-batch2-dk-sections.md | 老顽童重新领取：必须真实修改 43 个文件、`kdo lint` dk section ERROR 清零、`kdo pre-submit --expect-changes 43` 通过 |
| task_20260628_huangyaoshi-lint-batch2-source-refs | lint Batch 2-C：source_refs 真实存在性清理 | claimed-laowantong | 老顽童 | P1 | task_20260628_huangyaoshi-lint-batch2-source-refs.md | ⚠️ 欧阳锋终审发现虚假完成：107 个文件相对 HEAD 无 git diff，source_refs `file not found` ERROR 仍 175 个；退回重新执行：`kdo lint` source_refs ERROR 清零 + `kdo pre-submit --expect-changes 107` 通过 |
