...
| 7 | KDO 快速上手指南 → 中视频（分镜+画面） | 🔨 | 7a v2 当前。老顽童脚本已重写（十指讲香版），分镜需对齐新版脚本。详见表下方 |
...
|:--:|------|------|------|------|:--:|------|
| **7a v2** | 分镜修订 ← **当前** | 新版 `01-script.md`（十指讲香版） | `02-storyboard.md` v2 | 老顽童 Gate 0 通过 | 45min | 🛑 **Gate 1**：分镜必须对齐新版脚本的烹饪比喻/墓碑意象/产品经理故事/4层情绪弧线。审查标准见下方 [[#🛑 Gate 1 审查标准（分镜 v2）]] |
...
洪七公 7a v2 修订分镜 ← 当前位置
    │
    ▼
🛑 GATE 1 — 欧阳锋审查分镜 v2（对齐新版脚本的金句/比喻/情绪/故事线？）
    │
    ▼
...
| 洪七公 | 视频分镜修订（7a v2）+ 画面制作（7b-7g） | 🔨 7a v2 当前。老顽童新版脚本已完成（Gate 0 ✅），旧分镜需对齐十指讲香版 |
...
| 05-20 | 老顽童 | 🎬 KDO 视频脚本重写（十指讲香版） | ✅ 完成。从"清单式讲香"彻底重写为十指全覆盖版 |
...
### Gate 1 v2 — 洪七公分镜修订 → 🛑 待提报

**当前状态**：洪七公 7a v2 进行中。修订完成后提报 Gate 1。审查标准见 [[#🛑 GATE 1 审查标准（分镜 v2）]]。

### 审批节点状态

```
🛑 GATE 0 ✅ 通过（老顽童脚本十指讲香版 · A+）
🛑 GATE 1 ⏳ 等待洪七公 7a v2 提报
🛑 GATE 2 ⏳ 等待 Gate 1 通过 + 7b-7d
🛑 GATE 3 ⏳ 等待 Gate 2 通过 + 7e-7f
🛑 GATE 4 ⏳ 等待黄药师 render + 7g
```
