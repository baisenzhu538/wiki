...
|| 9 | 🔥 科学决策域 VA 交叉审查 | ✅ | 全部完成！35张图逐图 vision 审查，报告：`va-cross-review-scientific-decision-2026-05-23-final.md`。通过率 ~71%（20/28），🔴 重写 5 张，⚠️ 小修 8 张，结构性发现 4 项（张冠李戴/颜色偏差/虚构元素/归属错位） |
|| 9 | 🎯 **VA 前置 + 文章重启** ← 当前 | 🔥 | **A1 进行中**：前5张🔴卡VA已完成并写入（进步大地图/创业地图/个人地图/管理地图/36计全套地图）。待欧阳锋抽检后做后5张。 |
...
> **背景**：老顽童（替代者）对科学决策域 10 张现存卡 + 1 张新 ABCD 卡做了批量 VA 增强。每张配图写了五维分析（色彩语义/空间结构/信息层级/负形利用/视觉隐喻）。欧阳锋无法识图，VA 内容审查出现真空。
> **洪七公的角色**：你是 VA 专家，能看图。你的任务不是重写，是**验证**——逐图对照，判断描述是否准确、是否遗漏关键视觉信息。

#### 入参

| 项目 | 路径 |
|------|------|
| 待审卡片（11 张） | `30_wiki/concepts/yt-decision-*.md`（10 张）+ `30_wiki/frameworks/yt-decision-abcd-model.md`（1 张） |
| 原图（36 张） | `00_inbox/科学决策/*.png` |
| VA 位置 | 每张卡 frontmatter 的 `visual_analysis:` 字段，或正文 `## 视觉结构分析` 节 |

#### 审查清单（每张图）

| # | 检查项 | 怎么查 |
|---|--------|--------|
| 1 | **色彩语义** | 打开原图，对照 VA 描述中的色块命名——红/黄/蓝/绿/灰/黑/玫红是否准确？冷暖判断是否合理？ |
| 2 | **空间结构** | 对照原图布局——左右/上下/三栏/矩阵/对角线关系是否与图一致？比例描述是否准确？ |
| 3 | **信息层级** | L1→L2→L3→L4 的层级划分是否符合图中实际信息密度和视觉权重？ |
| 4 | **负形利用** | 留白、空白区、负形结构的描述是否符合原图设计意图？ |
| 5 | **视觉隐喻** | 操作系统/天平/容器/棋盘/交通灯等隐喻标签是否贴切？有无更准确的替代隐喻？ |
| 6 | **遗漏检查** | 图中是否存在 VA 描述**未提及**但视觉上显著的元素？ |

#### 输出

| 文件 | 内容 |
|------|------|
| `60_feedback/corrections/va-cross-review-scientific-decision-2026-05-21.md` | 审查报告：逐图结论（✅ 准确 / ⚠️ 小修 / 🔴 重写），标注具体哪一行、哪个维度、问题是什么 |

#### 工作流

```
1. 读原图（00_inbox/科学决策/）
2. 读对应卡片的 visual_analysis 字段
3. 逐图对照五维 → 结论
4. 汇总 → 报告写入 60_feedback/corrections/
5. 通知欧阳锋审查报告
```

#### 规则

- **原图优先于卡片文字**——如果 VA 描述与原图不一致，以原图为准
- **不自行修改卡片**——发现问题写入报告，由老顽童修改
- **🛑 分两批提报**：前 18 张图审完先提报一次，避免欧阳锋等太久
