...
| # | 任务 | 来源 | 优先级 | 工作量 |
|:--:|:--|:--|:--:|:--:|
| P-1 | **查询结果 Core 优先排序**：`kdo query` 命中结果按 Core → Extended → Reference 分层排列，Agent 先扫骨架再下钻 | 用户 2026-06-20 | P1 | 0.5d |
| P-2 | **domain 自动加权**：Agent 已知当前工作域时，`kdo query` 自动加权同域结果，减少跨域噪音 | 用户 2026-06-20 | P1 | 1d |
...
| P-5 | **多库架构设计**：四库拓扑（商业+人 / 电子+软件 / 结构工程 / 平面设计+推广）+ 调度中枢王语嫣（dashboard/production-queue 跨库协调）+ 欧阳锋质量终审 + 子库精简配置 + 启动序列 + Agent 分拆方案。已有讨论稿，2026-06-27 角色调整后需更新中枢人选 | 用户 2026-06-20 / 黄药师 2026-06-28 更新 | P2 | 待定 |
