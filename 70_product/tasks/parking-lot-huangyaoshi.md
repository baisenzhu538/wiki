...
| P-23 | **能力中台 Phase 1**：VLM 能力上线——建 `_capability_hub/` 骨架（registry + base + config）+ VLM 核心模块 + `python -m capability_hub list`。API Key 写死在 config.py，Agent 零配置。详见 `plan_20260707_capability-hub-architecture.md` | 洪七公建议书 + 黄药师架构方案 2026-07-07 | P1 | 0.5-1d | 待排期 | 无 |
