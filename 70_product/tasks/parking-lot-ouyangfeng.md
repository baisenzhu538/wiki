...
| O-18 | **#586 返工遗留小项**（2026-09-01 欧阳锋返工复审 PASS A- 时记录）：①Brooks 卡 Sam Altman 博客为域级验证（L1 域级 HTTP 200），具体篇目未逐篇定位，卡内已标「引用具体表述需回源」——下次触碰该卡时补篇目级锚点；②本批 4 卡「外部验证」节锚点为 09-01 单次 curl 实测快照（TED/a16z/Anthropic/SWE-bench 等 200），链接 rot 风险随时间增长——下次触碰该批卡时复验锚点活性。均不阻塞入库 | 欧阳锋 2026-09-01 | P3 | 0.2d | 待排期 | 老顽童（下次触碰对应卡时顺带） |
| O-19 | **双欧阳锋实例并发终审同单**（2026-09-01 深夜 #594 实证）：王语嫣时钟 23:02 拉起 headless 实例终审 + CLI 实例同时独立终审同一任务——双跑结论幸运同向（PASS），但两审互不知情、各写终审记录、各留 todos；headless 漏两卡 review_mark 由 CLI 侧补齐。若结论相左（PASS vs FAIL）将出现队列状态冲突，需三裁机制。修复方向：①编排侧——时钟拉起 headless 终审前先 grep 任务单 status，已 reviewed 则跳过拉起；②实例侧——CLI 实例开工终审前先查队列行是否已流转（本单漏查，教训在己）。挂上后待王语嫣裁定机制归属 | 欧阳锋 2026-09-01 | P2 | 0.2d | 待裁定 | 王语嫣（编排层防重拉起） |
| O-20 | **旧 mp4 陈旧引用残留**（2026-09-02 #604 终审实证）：10_raw/sources 8 文件 + 00_inbox/wechat-collect 2 文件仍引 `60_feedback/wechat-collect/` 旧路径，指向 6725b942182f6277/AWyGiJIRgc/f4faadff37c0b43b/7666646931699367986 等不在 #604 六件范围的 hash——目标文件早已不在原位，属历史陈旧引用（#604 边界已声明不动）。建议下次散点审计顺带清理：按 hash 查实际落位（10_raw/assets 或 quarantine）后批量改指或标注失效 | 欧阳锋 2026-09-02 | P3 | 0.2d | 待排期 | 黄药师（散点审计下一批顺带） |
