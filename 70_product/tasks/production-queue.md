...
| 201 | `task_20260726_thought-liberation` | 解放思想探索营·全量卡片化：25张新卡+8张已有卡修改（6 Waves）。P0骨架3张+P1工具5+P1案例4+P1子工具11+digest 1+收尾1。含Agent可发现性规范（discoverable_by/transferable_to/aliases） | pending_review | laowantong | 25张新卡+8张已有修改 | 无 | `60_feedback/tasks/task_20260726_thought-liberation.md` | 欧阳锋终审A-(第二轮)：Wave 4 9/9 discoverable_by补齐验证通过。P0 A级不动。<!-- 手动终审：队列状态为queued非pending_review，queue_transition.py拒绝执行 --> |
| 202 | `task_20260726_wangyuyan-bad-world` | 坏世界研究·协作底层哲学：3张卡（1 concept + 2 tool）。赵汀阳政治哲学→人类咨询场景。地位互换测试+博弈环境自检清单。补管理域制度设计的哲学地基 | pending_review | laowantong | 3张（1 P0 + 2 P1）+3张已有卡related更新 | 无 | `60_feedback/tasks/task_20260726_wangyuyan-bad-world.md` | 莫非口述解读赵汀阳《坏世界研究》；诊断见 `60_feedback/diagnosis/diag_20260726_wangyuyan-bad-world.md` |
| 203 | `task_20260726_wangyuyan-kdo-infra-health` | KDO基础链路健康检查+快赢：全库基线扫描(source_refs/broken links/status漂移)+17牌组件库draft→reviewed+Dashboard MVP。P2优先级——紧急任务到达时让路 | pending_review | huangyaoshi/欧阳锋 | 3项（扫描+终审+Dashboard） | 无 | `60_feedback/tasks/task_20260726_wangyuyan-kdo-infra-health.md` | 小昭外部审计建议；只取基线数字不修——修复是后续独立任务 |
| 204 | `task_20260726_wangyuyan-obsidian-ai-collab` | Obsidian+AI协作最佳实践：2新卡（concept结构化命名即基础设施 + method Obsidian协作五要素）+ 1已有卡升级（tool-清单式笔记法新增AI协作场景）。YAI诞生拼图+命名=基础设施+聊天不适合长期记忆 | pending_review | laowantong | 2新卡+1升级 | 无 | `60_feedback/tasks/task_20260726_wangyuyan-obsidian-ai-collab.md` | 于陆Live83分享+月白命名卡共振；诊断见 `60_feedback/diagnosis/diag_20260726_wangyuyan-obsidian-ai-collab.md` |
| 205 | `task_20260726_wangyuyan-bfm-yb-insights` | 半肥猫+月白线下聚会·暗知识卡片化：4张新卡（十字象限框架+压缩路径+AI共学模式+AI跨域推导）+2张已有卡注入+每卡标注建议标签。等#204完成后执行 | queued | laowantong | 4新卡+2注入 | 无 | `60_feedback/tasks/task_20260726_wangyuyan-bfm-yb-insights.md` | DeepSeek深挖追加：AI共学模式(老朱)+AI跨域推导(老朱)+AI不受压迫(半肥猫) |
| 206 | `task_20260726_wangyuyan-kdo-tag-system` | KDO多维标签体系恢复：恢复tag-registry.yaml（4维27标签）+scaffold模板新增tags字段+lint新增tags检查（WARNING级）。半肥猫标签哲学落地。存量卡不批量返工 | queued | huangyaoshi | 3项infra变更 | 无 | `60_feedback/tasks/task_20260726_wangyuyan-kdo-tag-system.md` | 飞书王语嫣标签缺口诊断；tag-registry已设计但归档——恢复即可 |
...
| 209 | task_20260726_wangyuyan-aliases-source-lint | 牌L9：aliases源材料名检查——source_refs引用的材料名必须在aliases中。kdo pre-submit新增_check_aliases_has_source_name()。与#199同级WARNING | queued | huangyaoshi | 1项lint规则 | 无 | 60_feedback/tasks/task_20260726_wangyuyan-aliases-source-lint.md |
