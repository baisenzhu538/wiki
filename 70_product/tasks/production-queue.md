...
| 28 | `task_20260629_kimi-lint-content-debt-by-domain` | lint 内容债按 domain 分批清理 | claimed-kimi | 老顽童(Hermes) | ~3255 WARNING / 14 个子任务 | 依赖 #27 reviewed | `60_feedback/tasks/task_20260629_kimi-lint-content-debt-by-domain.md` | 欧阳锋中期审查：第一轮净减 31 WARNING（3286→3255）；3 张 ai-collaboration 卡补 Critique 外部反对者；8 个 title_count_mismatch 修复；2 个 When NOT to Use copy-paste 重写；32 个 Wiki page 无可见来源清零；body 过短 80→651 为删除空 section 的已知副作用；`kdo lint --domain/--summary` 已由欧阳锋实现并可用；任务继续 |
| 29 | `task_20260629_wangyuyan-goat-milk-channel-partnership-bridge` | 羊奶「卖地图」跨域桥接卡生产 | claimed-hermes | 老顽童(Hermes分身-Claude) | 3 张卡 | 无 | `60_feedback/tasks/task_20260629_wangyuyan-goat-milk-channel-partnership-bridge.md` | 王语嫣九层深挖完成；产出 framework + case + tool 三张跨域桥接卡，连接渠道增长×商业模式×合伙激励三域 |
