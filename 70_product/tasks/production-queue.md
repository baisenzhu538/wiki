...
| 264 | \`task_20260809_huangyaoshi-e018-and-protocol-fixes\` | E018铁律升级（全角色context+prompt）+协议v0.1三修正（verified语义/verify_note/schema）+cap_hub注册 | queued | huangyaoshi | 3项 | #252 reviewed | \`60_feedback/tasks/task_20260809_huangyaoshi-e018-and-protocol-fixes.md\` | 欧阳锋#252条件项；E018四提四证刻不容缓 |
| 265 | \`task_20260809_wangyuyan-external-agent-feedback-loop\` | 外部Agent反馈→生产深化闭环机制（四回路：知识/数据/流程/模式）+教练反馈dogfooding | queued | wangyuyan | 机制文档 | #264（流程回路依赖） | \`60_feedback/tasks/task_20260809_wangyuyan-external-agent-feedback-loop.md\` | 用户战略问题+试点实证固化；双驱动机制 |
