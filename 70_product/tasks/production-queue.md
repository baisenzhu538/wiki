...
| 207 | `task_20260726_wangyuyan-debt-cleanup` | 存量卡质量债务渐进清理：6张月白命名卡draft→reviewed+标签补全+定位声明补全+历史残留清理。P3长程——老顽童碎片时间做，不设deadline，每批5-10张 | pending_review | laowantong | 10项待清理（渐进追加） | 无 | `60_feedback/tasks/task_20260726_wangyuyan-debt-cleanup.md` | 不批量返工——该卡因其他原因返工时顺手处理；发现新问题追加到清单 |
...
| 211 | task_20260727_wangyuyan-phase2-tag-enrich | Phase 2高价值卡人工精标：前50张framework/digest/agent-spec卡标method+industry+value-tier。标准：外部Agent只看tags不需kdo_read就能判断能否使用 | queued | laowantong | 前50张优先 | 无 | 60_feedback/tasks/task_20260727_wangyuyan-phase2-tag-enrich.md | 与#207分工：#207消灭null，#211让Agent只看tags可判断
| 212 | task_20260727_wangyuyan-rrf-tag-mcp | RRF tag维度匹配+MCP标签暴露：RRF融合感知tags维度智能排序+MCP search返回tags字段。小昭不看全文就能判断卡片适用性 | queued | huangyaoshi | 2项infra | 依赖#211 Phase2精标覆盖率 | 60_feedback/tasks/task_20260727_wangyuyan-rrf-tag-mcp.md |
