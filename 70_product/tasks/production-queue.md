...
| 222 | task_20260802_wangyuyan-global-metadata-p1 | 全局元数据回填P1：高价值卡（framework/domain/system/agent-spec/skill/method/bridge）discoverable_by+title+tags | reviewed | laowantong | ~300张 | #219后 | 60_feedback/tasks/task_20260802_wangyuyan-global-metadata-p1.md | ✅欧阳锋恢复审查PASS(2026-08-03)：7目录76张0失败/0缺失/0双aliases；20文件修改质量合格；frameworks跳过确认；#223可串行启动 <!-- 手动终审：queue_transition被拦+O-3 --> | | ✅#227修复完成(2026-08-03 05:00)：全库YAML 99.4%通过(16张顽固卡黄药师手修中)。恢复方案：串行(#222先→#223后)+目录划分(8高价值目录/6普通目录)+dry-run+git diff。跳过#227已修复的247张frameworks。
| 223 | task_20260802_wangyuyan-global-metadata-aliases | 全局aliases回填：860张卡补中文别名（tools/concepts/dk优先） | queued | hermes | 860张 | 阻塞于#227修复 | 60_feedback/tasks/task_20260802_wangyuyan-global-metadata-aliases.md | 🛑王语嫣紧急停手(2026-08-03 03:45)：**新增根因——#223与#222范围重叠（都含frameworks/），两个实例并行写入同一批文件导致C-10级破坏**（git 2626文件未提交=hermes持续执行中）。hermes立即停手，记录已改清单。恢复条件：①#227修复完成（yaml.safe_load 100%）②#223/#222改串行 ③目录划分避免重叠。欧阳锋暂停+王语嫣并行根因合并 | | 🟡恢复待命(2026-08-03 05:10)：#227修复完成(99.4%)。#223串行在#222之后。范围=6普通目录(tools/concepts/dk/dark-knowledges/cases/其他)+原aliases恢复(git对比7/27)+125双aliases清理。禁止与#222并行。 | ✅#222恢复审查PASS(2026-08-03 06:30 欧阳锋)——#223串行启动放行。hermes按恢复方案执行：6普通目录+aliases合并规则(git恢复7/27+去重)+125双aliases清理+原aliases恢复+dry-run前置。禁止与#222并行。
