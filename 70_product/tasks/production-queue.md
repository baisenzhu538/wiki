...
| 252 | \`task_20260808_wangyuyan-feature-consumption-pilot\` | 消费端协议试点（防死清单机制）：真实任务点菜5-10Feature→测试→复盘→回填周期表JSON→协议v0.1 | claimed-wangyuyan | wangyuyan | 协议v0.1+回填 | #248 reviewed | \`60_feedback/tasks/task_20260808_wangyuyan-feature-consumption-pilot.md\` | 试点已执行(2026-08-08，教练Agent执行)：点菜5 Feature(F045/F057/F078/F079/F087)全PASS→回填周期表JSON(verified 18→22)→协议v0.1沉淀；诊断见 diag_20260808_pilot-252-feature-consumption；**dk-agent-access-kdo-pitfalls 缺 Critique 段(王语嫣补完中)**；待欧阳锋终审 |
...
| 255 | `task_20260808_laowantong-feature-periodic-table-finalize` | 周期表收尾（欧阳锋#248条件项C1-C4）：补4个遮蔽Feature+missing标注+口述对账5候选+C4行号 | claimed-hermes-cli | laowantong | JSON收尾 | 与#249并行 | \`60_feedback/tasks/task_20260808_laowantong-feature-periodic-table-finalize.md\` | 周期表=消费端菜单/工具/agent公共数据源，完整性直接决定下游 |
