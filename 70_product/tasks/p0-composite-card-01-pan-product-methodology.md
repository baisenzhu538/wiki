...
### ✅ 延后项（已完成）
- [x] source_refs 迁移：00_inbox/ → 10_raw/（14 张 PNG → 10_raw/assets/yitang/，5 份口述稿 → 10_raw/sources/）
- [x] 关键原图归档：14 张 PNG 已复制到 10_raw/assets/yitang/

## 验收标准
- [x] kdo lint 0 error（2026-05-11 重跑确认：0 errors, 305 warnings 均为存量）
- [x] 所有 source_refs 指向 10_raw/（7/7 卡已验证，0 残留 00_inbox/ 引用）
...
| 检查项 | 结果 |
|--------|------|
| kdo lint errors | 0（0 errors, 305 warnings 均为存量） |
...
| source_refs | ✅ 全部指向 10_raw/（0 残留 00_inbox/） |
...
| 卡 | type | claims | source_refs | 状态 |
|----|------|--------|-------------|------|
| yt-composite-pan-product-methodology | composite-concept | 15 | ✅ 10_raw/ | A |
| yt-model-pan-product-36-strategies | framework | 11 | ✅ 10_raw/ | A |
| yt-model-pan-product-three-virtues | framework | 9 | ✅ 10_raw/ | A |
| yt-model-pan-product-climbing-map | framework | 13 | ✅ 10_raw/ | A |
| yt-model-pan-product-demand-toolkit | framework | 16 | ✅ 10_raw/ | A |
| yt-model-pan-product-aesthetic-toolkit | framework | 9 | ✅ 10_raw/ | A |
| yt-model-pan-product-execution-toolkit | framework | 18 | ✅ 10_raw/ | A |

kdo lint: **0 errors** | grep 00_inbox: **0 matches**
