...
| 3 | `framework-taste-as-judgment-system` | framework | 品味系统：判断力×审美×系统思维 | taste 五维（判断力、审美、系统思维、方向感、表达/交互）；训练路径；与 AI 生成能力的关系 | 必须与 `concept-AI时代双三角竞争力` 和 `framework-yihang-aesthetic-judgment-training` 互链 |
| 4 | `dk-ai-builder-illusion` | dk | Builder幻觉：用AI做出东西≠完成从0到1 | 现象、根因、反向信号、Action Triggers；与 fast fashion 软件、做出来不是从0到1的关联 | 必须包含至少 3 个真实失败信号 |
...
1. **第一批**：`framework-ai-native-organization-two-modes`
2. **第二批**：`concept-token-capital` + `framework-taste-as-judgment-system`
3. **第三批**：`dk-ai-builder-illusion` + `concept-jevons-paradox-in-ai`
4. **第四批**：`tool-open-closed-problem-classifier` + `case-ai-search-commerce-platform-hedge`
5. **第五批**：`agent-spec-codex-teammate`
6. **第六批**：反向更新 14 张已有卡片 related
...
4. 所有数据标注 `[确认]`/`[假设]`/`[空白]`；来源不明数据不得作为强证据。
