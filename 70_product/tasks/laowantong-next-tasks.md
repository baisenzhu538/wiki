...
> **此文件不再维护。** 所有生产任务已迁移至 `production-queue.md`。
> 本文件保留为历史档案（最后更新 2026-06-16），仅供参考，**不作为任务领取依据**。
> 老顽童启动后读 `production-queue.md`，不要读这个文件。
...
**生效时间**：2026-06-18  
**依据**：`60_feedback/diagnosis/wangyuyan-evaluation-of-huangyaoshi-laowantong-feedback-2026-06-18.md`

### 1. 精修分级：格式精修 vs 内容精修

| 类型 | 节奏 | 目标 | 验收标准 |
|:---|:---|:---|:---|
| **格式精修** | 30 张/批 | 补齐 metadata、status、source_refs、related、diagnostic_signals | 全库 P0=0，无 dangling 链接，status→enriched 或 diagnostic |
| **内容精修** | 5 张/批 | 正文深度提升：新增 case、新增可调用模板、提炼跨域模式 | 按 A/B/C 分级验收，B 级以上才能 status→enriched |

**内容精修 A/B/C 分级**（王语嫣验收用）：
- **A**：新增 ≥1 个独立 case / 或新增 ≥1 张可调用模板 / 或提炼 ≥1 条跨域模式。
- **B**：正文结构完整，有具体清单/表格/数字，但无新增 case 或模式。
- **C**：仅补全 metadata 和 related，正文内容未实质性加深 → 返回重修或 status→diagnostic。

### 2. dk 卡产出要求

- 每完成一批 30 张精修，必须产出 **1-2 张独立 dk 卡**，把精修中发现的暗知识从“散落在 30 张卡中的模式”固化为单张可调用卡。
- 优先建卡方向：跨域共同模式、工具失效根因、判断边界。

### 3. 跨域模式提取要求

- **每完成两个域的精修**，必须做一次跨域对比，产出 ≥1 条跨域共同模式。
- 跨域模式必须独立建卡（dk 类型），并标注 `bridges_to` 指向至少两个域的代表卡。
- 示例主题：
  - “把 AI/工具当成一次性生成器，而非分阶段校验器”（精益/ToB/短剧/建模/笔记）
  - “模型是提问的脚手架，不是答案”（建模/精益/ToB）

### 4. draft 分级处理

- 高价值 draft（conf≥0.7，related 非空）：按主题分批深度精修。
- 中间层 draft：快速格式精修或降级为 diagnostic。
- 低价值 draft（conf<0.5 或相关为空）：配合黄药师的批量审计脚本，降级为 diagnostic 或归档到 `_archive`。

### 5. source_refs 与 confidence 规则

- source_refs 必须指向 `10_raw/sources/` 下真实存在的相对路径；无法追溯时置空。
- 单来源或来源为空时，confidence≤0.89；≥2 来源时可到 0.90-0.93。
- reviewed_by 固定为 `欧阳锋`，且不能与 author 相同。
...
- [x] 批次 1：业务/医药/建模（7 张）
- [x] 批次 2：AI 协作技能（6 张）
- [x] 批次 3：Master 系统暗知识 F/P（8 张）
- [x] 批次 4：Master 系统暗知识 C/F/P（9 张）
- [x] 全库门禁复核 + 写小结
