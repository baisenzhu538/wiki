...
| LW-PL-006 | C 域（业务公式）深挖重建 + 反向蒸馏自有 agent | 老朱 2026-07-11：「孔源分享只是冰山一角，主域内容还没进来」；洪七公 OCR+VLM 进行中；**07-12 将到「业务公式 YAI」素材** | 两步：① 建域深度广度（总纲升级+L1-L6 参数冰山深化+工具族+案例族，按 #150 同规格；#145 版作底稿不废弃）② **反向蒸馏自有「业务公式 agent」**（同 D 域蒸馏管线） | P1 | **素材处理中（洪七公 VLM 进行中）** | 主域素材：`00_inbox/Handle the business/Business formula`（101 张一堂DOC新图 + `run_vlm_business.py`，2026-07-12 凌晨入库，VLM 输出待落盘）；现有底稿：孔源口述+4图+#145 七卡（冰山一角，不废弃）；**VLM 完成后王语嫣 9 层深挖+6层交叉验证+编排** |
