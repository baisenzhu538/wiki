...
def process(image_path: str, prompt: str = "", save: bool = True) -> dict:
    ...
    if not MINIMAX_API_KEY:
        raise RuntimeError("MINIMAX_API_KEY 未配置，请检查 cap_hub/config.py")
    ...
    response = client.messages.create(
        ...
        messages=[{
            "role": "user",
            "content": [
                {"type": "image", "source": {"type": "base64", "media_type": mt, "data": base64_image}},
    ...
    if save:
        ...
        output_path.write_text(f"# VLM 识别结果：{path.name}\n\n{content}", encoding="utf-8")

    return {"path": str(path), "content": content, "model": MINIMAX_MODEL, "saved": save}
