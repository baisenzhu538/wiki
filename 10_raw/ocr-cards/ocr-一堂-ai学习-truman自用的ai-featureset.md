...
> 🪧 指路：本卡为 Truman 自用精简版 FeatureSet（30+ 项）。完整 Feature 周期表（100 项，L0-L5 分层）见框架卡 `30_wiki/frameworks/framework-truman-feature-layered-system.md`；数据源 `10_raw/sources/feature-periodic-table-v1.0.json`；点菜式查询 `kdo-tools/feature_menu.py`。（#526 入口路标批）
# OCR: 一堂-AI学习-truman自用的AI FeatureSet
