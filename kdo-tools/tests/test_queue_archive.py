...
def _make_env(tmp: Path, monkeypatch):
    ...
    queue.write_text(
        ...
        "- ~~#1 task_old_reviewed｜hermes｜提审 06-01 10:00｜t.md~~ → 已终审 PASS A-（2026-06-05 欧阳锋）\n"
        "- ~~#5 task_recent_strike｜hermes｜提审 08-20 10:00｜t.md~~ → 已终审 PASS A（2026-08-21 欧阳锋）\n"
        "<!-- REVIEW-PENDING-END -->\n",
        encoding="utf-8")
    task_dir = tmp / "tasks"
    task_dir.mkdir()
...
def test_archive_only_old_reviewed(tmp_path, monkeypatch):
    ...
    assert "task_recent_strike" in text             # 近 30 天划掉行保留

    af = archive / "production-queue-2026-08.md"
    assert af.exists()
    a_text = af.read_text(encoding="utf-8")
