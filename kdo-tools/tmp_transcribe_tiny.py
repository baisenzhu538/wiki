...
MODEL_DIR = r"C:\Users\Administrator\wechat-collect\models\faster-whisper-tiny"
...
for f in files:
    ...
    lines = [f"# {vid} 视频逐字稿", "",
             f"> 转写工具：faster-whisper-tiny | 语言：{info.language} | 时长：{int(info.duration//60)}分{int(info.duration%60)}秒", "",
             "---", ""]
    for seg in segments:
