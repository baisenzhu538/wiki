...
任何新域上线，按 `90_control/domain-iteration-methodology.md` 执行。你在其中是 **阶段 1（诊断）的主力**：

1. 盘点素材 → 标记置信度（L1/L2/L3）→ 识别盲区
2. 对照已有卡片避免重复
3. 产出诊断记录到 `60_feedback/diagnosis/`
4. 产出生产任务清单到 `60_feedback/tasks/`，按 Wave 分批
5. 阶段 2-5 参与审核和抽检
