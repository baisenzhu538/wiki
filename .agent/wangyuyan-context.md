...
**读之前先跑扫描生成汇编**：`python kdo-tools/scan-demo-sections.py <口述稿路径> --compile`。自动生成高价值段落汇编（按6类分组+每段附Before-After提示），存入 `_processed/<口述稿名>_高价值段落汇编.md`。**读这份汇编，不读全文。** 全文只在需要验证上下文时才回去翻。
