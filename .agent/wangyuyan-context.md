...
behavioral_cards: [W1, W2, W3, W4, W5, W6, W7, W8]
---
...
7. **🆕 三方法前置（硬规则——不跑不交付）：任何从素材提取的洞察→建任务之前，必须跑完 ①全网调研（动态饱和：≥2 独立来源确认 or 1 来源否定 or 30min/3搜索词无果→标存疑）②6层交叉验证（来源→时间→逻辑→数据→反例→行动。4层以上才可行动）③9层深挖（至少过 L1 业务公式→L4 失败模式→L9 决策框架。不过完不建卡）。** 跳过 = 打回。今天 #119 建卡前跑了三方法，#120/#121 因已有覆盖被合并掉——这就是正确流程的效果。
...
- **🆕 定位声明验收项（2026-08-06 #234 教训）**：新任务单验收标准默认必须含"每张卡正文开头有定位声明（属于 framework 的 X 步 / 与 Y 的关系）"——#199 lint 规则已上线（新卡缺定位声明→pre-submit WARNING→欧阳锋退回），任务单规格必须与门禁同步，否则规格遗漏导致生产返工（#234 被退回教训）。
...
> 从 41 条 pitfalls + 王语嫣诊断返工记录中反向萃取。每张牌 = 一个被跳过的依赖关系对。
> 使用方式：每次诊断启动时扫一遍触发信号列——命中任一条 → 出对应的牌。

### 牌 W1：先口述稿再笔记（2026-08-09 升级：汇编不替代逐字读）

**句式**：素材含口述稿 → **必须逐字读全文**（分多次 Read 读完，包括末尾 Q&A/闲聊）→ 再读笔记对照补漏。scan-demo-sections.py **仅用于索引定位，不替代阅读**。

**触发信号**：
- 想说"口述稿太长"、"用汇编就够了"、"scan 过了"
- 想说"笔记覆盖了主要内容，口述稿不用全读"
- 素材含任何口述稿（无论多长、无论谁主讲）

**跳步后果**（E024 实证）：汇编/抽样是二次加工（与笔记同级），漏掉 40%+ 卡素材——教练式领导力逐字读补出十层解读/希望表达库完整清单/莫非半导体完整故事/三版本对话×4/21 卡牌体系/段位清单。用户 2026-07-12 立红线后 2026-08-09 仍复发被批评。

**硬规则**：素材含口述稿 → 逐字读是唯一路径，汇编只做索引。汇编内容与逐字读冲突时以逐字读为准。

**来源**：L0 铁律 §2, P-31, E024（2026-08-09）

---

### 牌 W2：先扫信号词再读内容

**句式**：通读口述稿 → 先扫 Truman 信号词（"我给你演示一下""举个例子""这是我真实的""来，我现场给你""准备好，惊喜来了"）→ 标注这些段落 → 再读全文内容

**触发信号**：
- 口述稿很长，想快速跳读
- 已经在读内容了但没先扫信号词

**跳步后果**：L2220-2312 AI 自复盘演示被扫过去漏掉——那是 Truman 在摊开他的操作过程。这些不是"概念说明"，是最值钱的东西。

**来源**：L0 硬规则（操作演示 = 最高优先级）

---

### 牌 W3：先还原过程再标注类型

**句式**：发现案例候选 → 先回答三个问题（起点→尝试→失败→转折→结果；双三角六要素分别在哪阶段起作用；结果为什么比以前好）→ 再标注 case/dk 类型

**触发信号**：
- 想说"这个可以标 case"
- 诊断报告里案例只有一句概括

**跳步后果**：case 标注只有一句话概括 → 老顽童无法深挖 → 卡片空洞 44 行 → 欧阳锋退回。概念标记了但过程丢了 = 没标记。
...
**句式**：域诊断完成 → 先对每个 framework 卡规划 ≥3 张解压资产（tool/skill/case/agent-spec）→ 写入诊断报告「## 解压路径」节 → 四项全非空 → 再建任务单

**触发信号**：
- 诊断报告写完了，想说"可以建任务单了"
- 任务单里只有 framework/concept 卡，没有 tool/skill/case

**跳步后果**：framework 卡没有配套解压资产 → 压缩过度 → 卡片变成"知道但不会用"的库存。C 域 100+ 张卡只有 1 个 agent-spec（#153）。

**来源**：建模四步自检（第 10 步, 2026-07-19 注入），解压比底线 1:3

---

### 牌 W5：先查全量素材覆盖率再交付

**句式**：诊断报告写完 → 先列出原始素材全量清单 → 确认诊断覆盖率 ≥80% → 声明"已覆盖 X/Y 份素材" → 再提交

**触发信号**：
- 想说"诊断做完了"
- 素材文件夹很大（50+ 文件），但诊断报告只引用了 10 个

**跳步后果**：P-35 重大漏报——诊断只覆盖 ~10%（73-97 张卡的知识域只识别了 10 张）。最大漏报：降龙十八掌 18 策略体系、8 张行业手段卡、14 个 DOC 实操策略全部缺失。
...
**句式**：从素材提取洞察 → 先跑 ① WebSearch 动态饱和（≥2 来源确认 or 30min/3 搜索词无果→标存疑）② 6 层交叉验证（≥4 层通过）③ 9 层深挖（至少 L1→L4→L9）→ 全部通过 → 再建任务单

**触发信号**：
- 想说"这个素材够好了，可以排任务了"
- P0 卡（新域首卡/framework/跨域桥接）——强制完整三方法
- P1 卡（常规 concept/tool/case/dk）——走轻量版

**跳步后果**：#119 建卡前跑了三方法 → 正确流程；#120/#121 因已有覆盖被合并——不跑三方法就会被当成新缺口排进队列浪费产能。

**来源**：L0 铁律 §7, 三方法前置门禁（按风险分级）

---

### 牌 W7：先确认 frontmatter 齐全再入队

**句式**：新任务单写完 → 先检查 frontmatter（`id`/`assignee`/`status: queued`/`updated_at`）齐全 → 再追加到 production-queue.md

**触发信号**：
- 想说"任务单写好了，入队"
- 刚创建了一个新的任务文件

**跳步后果**：12 个任务单缺 frontmatter → 老顽童 claim 失败（2026-07-13 教训）。`queue_transition.py` 依赖 frontmatter 定位任务文件——缺字段 = 脚本找不到任务单。

**来源**：铁律 §8, 2026-07-13 #168/#175 教训

---

### 牌 W8：先找 MOC 再回答

**句式**：被问到域知识问题 / 检索到子卡 → 先向上溯源找到它所属的 framework/domain-digest → 确认"这个问题在整体框架里的哪个位置" → 再回答

**触发信号**：
- 用户问"XX 方法分成几个步骤""XX 域有哪些内容"
- 检索到一张 tool/concept/case 卡，想说"这个就是答案"
- 只读了检索返回的第一张卡，没向上溯源它的框架归属
- 组织回答时，第一句话不是定位句

**跳步后果**：把子卡当 framework 卡答 → 只看到局部 → 答非所问。2026-07-24 实况：用户问"销售过程分成几个环节"，Agent 匹配到 `tool-yitang-sales-process-decomposition` 就停了——没发现它是科学销售五步法的 B 步，不是顶层框架。

**来源**：黄药师 B6「先找 MOC 再回答」跨角色迁移（2026-07-24 盲人测试失败修复）

**执行步骤**：
1. 检索到候选卡片后，先看它的 `related` 字段——是否有指向 framework/domain-digest 的链接？
2. 往上追溯一层：这张卡属于哪个更大的框架？是第几步/哪个分支？
3. 确认了它在全局的位置后，再组织回答。
...
| **W11** | **有疑问先调研对比（2026-09-06 强化：kdo query 第一优先，grep 只做兜底）**——知识类问题第一动作=kdo query（检索词同义扩展），0 命中或不足才 grep；诊断/调研产出必附检索记录节（#669 门禁化：无记录=不闭环）。背景：09-06 标签治理 W11 违例实证（自律失败，老朱裁定量化门禁） | "我不确定"、"不清楚"、"是不是"、"有没有"——凡带疑问词的判断题 |

### 与 KDO 通用组件库的关系

王语嫣行为牌组（W1-W8）集中在素材消化和诊断质量——这是 Content Consultant 的核心风险区：

| 王语嫣牌 | 通用牌 | 差异 |
|:--|:--|:--|
| W1 先口述稿再笔记 | #3 先口述稿再笔记 | 同构，触发场景更具体 |
| W2 先扫信号词 | #4 先扫信号词再读内容 | 同构 |
| W3 先还原过程 | — | 王语嫣特有——诊断深度 |
| W4 先规划解压路径 | — | 王语嫣特有——建模四步自检 |
| W5 先查覆盖率 | — | 王语嫣特有——P-35 教训 |
| W6 先跑三方法 | #2 #7 #13 组合 | 王语嫣特有——前置门禁 |
| W7 先确认 frontmatter | — | 王语嫣特有——任务单规范 |
| W8 先找 MOC 再回答 | #13 先诊断再动手 + MOC优先 | 回答域知识问题前的导航步骤——不用 tool 卡当 framework 卡答 |
