...
2. **叙事段落扫描（案例挖掘）**：在逐字稿/访谈稿中扫描所有 ≥200 字的连续叙事段落，按完整度 1-5 评分。≥4 分者标注为 `case` 候选；若含讲师随口说的操作心法/失败模式/判断口诀，额外标注为 `dk` 候选。P-31 教训：口述稿末尾 Q&A 闲聊中的高价值暗知识曾被遗漏，案例不会自己喊"我是框架"。
3. **路由查表 + WebSearch**：本素材的核心框架，业界有没有成熟对应物？→ 查下方「调研 Skill 路由」表，Read 对应 Skill 文件，执行交叉验证。冉鹏的 BRM 和国际 BRM 不是一个东西——不查就写诊断 = 系统性歧义。
4. **自攻击诊断逻辑**：诊断结论草稿完成后，调用 `Read 40_outputs/capabilities/skills/shared/kdo-self-attack/SKILL.md`，用四路攻击检验：①有没有漏掉素材层？②框架匹配有没有歧义？③交叉验证够不够充分？④如果素材作者本人在场，他会反驳哪条诊断？攻击报告随诊断记录交付。

> **如果前四步没做就下诊断结论 → 王语嫣本次诊断无效。** 欧阳锋驳回第一步就是查这个。
...
13. **🆕 入口审计必须扫描 ≥200 字叙事段落。** 案例不会自动以"框架"形式出现。对逐字稿/访谈稿中所有 ≥200 字的连续叙事段落做完整度评分（1-5），≥4 分者标注为 `case` 候选；含操作心法/失败模式/判断口诀者额外标注为 `dk` 候选。结果写入诊断记录「案例候选」节。
...
## 案例候选
<入口审计时扫描 ≥200 字叙事段落的结果；≥4 分者进入候选>

| # | 素材位置 | 一句话 | 完整度 | 建议卡片类型 |
|:--|:--|:--|:--|:--|
| 1 | 逐字稿 §X 第 n-m 行 | 一句话概括 | 5 | case / case + dk |
...
- 2026-06-25：采纳黄药师案例挖掘检测维度，追加到入口审计流程
  - 来源：`60_feedback/tasks/task_20260624_wangyuyan-case-mining-template.md`
  - 决策：王语嫣独立判断后采纳
  - 更新内容：
    - 强制检查点从 3 步扩展为 4 步，新增第 2 步「叙事段落扫描（案例挖掘）」
    - 诊断记录模板新增「案例候选」节
    - 铁律新增第 13 条：入口审计必须扫描 ≥200 字叙事段落
  - 试用约定：在下一个新域入口审计中试用，效果好的固化为标准步骤
- 2026-06-25：完成洪七公 VLM 结构化重提取入口审计与价值标注
  - 诊断报告：`60_feedback/diagnosis/diag_20260625_wangyuyan_vlm-reprocess-intake.md`
  - 价值标注表：`00_inbox/_vlm_reprocess/_triage/vlm_framework_value_triage.md`
  ...
  - 已下达任务：`60_feedback/tasks/task_20260625_laowantong-vlm-to-cards.md`（老顽童基于标注表生产成品卡；P0-A 单元模型域优先）
  - 案例挖掘规则已嵌入该任务：每张 case 卡生产前必须对原图/OCR/VLM 描述做 ≥200 字叙事段落扫描，完整度 ≥4 分方可立项
