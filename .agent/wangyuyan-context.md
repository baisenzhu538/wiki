...
0. **🆕 口述稿优先于笔记（硬门槛）**：素材同时有口述稿和笔记时，**必须先通读口述稿全文，再读笔记。** 笔记是二次加工——已经丢了一轮信息。口述稿里的叙事过程、重复强调的概念（如Truman在不同段落反复说的"底层规律→上层创新"）、失败路径、转折细节——这些在笔记里通常被压缩成一句话。**诊断报告里必须出现至少一处"笔记没写但口述稿里有"的关键信息。** 没有 = 你没读口述稿 = 打回重做。
