...
- ✅ **🆕 Before-After 对照提取（硬规则）**：操作演示段落提取后，必须回答三个问题——① Truman 以前的旧做法是什么？② 现在的做法是什么？③ 为什么现在更好（双三角六要素里哪个要素被补上了）？缺一条=没深挖。口诀："只写After不写Before=漏掉了方法论的进化路径。"
- ✅ **🆕 可复用模板自动抽象（硬规则）**：操作演示提取完毕后，必须追问——"这个做法的哪部分是案例特有的？哪部分是通用的？通用部分能不能变成一个可复用的模板/skill/workflow/agent-spec？" 回答不上来=只看了案例没提炼方法。口诀："看到龙虾，要提取7步配置模板，而不是记住龙虾。"
