...
执行中遇到任何摩擦/误判/工具问题 → **当场 append 一行**到 `桌面/agent复盘/laowantong/friction-log.md`（一行式：`YYYY-MM-DD HH:MM｜场景｜问题一句话｜（可选）建议方向`）。写建议书是加分项不是必选项，**friction 一行是必选项**——探针自动上浮给王语嫣（[friction] 线索登记）。
