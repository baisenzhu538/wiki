...
## P-37: 批量删除空 section 消除 copy-paste——body 过短 WARNING 激增

**症状**：为清理 700+ 个"常见失败模式" section 的 copy-paste WARNING，批量删除空 section 后，`kdo lint` body 过短 WARNING 从 80 激增至 651，总 WARNING 不降反升。

**根因**：
1. 空 section 虽然不产生有效内容，但增加了 body 长度，掩盖了卡片本身内容不足的问题
2. 删除操作前未评估对 body 长度检查项的影响
3. 把"消除一类 WARNING"当成目标，而不是"提升卡片质量"

**对策**：
- 不要通过删除 section 来消除 copy-paste；要么重写具体内容，要么保留空 section 作为占位并在后续回填
- 批量修改前声明"预期影响的检查项"，修改后重新全量 lint 验证，确认没有转移债务
- copy-paste 的根治方法是"针对具体方法重写"，不是"让 section 消失"

**关联**：P-29（批量脚本覆盖非空值）、P-30（批量操作未声明预期变更范围）

---

## P-38: 任务单假设的 `kdo lint --domain` 实际不存在（已修复）

**症状**：#28 任务单要求"每批独立跑 `kdo lint --domain <name>`"，但当时 `kdo lint --help` 中没有 `--domain` 选项，导致无法按 domain 直接过滤 WARNING。

**根因**：
1. 任务单编写时假设了尚未实现或已被移除的 CLI 功能
2. 没有先验证工具可用性再写执行规则

**修复**：欧阳锋在 `kdo/cli.py` 和 `kdo/commands/system.py` 中新增了 `--domain` 与 `--summary` 参数，`kdo lint --domain <domain> --summary` 现在可直接按 `30_wiki/<domain>/` 路径前缀或 frontmatter `domain` 字段过滤 WARNING。

**遗留对策**：
- 写任务单的执行规则前，先实际运行 `kdo lint --help` 确认可用选项
- 若工具不支持，要么实现该功能，要么改用替代方案并更新任务单
- 任务单应区分"理想流程"和"当前可执行流程"

**关联**：P-28（不查公告就调参——同类：不查工具能力就写流程）

---

## P-39: 审查者越界修改生产代码/卡片内容

**症状**：审查过程中发现 `kdo batch` 会丢失用户未提交改动、`kdo lint --domain` 不存在、`kdo freeze` 会破坏换行，审查者直接修改了 KDO CLI 源码或替生产者重写卡片 section。

**根因**：
1. 审查者急于让任务通过，混淆了「验收」与「执行」角色
2. 没有先把问题退回给生产者或升级为独立任务
3. 对「时间压力」过度反应，默认「我来改更快」

**对策**：
- 审查者发现问题后，首选动作是「记录 + 退回」或「升级独立任务」
- 仅当问题属于纯格式/索引类且已明确授权时，审查者才可做最小修正（如补 index、改 section 标题拼写）
- 基础设施 bug 必须由黄药师/老顽童作为独立任务处理，审查者只做验收
- 审查记录中必须区分「生产者交付」和「审查者修正」

**关联**：P-28（不查公告就调参）、P-38（任务单假设不存在命令）

---

## P-40: 审查测试污染工作区

**症状**：审查 `kdo snapshot/revert/batch` 时，测试产生了多个 snapshot tag 和 stash；`git stash pop` 误恢复了不相关的旧修改；临时测试文件残留在工作区。

**根因**：
1. 测试破坏性命令前没有先 `kdo snapshot`
2. 测试后没有立即 `git status` / `git tag -l` / `git stash list` 检查残留
3. 对 `git reset --hard` 和 `git stash` 的副作用不熟悉

**对策**：
- 测试回滚/批量命令前：先 `kdo snapshot`
- 测试后：检查 `git status --short`、`git tag -l snapshot-*`、`git stash list`
- 产生的不相关 tag/stash/临时文件立即清理
- 测试文件尽量用 `tmp-` 前缀并删除，或放在 `/tmp`
...
**症状**：生产者报告 copy-paste WARNING 从 76 降到 0，但 body-too-short WARNING 同时从 80 激增到 651。审查者最初只关注总 WARNING 数下降，没意识到这是把一类问题批量转成另一类问题。

**根因**：
1. 过度关注「净减数量」这个单一指标
2. 没有对比各类型 WARNING 的结构性变化
3. 对「批量删除空 section」的副作用缺乏警惕

**对策**：
- 审查时不仅看总数，还要看分类变化表
- 对「A↓ 同时 B↑」的结构保持敏感，要求生产者解释
- 明确禁止「通过删除 section 消除 copy-paste」，改为重写具体内容
- 要求批量修改任务声明「预期影响的检查项」，修改后对比验证

**关联**：P-37（body 过短债务转移）


## P-42: Agent 凭错误核查挑战审查结论——git 字节验证缺位（#224 争议）

**症状**：欧阳锋判 hermes 写入模板缺陷→王语嫣核查称"7/27已坏"→欧阳锋 git 字节验证推翻→证明王语嫣核查方法有误（错误路径/提交/解码）

**根因**：
1. 无标准事实核查协议——Agent 凭记忆/不完整验证做判断，结论自信但方法不可靠
2. 核查结果未附可复现命令——对方无法验证核查方法是否正确
3. 无"证据效力层级"——git 字节 > 审查报告 > Agent 记忆，这个层级未被明确约定

**对策**：
- 任何"文件在某时间点是什么状态"的断言，必须附 git show 命令+输出
- 跨 Agent 事实争议：双方各自跑 git show + yaml.safe_load，贴完整输出
- 不确定时说"需要第三方 git 字节验证"，不硬撑
- Agent 记忆不可靠——git 字节是唯一真相源

**关联**：P-15（声称完成但实际未做）、#224 终审分歧

## P-43: 可机械修复的 pre-submit WARNING 当背景噪音照提——「警告债」拖过终审变扣分（#582）

**症状**：#582 阿蕊 4 卡提审时 pre-submit 明示 10 条 WARNING，其中 5 条机械可修（case 卡定位声明一行、3 张卡 aliases 各补一词、框架卡 pos/tacit 段），生产者原样照抄进执行报告当「口径」。欧阳锋终审扣 A-（-0.5），修复项跨日留 TODO，老朱质疑提审速度触发第二实例全程复核——一条 5 分钟可清偿的警告债，实际成本=降档+两实例复核+跨日延期。

**根因**：
1. 心理账户错位：把 WARNING 归类为「已知悉的噪音」而非「未清的债」，检查器给了逐条修复命令（"Add these to the aliases field"）却没执行——复制粘贴级动作被跳过
2. 修复范围锚定错误：把终审扣分点清单当修复范围（扣什么修什么），而不是把检查器全量输出当修复范围（警什么清什么）——终审只点名 entity/case 的 aliases，framework/method 同款警告就漏了（实测 4 张全缺）
3. 「赶提审节奏」的隐性激励：提审快被当成绩，停手清警告显得慢

**对策**：
- 提交前 pre-submit 输出逐条过：每条 warning 二选一「修掉」或「写明不可修原因」，禁止原样照抄
- 修复范围=检查器全量输出，非终审摘要；同类警告一次清类（修类不修点）
- 时间成本对照：机械修复（分钟级）≪ 终审降档+复核消耗（小时级+信任成本）
