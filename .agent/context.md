...
updated: 2026-06-22
active_branch: main
active_task: "黄药师：路由体系加固已完成（context 文件 + 强制检查点）。下一项待命。"
blockers:
  - "🔴 CLI 王语嫣未上线 — 阻塞冉鹏 PPT 视觉层重标、老顽童 5 张 tool 卡 v2、战略域 Wave 9"
  - "🔴 老顽童待命 — 等王语嫣完成 PPT 视觉层标记后启动 `task_20260621_战略域PPT补强_黄药师标杆.md`"
  - "🟡 飞书王语嫣首轮误判冉鹏域 — 以知识点摘要为边界，漏逐字稿战术层(21张)+PPT视觉层(10张)。根因：工具链限制(飞书Hermes无批量文件扫描能力)+未做全量素材目录"
  - "🟡 113 张 VLM parse error (P-33) — 内层 JSON 高质量可用，需脚本修复。待黄药师排期"
...
next_session_hint: "CLI 王语嫣上线 → 读 wangyuyan-context.md → 走强制检查点 → 重标冉鹏 PPT 视觉层 → 产出诊断记录 → 老顽童启动 5 张 v2 tool 卡。黄药师待命：P-33 parse error 修复 + shared/README 更新 + Y 模型决策域 Skill 化。"
---
...
- **老顽童**：待命 ⏸️，等王语嫣完成战略域 PPT 视觉层标记后启动 5 张 tool 卡 v2 补强
