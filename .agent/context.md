...
blockers:
  ...
  - "🟡 Hermes 老顽童 CLI thinking 400 复发（2026-06-30）：Kimi 再次报 `invalid thinking: only type=enabled is allowed for this model`；真正根因：Windows 版 Hermes 实际安装在 `C:\\Users\\Administrator\\AppData\\Local\\hermes\\hermes-agent/`，而之前的防御性补丁只打到了 WSL 副本；当 model=kimi-for-coding 时仍进入 legacy thinking 分支发送 `thinking: {type: enabled, budget_tokens: N}`；已给 Windows 版 `agent/anthropic_adapter.py` 添加 `kimi-*` 跳过逻辑，并彻底删除 `C:\\Users\\Administrator\\.hermes/config.yaml` 和 `profiles/laowantong/config.yaml` 中所有 `reasoning_effort` 字段；清理了 `__pycache__`；需用户重启 hermes CLI 后验证"
