...
### 各角色当前任务
- **老顽童**：#10 单元模型域编译 + VA 修复（主线）。OCR Batch 4 等洪七公 VA 前置。
- **黄药师**：Dogfood 完成。10 项待办（P0→P1→P2），当前 P0 = produce 自动预填。
- **洪七公**：VA 前置 A1（前5张🔴卡 VA 完成，待抽检）+ 文章重启（≥3篇）。
- **段王爷**：Video ship 交付记录补全。

### KDO CLI 状态
- 47 .py 文件，~13,500 行，15 测试文件
- **pytest**：379/379 passing
