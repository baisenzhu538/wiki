...
blockers:
  ...
  - "🟡 Hermes 老顽童 CLI thinking 400 复发（2026-06-30）：Kimi 再次报 `invalid thinking: only type=enabled is allowed for this model`；根因：`reasoning_effort` 在 `~/.hermes/config.yaml` 和 `~/.hermes/profiles/laowantong/config.yaml` 中仍残留为空字符串，触发 legacy thinking 分支向 Kimi 发送 `thinking: {type: enabled, budget_tokens: N}`；已完全删除所有 reasoning_* 字段并清理 `__pycache__`；需用户重启 hermes CLI 后验证"
