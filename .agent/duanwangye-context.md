...
0. **🆕 更新技能进化日志** — 追加一行到 `桌面/agent复盘/duanwangye/技能进化日志.md`：
...
2. **写 Truman 10章复盘** — 用 Write 工具写到 `桌面/agent复盘/duanwangye/daily-context/YYYY-MM-DD.md`（格式见 agent-os.md §10.2，10章缺一不可）
3. **保存+自检** — 一条命令搞定：
   ```
