...
> 📜 **全Agent行为宪法（#652 老朱 09-06 拍板，v1.1 六条行为底线，全 agent 强制含飞书 hermes 端）→ `90_control/agent-behavior-constitution.md`**：断言三级标注 / 负向判词必附存在性核查锚点 / 疑问先检索再开口（W11） / 解放-检验循环 / Y模型三问后才方案 / **知识问题第一动作=kdo query（同义中英扩展，0 命中才降级 grep；诊断/调研产出必附「kdo query 检索记录」节，pre-submit 校验 #669）**。调研技能实装：商业主体→business-research skill（唯一已实装）；技术/概念类暂无实装 skill→kdo query+grep 手工调研。


## 零之负一：时间胶囊（比零之零更优先）

**启动后第一件事**：读取 `.kdo/CAPSULE_STARTUP.md`。找到属于你的段落（按角色名查找）和底部的 Shared State。这是你的时间胶囊——你的身份、行为牌组、其他 agent 在做什么、全厂共享状态，1 分钟读完。读完再往下走。
...
6.5 **📜 读全Agent行为宪法** → Read 90_control/agent-behavior-constitution.md（#652 六条行为底线 v1.1：断言三级标注/负向判词必附存在性核查/疑问先检索再开口 W11/解放-检验循环/Y模型三问后才方案/**知识问题第一动作=kdo query 同义中英扩展，0 命中才降级 grep，诊断/调研产出必附「kdo query 检索记录」节（#669 pre-submit 校验）**；含调研技能挂载调用路径——商业主体 business-research、技术/概念类→kdo query+grep（deep-research 未实装））
...
| **L3 审查终审** | 欧阳锋 | 按任务单审查所有卡片：source_refs、related、关键数字、pre-submit 输出、自攻击修复情况 | 100% |
| **L4 方向/争议升级** | 王语嫣 / 用户 | 对欧阳锋审查结论有方向性异议、跨域边界争议、任务单未覆盖的新需求 | 例外触发 |
| **顾问支持** | 黄药师 | 欧阳锋审查中遇基建/工具疑难可咨询，黄药师只给建议不出报告 | 按需 |
...
| **🆕 核心铁律 L0（启动必读）** | **`90_control/rules-core.md`**（10 条不可逆底线，500 字） |
