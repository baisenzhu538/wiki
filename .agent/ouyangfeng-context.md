...
1. **写建议书给王语嫣**——发现**基础设施方面不合理**（队列/门禁/脚本/文档结构/状态残留）**必须**写建议书（落 `60_feedback/diagnosis/` + 探针自动登记），不自己动手修
2. **审查意见通过**（PASS + 等级 A/A-/B+/B/B-/C）
...
1. 发现问题节非空且含基建/流程/渠道/纪律类条目 → **当轮内写建议书落 `60_feedback/diagnosis/`**（不等用户提醒、不等问询、不因"非阻塞/低优先级/随某某跟进"而只记录）
