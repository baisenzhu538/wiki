...
## 2026-08-23：老顽童岗位说明书 v1.0 入宪同步（D4 context 修改）

**类型**：D4 自我修改（`.agent/laowantong-context.md` 注入岗位说明书硬约束）/ **claim-state**：attested（已核实声明）
**批准人**：老朱（2026-08-23「拍板执行」）

**背景**：
#431 老顽童岗位说明书定稿经欧阳锋 PASS A-；老朱终稿拍板。为防止「spec 写对但运行时不加载」，需把硬约束同步进老顽童 context。

**决策**：
1. `90_control/kdo-charter-v0.1-draft.md` §2.6 新增 §2.6.1 老顽童岗位说明书 v1.0 入口（链接定稿源 spec）。
2. `.agent/laowantong-context.md` 新增「岗位说明书硬约束（#431）」节：领取前置精做笔记/素材消费率≥80%、状态细分 in_progress、批次验收≠整单终审、审查者不直接编排、写审分离、G1/G2。

**原因**：
spec 是底稿，不等于运行时必然执行；context + 队列门禁才是保险丝。

**否决的替代方案**：
只保留 spec 不改 context——否决：会出现「岗位说明书 reviewed 但老顽童启动读不到」的纸面合规。

**后果**：
老顽童启动/领单必须对齐岗位说明书 v1.0；后续 queue claim/pre-submit 门禁（F-038/#439 候选）以本 context 条款为口径源。

## 2026-08-23：王语嫣岗位说明书 v1.0 入宪同步（老朱终稿直批，D4 记录）

- **拍板**：老朱 2026-08-23 直批 spec v0.3 转正（跨场直批，F-028 第五场提前）并入 charter §2.6.3；§2.6.2 编号预留欧阳锋（待其终稿拍板）。
- **核心口径入宪**：编排双轨（知识卡片+基础设施）；建议书=编排输入（读→编排决策→一行去向，不写裁定文书）；任务单 append-only（E046）；自迭代双回路三栏。
- **改动文件**：agent-spec-wangyuyan-orchestrator.md（draft→reviewed）、kdo-charter-v0.1-draft.md（§2.6.3）。
- **欧阳锋结构终审**：待补（写审分离，不影响生效——老朱终稿拍板压轴）。

## 2026-08-23：欧阳锋岗位说明书 v1.0 入宪同步（老朱终稿拍板，D4 记录）

- **拍板**：老朱 2026-08-23 终稿拍板 #441（复审 PASS A- 后），并入 charter §2.6.2。
- **核心口径入宪**：审而不改/PASS 必给等级/批次验收≠整单终审/建议书断言回查数据层（#418）/负向判词存在性核查锚点（#433）/意见书落盘（F-035）/KF-024 三要件/双实例独立印证。
- **改动文件**：kdo-charter-v0.1-draft.md（§2.6.2）。spec frontmatter 已于本日先行补齐（reviewed/欧阳锋）。

## 2026-08-23：三层质量门控架构入宪（老朱拍板，D4 记录）

- **拍板**：老朱定调自动化工作流总架构——理清角色（§2.6 岗位说明书）是建自动化的前置；终态管道=老朱只做输入 → 入口门控（王语嫣）→ 自动化工作流 → 出口门控（欧阳锋）→ 老朱最终判断与审查。
- **落点**：charter §3.14 三层质量门控（新章）；#448 老朱角色卡收口述 №1（一等证据，职责节与 §3.14 对齐）；王语嫣 spec v1.0 职责「门禁判定」条与本架构天然一致（已含）。
- **与 F-033 关系**：自动化边界不变——机械全自动+agent 在人规则内执行，判断永远留人。

## 2026-08-23：已交冻结与文件流转规范入宪（老朱拍板，D4 记录）

- **拍板**：老朱定协作纪律——建议书进入看板流程后冻结不准再改，订正另起新文件；每 agent 统一命名/版本号/日期时间节点/唯一编号。
- **落点**：charter §3.15 总纲入宪（两类文件两种纪律：一次性交付物全冻结+任务单 append-only）；#449 规范细则（老顽童，知识轨）；#450 工具支撑（黄药师，基建轨，依赖 #449 生效）。
- **根因链**：E046 吞节+黄药师已交增补困境（592559104 先例）→ 老朱统一纪律化。

## 2026-08-23：黄药师岗位说明书 v1.0 入宪同步（老朱终稿拍板「部署」，D4 记录）

- **拍板**：老朱 2026-08-23「446 已终审，你部署下」=终稿拍板，并入 charter §2.6.4（欧阳锋 PASS A）。
- **核心口径入宪**：验证三铁律+验证分层四态声明+门禁词表三层+只拦机械项+单一实例。
- **解锁**：#447 风清扬场依赖满足（一角色一张过），可开工。
- **改动文件**：agent-spec-huangyaoshi-builder.md（frontmatter 补 reviewed/欧阳锋）、kdo-charter-v0.1-draft.md（§2.6.4）。

## 2026-08-23：F-045 记忆胶囊开放口径拍板（老朱，D4 记录）

- **拍板**：老朱「记忆胶囊等跑稳定了再开放，保证全量保存就好。其他的按照你的意见执行」——L1 暂不开放（保持风清扬独享读/审计权），query 命令挂起；现阶段唯一硬要求=全量保存（#434 写入端+#432 双盘镜像已覆盖）。
- **其余按王语嫣意见执行**：#456（agent_id 统一+审计器盲区修复）照立单执行；#188 补登记已完成。
