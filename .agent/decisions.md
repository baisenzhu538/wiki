...
## 2026-05-17：Agent 入职必须消化讲香域——工业化保底，讲香保深度

**背景**：老顽童完成科学决策域 10 张卡编译，全部通过 v1.5 格式门禁，外部攻击学者引用精准。但用户审查时指出：双三角模型在初稿中被理解反了（人类三角误为审美/体系/基本功，正确是创造力/体系/审美），且卡片内容偏"精炼搬运"而非"消化再表达"。

**决策**：所有 agent 入职时，讲香域 12 张卡（1 framework + 10 tool + 1 concept）列入必读清单，排在规则文件之后。CLAUDE.md Step 1 已更新。

**原因**：工业化标准（v1.5 三要件、格式门禁、理解门禁）只能防 C-8 式"格式完整但思维空洞"，防不了"内容搬运而非理解消化"。讲香十指模型（场景化/比喻化/口语化/金句化/数字化/情绪化/故事化/素材化/冲突化/升华化）是深度消化后重新表达的核心能力。没有这个基本功，agent 写的卡就像翻译而非原创——准确但没魂。

**后果**：
- CLI 黄药师已通过讲香域编译，天然具备讲香基本功
- 飞书老顽童入职时未读讲香域，需补课
- 未来任何新 agent（无论 Hermes/cc-connect/其他平台）入职第一条：规则 → 讲香域 → 当前状态
