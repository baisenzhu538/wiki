---
name: research-core
title: 调研能力层统一入口——四类型判定+OSCAR 意图路由+核心纪律+专项武器库
description: 统一调研入口（全 agent 基础能力层，老朱 09-02 拍板）——任何调研/查证/深挖/情报任务先走本入口：第一层判定路由（A 四类型分型：深→挖掘式/高→系统式/宽→爆炸式/动态→自动式 + B OSCAR 意图路由）→第二层核心纪律（交叉验证+质量门禁+深挖引擎）→第三层专项武器库按需载。触发：调研、帮我查、查证、验证断言、验证这个说法、这个说法靠不靠谱、fact check、数据核验、深挖、尽调、行业分析、竞品分析、市场研究、情报收集、新领域调研、案例建模、竞对盯梢。非调研任务（纯写卡/纯施工）不路由进本层。
version: 1.1.0
author: Skills 助理（#594 整合生产）；老顽童 #629 第一层嵌四类型判定
...
```
第一层 判定路由（必做，两段）：
  A. 四类型判定（15 秒）：深→挖掘式 / 高→系统式 / 宽→爆炸式 / 动态→自动式
     （挖掘/爆炸/自动三线直接加载对应方法线 skill；系统式继续走 B）
  B. OSCAR 定目标（30 秒，系统式线入口）
  ↓
第二层 核心纪律（所有调研必经，逐项过）：
...
## 第一层 A：四类型判定（15 秒，先于 OSCAR）

按调研雷达图四维给任务分型，路由到对应方法线（诊断信号表，最小自包含不必先读子卡）：

| 诊断信号（用户话语/任务形态） | 类型 | 维度 | 路由 |
|:--|:--|:--:|:--|
| 「这个信息太重要了必须拿到」「竞品真实成本/水下情报/单点验证」 | 挖掘式 | 深·挖 | 加载 `research-digging-approach`（穷尽手段五层升级+单点狙击） |
| 「决策太复杂需要系统分析」「多维度整合支撑决策」 | 系统式 | 高·准 | 继续走下方 OSCAR 意图路由（本层 B） |
| 「完全不了解这个领域」「新赛道建认知」「案例建模找规律」「信息五花八门像盲人摸象」 | 爆炸式 | 宽·全 | 加载 `research-explosion-five-step`（五步法+饱和覆盖建模） |
| 「长期跟踪这个变量」「竞对又出动作了」「政策盯梢」 | 自动式 | 动态·稳 | 加载 `research-ci-framework`（Define→Gather→Analyze→Implement 循环）；只搭监控不跑单次调研 |

**分型规则**：①一次只判主导类型，混合任务按「先宽后深」分阶段（爆炸建图→按图挖掘/OSCAR 决策→CI 盯梢）；②挖掘式不是首选——80% 调研不需要，判不出"必须拿到"就不选它；③爆炸式是起点不是终点——建完认知接后续深挖；④分型拿不准 → 默认走 OSCAR（系统式是核心能力线）。

**When NOT（四类型整体）**：信息已充分/纯执行型任务不调研；高度敏感信息（涉国家安全/商业机密）先过专业合规审查。

## 第一层 B：OSCAR 意图路由（30 秒，系统式线入口）
...
| 需求信号 | 加载 | 一句话 |
|:--|:--|:--|
| 单点关键情报/尽调/水下数据（深） | `research-digging-approach` | 穷尽手段五层升级+单点狙击，底线之上努力拉满 |
| 新领域建认知/案例建模（宽） | `research-explosion-five-step` | 五步法+饱和覆盖，规律基本稳定即停 |
...
1. **四类型判定**（第一层 A）：深/高/宽/动态分型，路由到对应方法线；系统式继续走 B
2. **OSCAR 定目标**（第一层 B，系统式线）：写 O/S/C 三行到文档头
3. **首轮搜索**：按 Checklist 拆 3-5 个 query 搜索，收集 ≥5 个独立渠道素材（渠道不足 → 加载爬虫/Dorking/OSINT 补）
4. **交叉验证**（纪律 1）：核心结论逐条标信源层级，L5/L6 追加搜索；≥2 独立来源
5. **深挖**（纪律 3，按需）：关键假设跑 KAC；找 ≥1 个反例；矛盾处标注
6. **质量门禁**（纪律 2）：六维自检，不合格回补
7. **输出**：结论 + 来源列表 + 置信度标注 + 立即行动建议
...
- `framework-yitang-four-research-types` — 四类调研模型（第一层 A 四类型判定依据）
- `framework-yitang-research-radar` — 调研雷达图（深/高/宽/动态四维定位）
- `framework-yitang-oscar-research` — OSCAR 五步法（第一层 B 依据）
- `framework-baozhashidiaochan-five-step` — 爆炸式五步法（爆炸式路由线依据）
- `tool-yitang-research-exhaust-means` — 穷尽手段五层（挖掘式路由线依据）
