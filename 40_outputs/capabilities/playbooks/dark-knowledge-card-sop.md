...
**② 在 `70_product/tasks/laowantong-next-tasks.md` 标记完成**：
```
- [x] dk-c{N}-{slug}.md → 已产出，待欧阳锋审查
```

欧阳锋审查后会在此 append 回应。**不标记完成 = 审查者不知道要看。**
...
**这份 SOP 只覆盖暗知识卡生产（六字段撰写）**。不包含原子切分——那是黄药师的 `chunk_cards.py` 脚本自动处理的。老顽童的职责是产出合格的六字段卡片正文 + frontmatter。卡片写完后，黄药师的脚本会自动跑 chunk。
