...
| 4 | 至少 3 处音画同步修正标注 | ✅ 4 处（均匀帧时长 / Seg5 异常 / 帧-句子对位 / 缺动态时长分配） |

### 待修（3 项，洪七公 15min）

1. **交付物命名**：任务书要求 `timing.md`，产出为 `03-qa-report.md`。将 7g 部分（原 L182-229）拆成独立 `timing.md`。
2. **补逐段时长表**：当前只给了总时长（500.1s / 40 = 12.2s/帧），缺少按 5 个 segment 拆解的实测时长。需补：

| Segment | 帧范围 | 帧数 | 脚本标注 | 音频实测 | 建议帧时长 |
|---------|--------|:--:|:--:|:--:|:--:|
| Seg 1 Hook | F001-010 | 10 | ~45s | _待测_ | _待算_ |
| Seg 2 知识流水线 | F011-017 | 7 | ~2min | _待测_ | _待算_ |
| Seg 3 KDO工厂 | F018-031 | 14 | ~3-4min | _待测_ | _待算_ |
| Seg 4 真实流水线 | F032-034 | 3 | ~1.5min | _待测_ | _待算_ |
| Seg 5 Closing | F035-040 | 6 | ~1min | _待测_ | _待算_ |

3. **Seg 5 TTS 异常标注**：报告中提到 Seg 5 生成 558.5s（预期 ~60s），需在 timing.md 中记录此 bug 供黄药师排查。

### 做得好的

- **技术验证扎实**：分辨率/编码/时长/帧数逐一核实
- **主动发现并取证工具 bug**：Seg 5 TTS 异常 + compose 均匀分配问题，备份了异常音频
- **诚实标注局限**："精确对位需后期剪辑软件精调"
- **流程纪律合格**：停等信号遵守，未擅自进入 ship

### 后续

| 谁 | 做什么 | 估时 |
|----|--------|:--:|
| 洪七公 | 拆出 `timing.md` + 补逐段时长表 | 15min |
| 黄药师 | 修 Seg 5 TTS 异常 + compose `--segment-durations` | backlog |
| 欧阳锋 | timing.md 修正后放行 ship | — |

修正完成后通知欧阳锋，进入 `kdo video ship`。
