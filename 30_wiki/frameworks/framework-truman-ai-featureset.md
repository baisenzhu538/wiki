...
updated_at: '2026-08-28'
  # v0.1.1 终审返工：三处计数失真修正（P2 机械修订，欧阳锋 PASS A- 带返工项）
version: v0.1.1
tags:
- audience:general
- scene:learning
...
| 层 | 管什么 | 子域 | 项数 |
|:--|:--|:--|:--|
| 一、LLM 层（大模型层） | 模型与提示词本身 | 选模型 / 提示词 | 8（选模型 5+提示词 3；多轮对话另含 7 子项） |
| 二、数据层 | 给模型喂什么、怎么喂 | 上下文控制 / 增强数据 | 14（8+6） |
...
1. **OCR 单源风险**：全卡源自单张图的一次 OCR，虽对照周期表还原，仍有 5 处 ⚠️AI 推断未获原文证实——置信度上限受此约束。
2. **效率层明显单薄**（5+ 项 vs 数据层 14 项）：可能是原图截取不全，也可能是 Truman 自用版本就如此——无法从现有素材分辨，如实标注。
...
*v0.1 · 2026-08-28 · 老顽童按 #571 产：OCR 源对照 Feature 周期表 v1.0 还原，⚠️ 标记 5 处推断待原文证实（歌子角色/分展标注/主动搞要/分支环/楼型正配）*
