...
1. **适合**：问题已大致定义但需要快速找到关键原因/方案的商业分析场景；有一定行业经验可以形成合理初始假设的分析师；时间有限需要在1-2周内产出方向性结论的项目
2. **不适合**：问题完全未定义的早期探索（没有足够信息形成任何合理假设）；高度情绪化/政治化的组织决策（假设会被立场污染）；需要100%确定性结论的场景（假设驱动产出的是"最可能"而非"一定"）

### 使用限制

1. **初始假设的质量决定上限**：如果初始假设完全偏离真实原因，后续的验证只是在错误方向上高效地浪费资源
2. **验证实验的设计能力要求**：同一个假设，可以设计出"几乎一定能证实"或"几乎一定能证伪"的实验——实验设计者的主观性无法消除
3. **迭代次数受时间约束**：理论上可以无限迭代直到找到正确假设，但商业场景中通常只有1-2次迭代机会
4. **不能替代深度行业知识**：假设驱动是思维框架，不能补偿行业经验的不足

### 常见误用场景

1. **把假设当成结论**：形成初始假设后不再验证，直接用假设作为决策依据
2. **验证实验设计偏袒假设**：无意识中选择能证实假设的数据来源和实验方法
3. **在问题未定义时强行假设**：对完全不了解的领域凭空猜测，导致假设毫无根据
4. **忽视证伪信号**：当数据否定假设时，找借口推迟接受结论
