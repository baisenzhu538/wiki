...
**通过维度**：四要素定义（口述上 L534-560 ✓）、T/F 分界（L462-510 ✓）、五大学派（L1084-1110 ✓）、预测属性引语（口述下 L1812-1814 ✓ 高保真）。

**P1-1「8个格刻意留白（门捷列夫空位类比）」无源（L161-167）**：三个 source_refs 全查——口述上/下 grep 留白/门捷列夫/空位=0 命中；JSON gaps=82、inferred_from_oral 仅 F097-F100 四项，无「8」任何出处。预测属性本身有源，但「8个格留白」为无源演绎。

**P2-1 引语添加原文所无（L101）**：「成本越低」不见于口述上 L614-620（原文：「越小它越能验证，它越小它越快，它越小你就开始能使用」）。

**P2-2 引语演绎（L115）**：「从新手野路子到进阶体系态的思维分界线」不见于 L462-510 原文。

**期望形态**：① 删除或改写「8个格留白」（保留须补真实出处）；② 两处引语去引号改述或补正确原文；③ 复审走对照法。

**残余风险（非本卡缺陷）**：JSON gap_note/missing_note/c3_reconciliation 字段 mojibake 损坏（双重编码含替换字符，不可逆）——源数据质量问题，已另立建议书。
