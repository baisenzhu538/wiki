...
visual_analysis:
  - image: 一堂-科学决策-ROI决策评估画布.png
    dimensions:
      - 色彩语义: 蓝=宽度区(信息收集)；橙=深度区(分析计算)；绿=高度区(视角提升)；灰=行动区(落地执行)
      - 空间结构: 四象限画布，中心交汇=三维度协同；从外向内收敛=从发散到聚焦
      - 信息层级: L1 画布标题 → L2 四区标签 → L3 各区内子项 → L4 填写示例
      - 负形利用: 中心交汇区=三维度碰撞的"决策核心"；空白填写区=待填充的认知工作
      - 视觉隐喻: 画布=空白待创作；四象限=完整决策空间；中心=决策的"奇点"
  - image: 一堂-科学决策-ROI决策评估画布-案例01.png
    dimensions:
      - 色彩语义: 填色后的实际案例，蓝色块=已完成宽度分析，橙色块=深度计算中
      - 空间结构: 同一画布结构，但内容填充后呈现"左满右空"=宽度已找全深度待下探
      - 信息层级: L1 案例背景 → L2 已填内容 → L3 待填空白 → L4 决策结论
      - 负形利用: 空白区=认知缺口；已填区的密集=分析完成度
      - 视觉隐喻: 填色过程=决策的"着色"；从空白到完整=认知的渐进清晰
  - image: 一堂-科学决策-ROI决策评估画布-案例02.png
    dimensions:
      ...
      - 负形利用: N/A空白=认知断裂(战略投资无法直接衡量)；机会成本留白=教学互动点
