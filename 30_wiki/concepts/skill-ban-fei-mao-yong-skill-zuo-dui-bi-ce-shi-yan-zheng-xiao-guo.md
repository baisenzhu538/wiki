...
- **没有 A/B 测试的 Skill 是信仰而非工具**。半肥猫的观察：很多人做完 Skill 后觉得"应该有用"，但没有数据支撑。A/B 测试是唯一能从"感觉"进入"证据"的桥梁。
- **反向测试比正向测试更能暴露问题**。正向测试是必须的，但反向测试（故意提交边界/越界/无关场景）更能检验 Skill 的边界是否清晰。一个在正向测试中表现很好但在反向测试中崩溃的 Skill 是危险的。
- **高风险场景测试是必需的**。保险、医疗、金融、法律等低容错场景需要单独设计测试集。半肥猫的保险场景测试结果：Skill 在合规性和可观测性上的表现证明了它在高风险场景下的价值。
- **三类测试集缺一不可**：正向测试集（典型适用场景）、反向测试集（越界场景）、高风险测试集（敏感行业或低容错场景）。
- **对比两组得分并分析差距来源**。方案 A（用 Skill）vs 方案 B（不用 Skill），结构化评分后重点看哪些维度差距最大、为什么、如何改进。
...
- [ ] 已列出 3 类测试集：正向、反向/越界、高风险
- [ ] 已和 AI/团队约定好多维度评分标准（建议 ≥8 个维度）
- [ ] 已执行"有 Skill"vs"无 Skill"的对照测试
- [ ] 已记录每组测试的评分和具体证据
- [ ] 已识别差距最大的维度并制定改进动作
- [ ] 已检查高风险场景下的合规性、可观测性和拒绝能力
- [ ] 已确认评分过程尽可能独立或引入第三方评分
- [ ] 已将 A/B 测试作为 Skill 迭代和上线的门禁
...
| 关系 | 目标节点 | 说明 |
|------|----------|------|
| 上位 | [[skill-ban-fei-mao-she-ji-skill-de-ping-fen-gui-ze-yu-feng-xian-bian-jie]] | 测试需要基于评分规则——两者配套使用 |
...
- [[skill-ban-fei-mao-she-ji-skill-de-ping-fen-gui-ze-yu-feng-xian-bian-jie]] —— A/B 测试需要基于评分规则，两者配套使用
- [[case-ban-fei-mao-skill-ab-test]] —— 半肥猫 A/B 测试的完整实例
- [[dk-ban-fei-mao-skill-rejection-value]] —— "Skill 的最大价值是拒绝"，反向测试是验证核心
- [[skill-半肥猫-课程Skill化的八步工作流]] —— 八步工作流中的第 7 步（测试验证）
