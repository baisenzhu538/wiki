...
- claim:boundary-01 [conf=0.85] **"验证通过"不等于"假设为真"**：调研只能提供"支持证据"或"反驳证据"，不能"证明"假设为真。即使100个用户都说愿意付费，不代表第101个会付。验证结果需要概率化理解，而非二元判断。
- claim:boundary-02 [conf=0.80] **最小实验的"最小"有下限**：某些假设（如"长期留存率>30%"）无法在短期实验中验证。强行"最小化"可能导致验证的是"短期指标"而非"真正关心的假设"。
- claim:boundary-03 [conf=0.75] **"证伪恐惧"导致验证失效**：团队可能无意识地设计"容易通过"的实验，或在结果不利时找借口解释。验证工具本身不能解决"人们不想听到坏消息"的心理问题。
