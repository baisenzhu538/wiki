...
type: concept
status: enriched
source_context: 一堂 2026-06-13 业务公式拆解培训，参数冰山模型
source_refs:
...
使用以下 7 个问题判断当前拆解是否足够深、足够准：

| 编号 | 自检问题 | 如果答案为“否” |
|---|---|---|
| 1 | 当前公式能否提出**可验证假设**？ | 继续往下拆到 L3-L4 |
| 2 | 每个**定性参数**是否找到 **3-5 个行为指标**？ | 把“我觉得”改成“我看到” |
| 3 | 是否已经**先切分（+）再拆转化（×）**？ | 重拆，避免总指标掩盖子群体差异 |
| 4 | 运算符号是否写对：+ 是叠加，× 是漏斗？ | 检查每个参数的替代关系 |
| 5 | 是否区分了**相关与因果**？ | 画因果链，做控制变量实验 |
| 6 | 是否为关键参数找到**行业/历史基准**？ | 找不到基准就无法判断“低还是高” |
| 7 | 公式团队**能否执行**？ | 拆得太复杂要回退到 L4 可操作层 |

> **判断标准**：一个参数是否需要继续拆到 L6，只有一个标准——这个参数能不能提出可验证的假设？能，就停；不能，继续拆。
