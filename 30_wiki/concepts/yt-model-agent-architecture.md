...
| 强烈不确定的探索期（噪声反馈、未知未知主导、连变量名都不知道） | Learning Agent依赖反馈信号来更新部件，但噪声反馈会让它学到虚假模式——相当于在随机数据上做梯度下降。在"哪些变量重要"都不知道的阶段，优化是对噪声的过拟合，越学越偏 | 用Dave Snowden的**安全失败探测试验**（Safe-to-fail Experiments, Cynefin框架）：不做优化，做探测——设计最小成本、最小后果的干预，只观察不校准，积累足够信号后再切换为utility-based。具体操作：跗≥5个低成本平行探针（parallel probes），观察≥3个周期后再做第一次模式判断 |
...
| 触发场景 | 第一个动作 | 成功指标 |
|---------|-----------|---------|
| 要搭建一个新的工作流、自动化管道、或任何决策系统 | 在白板上画PEAS四元组表。按以下顺序填写——先填A（执行器/可用工具清单）和S（传感器/反馈来源清单），再填E（环境里的其他玩家和规则），最后填P（成功指标）。这个顺序强制你先盘点手段再定目标，避免P膨胀到A够不着 | 四格全部填满，且每个格子有≥3条具体条目（不是"A=写代码"这种笼统项，而是"A=用Cursor写、用Vercel部署、用Sentry监控"这种可操作项）。填完后如果有空格子，设计未完成——不要进入执行阶段 |
| 发现自己在同一模式的问题上反复碰壁——拖延半年还没动、同一类冲突反复发生、每次项目到某个阶段就崩 | 拿出五层架构图，做"gap诊断"——写下两句话：（a）"我当前主要在哪个层级操作？"（b）"这个问题的复杂度要求哪个层级？"如果（a）低于（b），gap就是根因。例如："我用reflex agent（看到任务→应激处理）面对一个需要goal-based规划（先定义目标状态→反向推导动作序列）的项目管理问题" | 诊断结果写成"卡在第X层，问题需要第Y层"的格式，且附至少一个具体事例说明为什么第X层不够（例如："我用if-then处理客户投诉——客户说贵就解释、客户不说就不动——但这需要goal-based：先定义'客户关系状态目标'，再反向规划接触节奏"）。不允许输出"我觉得自己不够理性"这种没有层级标签的诊断 |
| 读完一本书、上完一门课、完成一个项目、参加完一个会议后——任何"我学完了"的时刻 | 拿出一张纸，写下这句话并填空："我更新了__（四选一：世界模型/效用函数/动作策略库/评分体系），具体是__从__变成了__，证据是__。"如果不能完成这个填空，你没有学习——你消费了内容 | 能写出至少1条具体修正记录，格式为"在[部件名]中：[旧认知/旧做法]→[新认知/新做法]，证据是[具体经历/数据/反馈]"。"我学到了很多"不算通过——必须指出哪个部件被修改、修改前后的差异、以及触发修改的证据 |
| 在两个以上相互冲突的目标之间反复纠结——赚钱vs意义、速度vs质量、深度vs广度、家庭vs事业。纠结超过一周没有行动 | 写效用函数。步骤：（1）列出所有相关变量（≥4个）；（2）给每个变量赋权重，权重之和=1.0；（3）用这个加权公式评估至少3个备选方案。把决策从"跟着感觉反复横跳"变成"给定我的权重，这个方案得分最高" | 产出至少一个效用公式，权重精确到小数（如0.35不是"重要"），且能向另一个人用≤3分钟解释清楚"为什么健康权重0.4而不是0.3"。允许事后调整权重（这就是learning agent更新效用函数），但不允许"权重差不多就行"——模糊的权重等于没有权重 |
