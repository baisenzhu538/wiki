...
- trigger:01 **准备做知识萃取时 → 先判断是"显性知识密集型"还是"隐性知识密集型"领域**。第一步：问自己"这个领域最牛的人做完示范后，他能不能一步一步写出他在做什么？"如果能——显性为主，直接萃取。如果不能（如销售语感、设计直觉、领导时机判断）——隐性为主，萃取仅做"决策启发式"而非"可复制SOP"。成功标志：你能说出萃取目标是什么类型，以及产物形式（SOP vs 启发式清单）是否匹配这个类型
- trigger:02 **辩证式讨论后达成了"专家共识" → 追加一轮"狐狸型挑战"**。第一步：请专家们各自列出"我的这个判断最可能因为什么而错？在什么条件下这个方法论会失效？"。成功标志：你能列出≥3个"这个共识可能失效的具体条件"——如果没有，说明讨论可能由刺猬型专家主导，需要引入更多边缘声音
- trigger:03 **案例大爆炸收集到一定程度 → 停下来检查是否饱和**。第一步：随机取最近收集的20个案例，数其中有多少个让你产生了新的分类/新的边界/新的洞察。如果<2个，说明接近饱和。成功标志：你能用一句话定义"这个领域案例的饱和标准"（如"新案例不产生新分类也不改变已有分类的比例"）并说出当前接近还是远离饱和