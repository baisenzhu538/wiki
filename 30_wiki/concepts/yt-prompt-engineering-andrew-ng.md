...
- claim:boundary-02 [conf=0.85] 信息要充分但不等于越多越好。无关信息会严重干找AI输出（案例：给AI喂了无关的医疗指标导致它坚持建议不必要的检查）。只给与当前问题相关的背景，不同问题用不同对话窗口避免交叉污染
...
| 团队成员AI素养参差不齐——部分人用提示词、部分人不用 | 不均匀的AI使用会造成决策质量的两极分化，最终会形成"会用AI的人更快更好——不用AI的人被拉下" | 团队级别的提示词模板和最佳实践分享，确保基本能力均匀化 |
