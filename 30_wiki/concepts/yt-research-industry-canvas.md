...
| 边界 | 说明 |
|:-----|:------|
| ✅ 适合条件 | 需要快速了解一个陌生行业的创业者/投资者、做行业初步筛选（淘汰明显不可行的方向）、作为完整调研的前置快速扫描 |
| ❌ 不适合条件 | 高 stakes 决策（如投入全部身家）、行业处于技术颠覆期（历史数据失效）、需要精确市场规模而非粗略估算的场景 |

### 常见失败模式

| 模式 | 症状 | 修复 |
|:-----|:------|:------|
| **画布太泛——范围界定过宽** | 画布覆盖了行业的所有维度但每个维度都很浅。用户填完后说"我了解了这个行业"但说不出"所以我应该做什么"。信息密度高但决策密度低。 | 在开始画布之前先写"决策问题声明"：用一句话写下"我要用这张画布回答什么具体问题"。画布的所有维度都围绕这个问题展开，与问题无关的维度直接砍掉。 |
| **确认偏见陷阱** | 用户只关注支持"自己想进入"的证据，忽略反证。画布的结构化反而给了"我已经全面考虑过了"的虚假安全感。 | 在每个维度最后加"证伪问题"：不是问"这个行业增速是多少"，而是问"什么样的证据会让我认为这个行业不值得进入"。画布不产出"不进入"的结论，就等于没做完。 |
