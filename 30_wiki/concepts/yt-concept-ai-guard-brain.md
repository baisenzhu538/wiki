...
- claim:boundary-01 [conf=0.90] 四遍学习法不适用于所有场景。如果知识的价值是一次性的（一个操作步骤、一个临时决定），直接让 AI 执行即可。四遍学习法适用于你判断为"值得内化为长期能力"的知识——判断标准：未来 3 个月内是否会重复用到

- claim:boundary-02 [conf=0.85] 不要让 AI 替你完成"手写笔记"这一步。手写的价值不在于笔记本身，而在于你在手写过程中必须决定"什么值得写、怎么写才准确"——这是一个强制理解的步骤。如果跳过这一步直接让 AI 生成笔记，后面三遍全部建立在 AI 的理解之上而非你的理解之上
