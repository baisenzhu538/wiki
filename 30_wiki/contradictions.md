---

id: contradictions
type: index
status: draft
title: contradictions
domain:
- master
source_refs:
- src_unknown
author: system
reviewed_by: pending
confidence: 0.5
trust_level: low
created_at: '2026-06-16'
updated_at: '2026-06-16'
source_context: （原始 source 无法追溯，已标记为 src_unknown，待后续补充）
related:
  - '[[graph-rag-retrieval-layer]]'
  - '[[plan_20260501_e1e150b9-improvement-plan]]'
  - '[[plan_20260501_85a84b92-improvement-plan]]'
  - '[[plan_20260501_ca61cdd7-improvement-plan]]'
  - '[[plan_20260501_8001399c-improvement-plan]]'
---
# Contradictions

Use this file to preserve unresolved conflicts instead of smoothing them over.

| Date | Topic | Source A | Source B | Conflict | Current judgment | Next check |
| --- | --- | --- | --- | --- | --- | --- |
| 2026-05-02 | 紫鲸AI两篇wiki页面内容重叠 | 30_wiki/concepts/紫鲸AI_智能体工作流平台_深度分析与产品设计 | 30_wiki/concepts/紫鲸AI智能体工作流平台 | 两篇wiki内容高度重叠（共享核心术语），可能是重复页面或需要合并 | 已定案：保留两篇，深度分析篇加 wikilink 指向平台介绍篇 | resolved |
