...
- **内部局限**："相关指标 vs 因果抓手"的区分依赖因果识别能力——多数企业无法低成本做因果实验（A/B 测试不可行场景）；"找因果抓手"可能延误行动（相关性指标在快速变化期也有效）。
- **外部攻击（数据科学视角）**：严格因果推断（RCT/工具变量）在商业场景常不可行——务实路径是"准实验+多指标三角验证"（相关+机制+反例）；"因果抓手"叙事有幸存者偏差（事后能找到因果，事前很难）；运营上"相关指标先跑+因果验证后调"是更现实的分层策略。
