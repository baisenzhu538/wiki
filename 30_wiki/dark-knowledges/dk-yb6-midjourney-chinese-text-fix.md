...
- **内部局限**：中文文字修复的"极简提示词"依赖 Midjourney 版本能力——MJ 对中文文字渲染的支持随版本变化（V7/V8 改进），提示词会过时；修复效果受字体/复杂文字（多字/竖排）影响大，极简提示词对复杂场景不够。
- **外部攻击（工具视角）**：中文文字生成已不是 Midjourney 独有难题——GPT-4o/即梦/通义等原生中文支持更好；"修复提示词"是治标（在弱文字能力的模型上补丁），选对模型（原生支持中文）才是治本；工具选型比提示词技巧更重要。
