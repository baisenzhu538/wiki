...
- **内部局限**：用 AI 对抗 AI 生成标准的前提是「评审器比生成器更可靠」——如果评审器同样水平（同模型同资料），对抗可能只是自我强化（确认偏误）；且 7-8 轮迭代的成本不低，不适合所有场景。
- **外部攻击（博弈论视角）**：生成器+评审器对抗可能收敛到「评审器偏好」而非「真实质量」——两 AI 相互对齐（collusion）后标准空洞化；需要外部锚（真实用户反馈/客观指标）打破闭环，否则标准是「AI 自己定义的好」。
