...
- **内部局限**：质量=标准×迭代的公式隐含假设「迭代方向正确」——如果标准本身错误，迭代只会更高效地做出错误的东西；且公式对「标准难以定义」的任务（探索性工作）不适用。
- **外部攻击（质量理论视角）**：戴明质量观批判「检验出质量」——依赖迭代打磨质量是事后补救（rework），真正高质量来自上游（输入/过程）设计；「多轮迭代」在成本敏感场景可能不如「一次做对」（有经验的领域专家一次交付质量更高）。
