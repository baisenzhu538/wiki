...
- **内部局限**：逆向视觉书法（不训练模型锁定风格）依赖"参考图质量"——参考图模糊/风格不纯时锁定失效；"逆向"方法对抽象风格（笔触情绪/墨韵）的描述困难，三要素法有表达上限。
- **外部攻击（AI 图像视角）**：不训练锁定风格是"零成本方案"——但 LoRA/风格训练的成本已大幅下降（少量图即可微调），复杂风格仍建议微调而非描述；"逆向描述"依赖生图模型的风格理解力（V7/V8 理解力提升但非完美）；方法适用性是"快速原型"而非"生产级稳定"。
