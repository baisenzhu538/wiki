...
- **内部局限**：八要素命名体系增加命名成本（长文件名/编码负担）——对个人快速产出场景（随手截图）执行成本高；"八要素"是经验总结，不同团队/项目对命名粒度的需求不同，一刀切八要素会过度工程。
- **外部攻击（信息管理视角）**：命名规范的价值在检索——但检索工具（Everything/标签系统/数据库）演进后，命名的重要性下降（内容搜索优于文件名搜索）；"AI 增效前置"的命名体系是面向人工检索的设计，AI 时代更高效的是"内容结构化+元数据"，命名只是其中一环。
