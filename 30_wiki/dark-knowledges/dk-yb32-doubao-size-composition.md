...
- **内部局限**：豆包生图尺寸是唯一关键排版因素（针对豆包特定模型）——模型更新后结论可能失效（豆包 V2 可能改善尺寸控制）；"唯一关键"是经验断言，不同模型（即梦/通义）表现不同。
- **外部攻击（工具视角）**：尺寸问题的本质是"生图模型对画布的语义理解弱"——其他模型（Midjourney/即梦）对尺寸-构图控制更好，工具选型可绕过；"尺寸先行"是 fallback 方案，正确方向是"模型能力提升+提示词表达构图意图"；经验应标注"模型版本适用性"。
