...
- **内部局限**：浑水→清水→浑水的三阶段模型是概念隐喻——对训练细节（数据清洗/对齐的具体方法）不提供操作指导；"浑水"阶段的质量依赖数据本身的分布，模型无法完全脱离训练数据的限制。
- **外部攻击（机器学习视角）**：训练过程的"清水"（预训练）与"浑水"（对齐/微调）的比喻是简化——RLHF/对齐的本质是"注入人类偏好"而非"变浑"；对齐可能牺牲部分能力（对齐税），"浑水"不是坏事而是权衡；比喻过度会误导对模型能力的直觉（以为对齐后模型变差是 bug）。
