...
- **内部局限**：先跑 MVP 再开 PS 的前提是「方向验证可以脱离审美执行」——对品牌一致性要求高的设计（视觉资产延续性），粗糙 MVP 可能误导方向判断；且「找图不需要审美」对新手适用，资深设计师可能不需要这种分离。
- **外部攻击（设计方法论视角）**：先验证后执行是通用工程思维，但设计领域「方向与执行互相塑造」——草稿质量影响方向判断（做得好的方向可能因为草稿差被误杀）；对高度审美驱动的项目，粗 MVP 验证可能产生错误结论。
