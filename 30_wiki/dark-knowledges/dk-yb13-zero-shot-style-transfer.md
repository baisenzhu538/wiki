...
- **内部局限**：零训练风格迁移（三要素描述法）依赖三要素（主体/风格/细节）的准确提取——复杂风格（综合多种流派）单靠三要素描述不够；"零训练"的上限低于微调（无法捕捉风格的全部细节）。
- **外部攻击（技术视角）**：三要素描述法是 prompt 工程的简化——现代生图模型支持图像参考（IP-Adapter/风格参考图），比文字描述更保真；"零训练"在风格极强/极复杂场景应升级为"参考图+微调"组合；描述法适合"快速尝试"不适合"稳定复现"。
