...
- **适用边界已显式声明**（见「适用边界」节）：本卡的核心主张——"解释性本质必须能指导下一次真实决策"——对纯认知兴趣/知识自洽场景不适用，那里"解释性本质"仍有价值；若读者将该主张无差别套用于所有知识学习场景，属于误用。
- **"能指导决策"标准本身有模糊地带**：不同决策者对"指导"的判定松紧不同（有人要具体操作步骤，有人只要方向约束）。本卡未给出可操作的判定标尺，判定仍依赖读者经验。
- **单一叙事样本**：本卡的核心论证来自少数建模实践案例（真实案例节），未覆盖"解释性本质恰好在长期后转化为决策优势"的反例（如基础理论类知识常延迟兑现）。
