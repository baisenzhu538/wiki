...
- **内部局限**：伪需求检测依赖"真需求"的定义——定义偏差（把低频当伪/把场景缺失当真）导致误判；检测方法（访谈/数据）各有盲区（访谈会说谎/数据会误导）。
- **外部攻击（产品视角）**：伪需求 vs 真需求是产品界经典难题——"伪"的判定应看"用户是否付费/高频使用"（行为证据）而非"用户是否声称"（言语证据）；伪需求检测应"低成本试错"（MVP 验证）而非"纸上分析"（分析无法证伪）；"伪需求"标签有事后归因风险（失败的产品常被归因伪需求）。
