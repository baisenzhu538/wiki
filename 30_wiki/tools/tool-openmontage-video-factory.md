...
- 数据严重不足时暂停使用——OpenMontage 中文 MCP 版的输出质量高度依赖输入数据的代表性和完整性。宁可先补数据。
- 刚接触OpenMontage 中文 MCP 版的新手不要直接用——先通过3-5个真实案例建立体感，否则执行只是走流程。
- 资源极度受限时不要追求OpenMontage 中文 MCP 版的完整流程——做20%的核心步骤比做100%的敷衍步骤更有价值。

## 质疑


- **具体假设**：OpenMontage 中文 MCP 版工具假设结构化方法论能产生正确结论，但结论质量取决于输入数据和执行者判断力。
- **边界**：在OpenMontage 中文 MCP 版相关的数据稀缺或快速变化场景中，已有经验框架可能完全失效。
- **反例**：团队在OpenMontage 中文 MCP 版任务中完整执行了所有步骤，但核心假设从一开始就是错的。
- **前提**：使用者已具备OpenMontage 中文 MCP 版领域的基础认知，且数据来源具有代表性。
...
1. 确认OpenMontage 中文 MCP 版的适用前提是否满足
2. 梳理当前状况与目标的差距
3. 选择对应的OpenMontage 中文 MCP 版分析维度
4. 逐维度填写并标注数据来源
5. 汇总形成行动建议
...
系统化解决OpenMontage 中文 MCP 版中的常见陷阱——通过前置检查和后置验证，将错误率降到最低。
