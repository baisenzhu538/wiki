...
- 把第一版的范围从“完整产品”收敛到“验证单一核心假设的最小集合”，降低早期投入的沉没成本。
- 用一堂 6 类可砍模块清单做团队对齐，避免靠直觉或权力博弈决定砍哪些功能。
- 为每个保留功能绑定可观测指标，让最小版本发布后能直接产生验证假设的数据。
...
- 团队还没锁定唯一要验证的核心假设时。此时“砍功能”会失去标尺，容易变成谁声音大就砍谁，而不是服务于假设验证。
- 产品内核价值必须依赖多个模块同时存在才能成立时。例如双边市场若只保留一边，用户无法完成关键交易行为，砍模块会直接让实验失效。
- 所处行业对合规、安全、信任模块有硬性底线要求时。金融、医疗、食品等领域中，KYC、资质展示、隐私协议等功能属于“不可砍”，否则实验未开始就已触发法律或品牌风险。
- 团队把“砍功能”当作逃避用户访谈和快速发布的借口时。如果真实瓶颈是没人去见用户，而不是范围过大，继续砍功能只会延迟失败。
...
- **假设**：本工具隐含「产品内核价值可在 1-2 个功能中独立交付」的假设；若价值必须依赖多个模块同时存在（如双边市场），砍功能会提前证伪一个其实成立的方案。
- **边界**：适用于需求已被初步验证、只需压缩范围的早期产品；不适用于强合规行业或品牌敏感型高客单价产品，因为这些领域存在不可砍的信任/合规模块。
- **反例**：Dropbox 早期砍掉大量功能但仍保留「跨设备同步」内核而成功；相反，某些 SaaS 因砍掉权限管理、审计日志等「非核心功能」，在 enterprise 客户处直接出局。
- **前提**：团队必须先锁定唯一要验证的关键假设，否则砍功能会变成权力博弈而非理性收敛。
