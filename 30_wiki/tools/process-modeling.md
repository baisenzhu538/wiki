...
- **C1 [conf=0.9]**: 流程建模的三大重点是：**及时复盘、清单/SOP、稳定使用**。——依据：10_raw/sources/src_20260614_623cfbfd-高阶建模-流程建模.md:1`。
- **C2 [conf=0.85]**: 及时复盘的关键是「趁热」——在业务现场或结束后立即总结，过了几小时或几周经验就丢失了。——依据：10_raw/sources/src_20260614_623cfbfd-高阶建模-流程建模.md:3`。
- **C3 [conf=0.85]**: 清单/SOP 需要具备「逻辑洁癖」：生清单（遇到换行就分点）、优先级（遇到决策就分级）、MECE（遇到建模就完备）、逻辑链（循序渐进）。——依据：10_raw/sources/src_20260614_623cfbfd-高阶建模-流程建模.md:5`。
- **C4 [conf=0.8]**: 稳定使用需要给 SOP 加锁：版本 1 只靠自觉（50-70% 执行率），版本 2 增加督导 SOP（70-90%），版本 3 增加品控 SOP（接近 100%）。——依据：10_raw/sources/src_20260614_623cfbfd-高阶建模-流程建模.md:7`。
- **C5 [conf=0.8]**: AI 可辅助流程建模，例如自动整理会议反馈、生成自查清单、扫描对话窗口封装技能。——依据：10_raw/sources/src_20260614_623cfbfd-高阶建模-流程建模.md:9-12`。
- **C6 [conf=0.75]**: 直播前 SOP 案例包含 20-30 条细则，覆盖饮食、休息、设备调试等，确保直播状态稳定。——依据：10_raw/sources/src_20260614_42f1e977-一堂-建模能力培训-truman-笔记.md:41-42`。
