...
- 需要秒级决策的紧急场景中不要用——光谱模型定位法的完整流程耗时较长，紧急场景需要更轻量的判断。
- 资源极度受限时不要追求光谱模型定位法的完整流程——做20%的核心步骤比做100%的敷衍步骤更有价值。
- 在问题边界尚不清晰时不要使用——光谱模型定位法需要明确的目标和约束才能有效。先做探索性分析再回来。

## 目的


帮助团队在光谱模型定位法任务中减少盲目试错——提供清晰的执行路径和验证节点，降低返工率。
