...
4. **产出资产化（#396 补强）**：最佳实践指南文档=资产包（楚门口述 L1550-1554："最佳实践指南文档是一个参考文档……那是一个最佳实践的最佳文档是个资产包"）——Pipeline 的最终产出应落库为可复用资产包（Obsidian/知识库），而非一次性报告；"口喷→Agent 科学调研→Obsidian 资产"（逐字稿 L343）是 AI 原生调研流的完整闭环。
