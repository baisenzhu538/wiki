...
| 失败模式 | 症状 | 修复 |
|:-----|:-----|:-----|
| **跳过第1步质量门** | 直接对营销包装华丽但无科学底层的方法论做Skill化，输出看似专业实则不可信 | 严格执行第1步：检查课程是否有可验证案例、明确边界、可复现的方法论，不满足则拒绝 |
| **把课程原话当真理** | 第5步批判性审查流于形式，Skill直接复制讲师观点，出现事实错误或过时结论 | 不默认老师全对：交叉验证核心主张，补充真实案例和权威资料，标注不确定点 |
| **结构化时过度删减** | 第2步整理文档时只保留金句和案例，丢失方法论的操作前提和推导逻辑 | 以"可复现执行"为标准保留主线：输入、步骤、判断条件、输出格式缺一不可 |
| **诊断协议只有适用没有拒绝** | 第4步只写"什么时候用"，没写"什么时候不能用"，导致Skill被滥用 | 诊断协议必须包含拒绝条件、风险分级和触发边界，明确"此情况不处理" |
| **A/B测试只做正向案例** | 第7步测试只跑成功场景，遇到反向案例和边界案例时Skill表现崩溃 | 测试集必须包含正向、反向、高风险三类场景，用多维度评分体系量化对比 |
| **目录结构过度原子化** | 第6步把Skill切得太碎，用户调用时需要组合多个原子文档，实际体验 fragmented | 按"一次完整任务"为粒度切分目录，原子化以可独立调用为前提 |
| **安装后不再迭代** | 第8步只完成部署，不积累反例、不更新版本，Skill随业务变化快速失效 | 建立反例收集机制和迭代周期，把维护成本计入Skill总投入 |
| **小团队强行跑满八步** | 1-2人团队在无测试资源的情况下硬上完整流程，交付周期过长 | 根据团队规模裁剪：小团队先做1-2-4步的最小可用Skill，再补测试和迭代 |
