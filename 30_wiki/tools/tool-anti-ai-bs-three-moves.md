...
> OpenClaw 数字员工搭建者（口述 L652-660）

- 「第一个就是如果说这个回复里面有一些内容你是看不懂的，你就让 AI 给你做一个详细的解释。如果说解释以后还不能解决你的困惑，你就让 AI 去找同类型的事件或者问题。如果第二个问题还是不能解决，就按 AI 给你出一个最低成本的验证方法。」（L654-659）——防忽悠三层递进被独立案例证实（解释→找同类→最低成本验证）
