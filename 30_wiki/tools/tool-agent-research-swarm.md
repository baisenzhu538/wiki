...
- **具体假设**：Swarm 模式假设"无中央协调者也能收敛"，但多 Agent 自由交接的后果是路径不可预测——某些任务可能在 Agent 间反复弹跳而永不收敛，这是其适用**边界**。
- **反例**：在需要多步推理的研究任务中（如"分析某公司财务健康度"），Swarm 可能出现 Agent A 搜索财报 → Agent B 提取数据 → Agent C 发现缺关键指标 → 交回 Agent A 重新搜索 → 无限循环。

**Harrison Chase**（LangChain 创始人）会质疑：Swarm 的"省 token"优势在简单任务中成立，但当任务复杂度上升时，Agent 间的重复搜索和信息丢失反而导致总 token 消耗激增——"无中央协调"不是免费午餐，它把协调成本转移到了每个 Agent 的 prompt 中。
