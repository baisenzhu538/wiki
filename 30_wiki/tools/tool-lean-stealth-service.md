...
- 当本轮要验证的核心假设是「算法/匹配引擎/定价模型本身能否达到可用精度」时，不应使用偷偷服务——人工后台无法暴露推荐、风控、动态定价等系统能力的真实误差。
- 当目标场景对响应延迟、并发稳定性或合规披露有刚性要求（如证券交易、医疗问诊、紧急救援），人工后台难以持续履约，且隐瞒后端可能触发监管或品牌风险。
- 当团队为做一个「看起来像产品的界面」所需投入已经接近甚至超过直接开发轻量系统 MVP 时，偷偷服务失去低成本验证意义，应直接切系统 MVP。
- 当团队没有足够领域专家或运营人力在实验周期内稳定交付服务标准时，失败可能来自人工交付能力不足，而非需求不存在，导致假阴性。
- 当需要验证的是规模化后的单位经济模型（边际履约成本、毛利结构）而非用户付费意愿时，人工交付的成本结构会误导对真实商业模式的判断。
