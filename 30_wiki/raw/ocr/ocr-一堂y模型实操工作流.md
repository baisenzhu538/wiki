...
- 具体假设：五步流程（圈定→明确→形成→预判→升级）是线性执行的。但 Mintzberg 证明大多数成功战略是涌现的而非计划的。
- 边界：本工作流适用于有明确问题域的项目，不适用于探索性创新或需要快速试错的场景。
- 反例：在已被巨头占据的市场中，五步流程做得越好可能失败得越快——因为"把事情做对"不等于"做对的事情"。
- 前提：项目开始时就能"圈定范围"和"明确目标"——但创业的本质常常是在行动中才发现真正的目标。
