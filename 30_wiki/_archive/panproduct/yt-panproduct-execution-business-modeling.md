...
- 资源极度受限时不要追求泛产品设计·落地卡片：业务建模的完整流程——做20%的核心步骤比做100%的敷衍步骤更有价值。
- 数据严重不足时暂停使用——泛产品设计·落地卡片：业务建模的输出质量高度依赖输入数据的代表性和完整性。宁可先补数据。
- 需要秒级决策的紧急场景中不要用——泛产品设计·落地卡片：业务建模的完整流程耗时较长，紧急场景需要更轻量的判断。
