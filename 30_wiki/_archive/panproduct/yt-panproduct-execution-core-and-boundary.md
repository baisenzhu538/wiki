...
- 数据严重不足时暂停使用——泛产品设计·落地卡片：内核和边界的输出质量高度依赖输入数据的代表性和完整性。宁可先补数据。
- 刚接触泛产品设计·落地卡片：内核和边界的新手不要直接用——先通过3-5个真实案例建立体感，否则执行只是走流程。
- 资源极度受限时不要追求泛产品设计·落地卡片：内核和边界的完整流程——做20%的核心步骤比做100%的敷衍步骤更有价值。
