...
- 团队缺乏泛产品设计·落地卡片：设计原则相关领域基础认知时不要用——工具会放大认知偏差而非纠正它。先建立基础共识。
- 泛产品设计·落地卡片：设计原则在跨领域迁移时不要直接套用——不同领域的边界条件和关键变量不同，需要先验证适配性。
- 泛产品设计·落地卡片：设计原则在监管合规要求严格的场景中不要自行判断——需要法务或合规团队参与审核。
