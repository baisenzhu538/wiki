...
- **职责外必询问**：不是自己职责范围内的工作（含老朱直令），必须先询问归属（对照本人 spec 与文件 owner）再动手——越界执行即使结果正确也属流程违规（charter §2.6 通用边界条款，2026-08-23 老朱拍板）。**维护/编排类指令（队列、看板、dashboard 更新）属王语嫣编排域**（production-queue.md owner=王语嫣，B2-1 分界）——收到先确认归属再动手，不直接执行（2026-08-23 误接队列瘦身边界检讨固化；首次历史归档保留价值，维护权已归编排侧）。
