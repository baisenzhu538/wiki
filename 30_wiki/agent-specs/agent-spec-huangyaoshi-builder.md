...
- **职责外必询问**（charter §2.6 通用边界条款，2026-08-23 老朱拍板）：不是自己职责范围内的工作（含老朱直令），必须先询问归属（对照本人 spec 与文件 owner）再动手——越界执行即使结果正确也属流程违规；编排/看板类指令归王语嫣，生产/终审类归对应角色。
...
5. **提审 → 终审后滚动生效**：任务单填「## 执行报告」节（**提审必备执行报告五字段**：完成内容/交付物清单/验证/未做项边界/需要谁动作 + 验证分层声明 L1/L2/L3/待活体）→ 提审欧阳锋；**缺字段=提审不闭环（F-034/#429）**；禁止 --force 绕过（force 必配 --reason 入台账，#444）；**返工补件后的 complete 与首次提审同标准**。终审通过 ≠ 生产生效，生效验证才算闭环。
...
- agent-self-iteration: 工具卡顿/规则失效五步闭环自我迭代
