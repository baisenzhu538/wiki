...
公开的通用知识（AI 训练语料里已有的）越来越不值钱。值钱的是**只有你有的、AI 没见过的、需要提炼才能用的**数据。

### KDO 已经积攒但从未处理的四类暗知识

#### 类型 1：纠偏过程数据 — 最值钱，全部遗漏

| 已有数据 | 位置 | 当前状态 | 暗知识价值 | 
|---------|------|---------|-----------|
| **corrections.md**（12条踩坑记录） | `20_memory/corrections.md` | 纯文本，无结构 | 每条 = 1个 `error_data` chunk：症状→根因→修正→再犯后果 |
| **failure-modes.md**（22种失败模式） | `90_control/failure-modes.md` | 纯表格，无 chunk | 每条 = 1个 `error_data` chunk：触发条件→表现→根因→防御 |
| **pitfalls.md**（15条踩坑） | `.agent/pitfalls.md` | 纯叙述，无结构 | 每条 = 1个 `process_data` chunk：症状→根因→对策→关联 |
| **欧阳锋审查意见** | `70_product/tasks/` 各任务文件 | 散落各处 | 每条 = 1个 `error_data` chunk：谁审了什么→哪里不对→怎么改 |

**这些数据的共同特征**：它们就是 Truman 说的"Not Do List"——比正向案例更稀缺、更能纠正 AI 行为。全库 384 张概念卡没有一张引用了这些纠偏数据。

**实操案例**：C-10 "基础设施工具改后直接跑批量 → 71 张卡攻击者内容被清空"。这条如果被原子化标注：
```
chunk_id: correction-C-10/error_data/001
card_slug: correction-C-10
chunk_type: error_data
content: "改了 enrich 脚本后直接批量跑，结果 71 张卡的 Critique 部分内容被空字符串覆盖..."
inherited:
  tags: ["#source_type/error", "#method/evaluation-method", "#domain/knowledge-management"]
  perspectives:
    - 技术视角: "批量脚本修改后必须先 dry-run 单卡"
    - 流程视角: "基础设施变更触发强制审批节点"
    - 成本视角: "71 张卡修复成本 >> 单卡 dry-run 成本"
```

#### 类型 2：决策过程数据 — 有记录但不可检索

| 已有数据 | 位置 | 暗知识价值 |
|---------|------|-----------|
| **decisions.md**（架构决策记录） | `.agent/decisions.md` | 每条决策 = 背景→选项→选择→理由→后果 |
| **context.md** 历史决策区 | `.agent/context.md` | 跨会话的决策脉络 |
| **Oral transcripts**（口述稿） | `00_inbox/AI-study/` | Truman 的即兴专业口吻，未被萃取 |

**特别值钱的暗数据**：`00_inbox/AI-study/` 下的口述稿包含了 Truman 在非授课状态下的即兴专业判断（"楚门喷解放思想"、"起名 Trumen 的完整过程"、"跟周鸿祎连线直播的准备复盘"）。这些都还是一整坨文本，没有被切分、标注、关联。

#### 类型 3：Before/After 对比数据 — 完全缺失

课程中强调的 BA 对比（改之前 vs 改之后 + 为什么改），在 KDO 中：
- Git diff 有，但从未被结构化（"这个函数为什么从 A 改成 B"）
- 卡片版本迭代有，但从无 BA 记录（v1→v2 改了哪里？为什么？）
- 欧阳锋审查意见有，但从无"审查前 vs 审查后"对比

**这是 KDO 管线最大的数据盲区**。

#### 类型 4：多视角话术数据 — 从未尝试

同一条知识，面向不同受众应有不同话术。但 KDO 的每一张卡片只有一种写法：

| 卡片 | 当前只有 | 缺失的视角 |
|------|---------|-----------|
| `master-decision-hygiene` | 通用方法论表述 | CEO视角怎么说？一线执行者视角怎么说？ |
| `graph-rag` | 技术实现描述 | 非技术决策者视角：它能解决什么问题？能带来什么ROI？ |
| `yt-decision-y-model` | 课程笔记体 | 信这门课的人怎么用？怀疑这门课的人怎么反驳？ |
